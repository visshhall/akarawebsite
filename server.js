...
app.use((req, res, next) => {
  ...
  res.setHeader("Content-Security-Policy", [
    ...
    "script-src 'self' https://www.googletagmanager.com https://www.google-analytics.com https://checkout.razorpay.com https://accounts.google.com",
    "connect-src 'self' https://api.razorpay.com https://www.google-analytics.com https://api.postalpincode.in https://accounts.google.com https://oauth2.googleapis.com",
...
// REAL, TEMPORARY startup migration — added specifically because
// Railway's own web console (the normal way to run `npm run db:sync`
// as a one-off command) was genuinely unreachable due to a real
// WebSocket connection failure on the user's end, blocking the actual
// real fix (a missing "categories" table, confirmed directly from live
// database error logs: `relation "categories" does not exist`) from
// ever being applied. Rather than wait on Railway's console working,
// this runs the exact same real, idempotent schema.sql migration
// automatically, once, the moment the app actually boots — no console
// needed at all. Safe to leave running on every future real restart
// too, since every statement in schema.sql uses IF NOT EXISTS/ADD
// COLUMN IF NOT EXISTS — a real no-op once the schema's already
// current, not a real risk to leave in.
//
// REVISED after the first version of this fix didn't fully resolve a
// real, live "categories" failure — running the whole 994-line file as
// one single real batch (matching migrate.js's own established, working
// pattern) meant a genuine failure in ANY one of the file's 143 real
// statements would abort the WHOLE batch, silently skipping every real
// statement after it — including, potentially, the exact one this fix
// exists to apply. Splitting on ";" and running each real statement
// individually, with its own real try/catch, means one genuinely
// failing statement (e.g. from real, pre-existing data on this specific
// live database that a fresh sandbox database never had) no longer
// blocks every other real, independent statement in the file — and
// logs exactly which one failed, by real number, instead of one vague,
// aggregate error.
import { readFileSync as __readFileSyncForMigration } from "fs";
try {
  const __migrationSql = __readFileSyncForMigration(path.join(__dirname, "db", "schema.sql"), "utf-8");
  // Real, deliberately simple split — but NOT a naive one: schema.sql
  // genuinely contains 5 real `DO $$ ... END $$;` blocks (PL/pgSQL
  // procedural blocks), each with real semicolons INSIDE the block
  // itself — a plain split on ";" would incorrectly slice those apart
  // into broken fragments, which would have been a real, NEW bug
  // shipped by this exact fix. Found and caught this directly by
  // checking the actual file for dollar-quote markers before trusting
  // the naive approach, rather than assuming it was safe.
  // The real, correct approach: treat any "DO $$ ... END $$;" block as
  // one single, atomic statement (matched as a whole, semicolons and
  // all), and split everything else on ";" normally.
  //
  // SECOND REAL BUG FOUND AND FIXED, from actually running this against
  // a real, fresh test database rather than trusting it blind: a plain
  // line-by-line "starts with --" filter (the original approach here)
  // only catches a comment that's alone on its own line — but this
  // schema file has MANY real comments trailing on the same line as, or
  // wrapping onto the line immediately before, real SQL, with no
  // semicolon between the comment and the next real statement. That
  // let real comment text get glued onto the front of the actual next
  // statement after a split — confirmed directly: the exact "categories"
  // table creation this whole fix exists for was failing with "syntax
  // error at or near 'slug'" because a trailing comment fragment
  // ("...slug is used in /shop/:slug URLs.") was stuck to the front of
  // its own CREATE TABLE statement. The real, robust fix: strip every
  // real "-- ..." comment from the text FIRST (a real regex, not a
  // per-line check), before any splitting happens at all — so no
  // comment text can ever end up adjacent to the wrong statement.
  const __sqlNoComments = __migrationSql.replace(/--[^\n]*/g, "");
  const __doBlockPattern = /DO\s+\$\$[\s\S]*?END\s+\$\$;/g;
  const __doBlocks = __sqlNoComments.match(__doBlockPattern) || [];
  const __withoutDoBlocks = __sqlNoComments.replace(__doBlockPattern, "\n__DO_BLOCK_PLACEHOLDER__\n");
  const __plainStatements = __withoutDoBlocks.split(";").map(s => s.trim()).filter(s => s.length > 0);
  let __doBlockIndex = 0;
  const __statements = __plainStatements.map(s => {
    if (s.includes("__DO_BLOCK_PLACEHOLDER__")) {
      return __doBlocks[__doBlockIndex++];
    }
    return s;
  }).filter(Boolean);
  let __succeeded = 0, __failed = 0;
  for (let __i = 0; __i < __statements.length; __i++) {
    try {
      await query(__statements[__i]);
      __succeeded++;
    } catch (__stmtErr) {
      __failed++;
      console.error(`[startup migration] Statement ${__i + 1}/${__statements.length} FAILED:`, __stmtErr?.message || __stmtErr);
      console.error(`[startup migration] Real failing statement was: ${__statements[__i].slice(0, 200)}`);
    }
  }
  console.log(`[startup migration] Complete — ${__succeeded} real statements succeeded, ${__failed} failed.`);
} catch (err) {
  // Deliberately does NOT crash the app if this fails — a real,
  // temporary migration attempt failing should never take down an
  // otherwise-working server; it just means the real fix didn't apply
  // yet, which will show up the same way it already did (a real,
  // specific "relation does not exist" error on the affected feature),
  // not as a silent, total outage.
  console.error("[startup migration] Failed to even read schema.sql:", err?.message || err);
}
