...
class ErrorBoundary extends Component {
  constructor(props){ super(props); this.state={hasError:false, path: typeof window!=="undefined"?window.location.pathname:""}; }
  static getDerivedStateFromError(){ return {hasError:true}; }
  static getDerivedStateFromProps(props, state){
    // When the user navigates away, clear the error so the SPA recovers
    // without a full browser reload.
    const path = typeof window!=="undefined"?window.location.pathname:"";
    if (state.hasError && path !== state.path) return { hasError:false, path };
    if (!state.hasError && path !== state.path) return { path };
    return null;
  }
  ...
  render(){
    ...
    return <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"24px",textAlign:"center",backgroundColor:"#E3DAC9",fontFamily:"'Space Grotesk',system-ui,sans-serif"}}>
      ...
      <p style={{fontSize:"14px",color:"rgba(36,62,65,0.75)",maxWidth:"420px",marginBottom:"28px",lineHeight:1.7}}>
        This page hit an unexpected error. Try going home or reloading — if it keeps happening, email support@akaraonline.co.in.
      </p>
      <div style={{display:"flex",gap:"12px",flexWrap:"wrap",justifyContent:"center"}}>
        <button onClick={()=>{ this.setState({hasError:false}); window.location.href="/"; }} style={{padding:"14px 28px",fontSize:"12px",letterSpacing:"0.14em",textTransform:"uppercase",backgroundColor:"#183630",color:"white",border:"none",cursor:"pointer"}}>Go home</button>
