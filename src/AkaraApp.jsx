...
import { T, FONTS, sanitize, apiFetch, getCsrfToken, Mac, SweepButton, InputField } from "./shared.jsx";
...
function CartView({ navigate, cart, setCart, appliedCoupon, setAppliedCoupon }) {
  const [coupon,setCoupon]=useState(appliedCoupon||""); const [couponErr,setCouponErr]=useState("");
  ...
  const discount=appliedCoupon==="AKARA10"?Math.round(subtotal*0.1):0;
  ...
  const applyCoupon=()=>{
    ...
    if(code==="AKARA10"){setAppliedCoupon(code);setCouponErr("");}
...
function CheckoutView({ navigate, cart, setCart, setOrder, appliedCoupon, setAppliedCoupon, user, setPostLoginRedirect }) {
  ...
  // the real numbers come back from the server's response.
  const discount=appliedCoupon==="AKARA10"?Math.round(subtotal*0.1):0;
  const afterDiscount=subtotal-discount;
  const shipping=afterDiscount===0||afterDiscount>=2500?0:150;
  const gst=Math.round((afterDiscount+shipping)*0.18);
    ...
    try{
      const res=await apiFetch("/api/orders/checkout",{
        ...
        body:JSON.stringify({
          ...
          couponCode: appliedCoupon || undefined, // only the CODE is sent — the server computes the actual discount itself, never trusts a client-sent amount
        }),
      });
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-14">
      ...
      <div className="h-fit p-7" style={{backgroundColor:T.card,boxShadow:"0 8px 24px -14px rgba(36,62,65,0.14),0 0 0 1px rgba(36,62,65,0.06)"}}>
        ...
        <div className="pt-4 mt-3 flex flex-col gap-2.5 text-[13.5px]" style={{borderTop:"1px solid rgba(36,62,65,0.12)",color:"rgba(36,62,65,0.7)"}}>
          ...
          {discount>0&&<div className="flex justify-between" style={{color:T.gold}}><span>Discount ({appliedCoupon})</span><span>−₹{discount.toLocaleString("en-IN")}</span></div>}
...
function MyAccountView({ navigate, wishlist, setWishlist, user, order, setOrder, initTab }) {
  const { products } = useProducts();
  ...
  return <div className="px-6 md:px-14 py-16 max-w-[1000px] mx-auto">
    ...
    {tab==="Orders"&&(!user
      ...
      :<div className="flex flex-col gap-4">
        {orders.map(o=><div key={o.orderNumber} className="p-6" style={{backgroundColor:T.card,boxShadow:"0 0 0 1px rgba(36,62,65,0.08)"}}>
          <div className="flex items-start justify-between mb-4 flex-wrap gap-3">
            ...
            <span className="text-[10.5px] uppercase tracking-[0.08em] px-3 py-1.5" style={o.paymentStatus==="paid"?{backgroundColor:"rgba(184,147,90,0.14)",color:T.gold}:{backgroundColor:"rgba(192,57,43,0.08)",color:T.error}}>
              {o.paymentStatus==="paid"?(stageIndexFromOrder(o)>=4?"Delivered":stageIndexFromOrder(o)>=3?"In Transit":"Processing"):o.paymentStatus==="failed"?"Payment Failed":"Payment Pending"}
            </span>
          </div>
...
function AkaraAppRoot() {
  ...
  <div className="min-h-screen w-full" style={{backgroundColor:T.cream,fontFamily:"'Space Grotesk',system-ui,sans-serif",color:T.teal}}>
    ...
    <main id="main-content" key={`${view}-${productId}-${shopCategory}-${searchQuery}`} className="akara-page-enter">
      ...
      {!notFound&&view==="account"&&<MyAccountView navigate={navigate} wishlist={wishlist} setWishlist={setWishlist} user={user} order={order} setOrder={setOrder} initTab={accountTab}/>}
...
export default function AkaraApp(){
  ...
  return <ErrorBoundary>
    <style>{FONTS}</style>
    {isAdmin
      ? <Suspense fallback={<div className="min-h-screen w-full" style={{backgroundColor:T.cream}}/>}><AdminApp/></Suspense>
      : <AkaraAppRoot/>}
  </ErrorBoundary>;
}
