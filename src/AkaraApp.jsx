...
async function downloadInvoicePDF(order){
  // Tax-invoice layout aligned to Precision Forge Labs (ĀKĀRA) print style:
  // teal header, cream company band, bill/ship, line table, tax box, payment/terms, declaration.
  ...
  const {
    subtotal = 0, discount = 0, shippingCost: shipCost = 0, codFee = 0,
    cgst = 0, sgst = 0, total: grandTotal = 0,
  } = order;
  const igst = 0; // Intra-state Maharashtra studio → CGST+SGST; IGST reserved if ever inter-state only
  ...
  const paid = !cancelled && (
    ...
    (order.paymentMethod === "cod" && ["dispatched", "delivered"].includes(order.status))
  );

  // Cream page + white card (matches on-screen tax invoice)
  doc.setFillColor(...CREAM);
  doc.rect(0, 0, 210, 297, "F");
  doc.setFillColor(...WHITE);
  doc.roundedRect(10, 12, 190, 273, 2, 2, "F");

  doc.setTextColor(...TEAL);
  doc.setFont("NotoSans", "bold");
  doc.setFontSize(16);
  doc.text("AKARA", 105, 24, { align: "center" });

  doc.setFont("NotoSans", "bold");
  doc.setFontSize(11);
  doc.text("AKARA", 18, 36);
  doc.setFont("NotoSans", "normal");
  doc.setFontSize(8);
  doc.setTextColor(...MUTED);
  doc.text("Precision Forge Labs", 18, 41);
  doc.text("Thane, Maharashtra 400601", 18, 45);
  doc.text("GSTIN: 27GZCPS9353H1ZQ", 18, 49);
  doc.text("support@akaraonline.co.in · +91 82780 85572", 18, 53);

  doc.setTextColor(...TEAL);
  doc.setFontSize(9);
  doc.text("TAX INVOICE", 192, 36, { align: "right" });
  doc.setFontSize(8.5);
  doc.text(`Invoice #: ${order.orderNumber || "—"}`, 192, 42, { align: "right" });
  doc.text(`Date: ${invoiceDate || "—"}`, 192, 48, { align: "right" });

  // —— Divider + billed & shipped (matches on-screen tax invoice) ——
  let y = 60;
  ...
  doc.setFontSize(7.5);
  doc.setTextColor(...MUTED);
  doc.text("BILLED & SHIPPED TO", 18, y);
  y += 6;
  const billName = order.name || order.customerName || "Customer";
  doc.setFont("NotoSans", "bold");
  doc.setFontSize(10);
  doc.setTextColor(...TEAL);
  doc.text(String(billName).slice(0, 60), 18, y);
  y += 5;
  doc.setFont("NotoSans", "normal");
  doc.setFontSize(8.5);
  doc.setTextColor(...MUTED);
  ...
  for (const line of doc.splitTextToSize(addrBits, 170)) {
    doc.text(line, 18, y);
    y += 4.5;
  }
  if (order.phone) { doc.text(String(order.phone), 18, y); y += 4.5; }
  if (order.email) { doc.text(String(order.email), 18, y); y += 4.5; }
  y += 6;

  // —— Line items ——
  // Prices on site are GST-inclusive; taxable = amount / 1.18 when tax present
  const taxRate = totalTax > 0 && subtotal > 0 ? 18 : 0;
  const body = (order.items || []).map((it, idx) => {
    const lineGross = Number(it.price) * Number(it.qty || 1);
    const taxable = taxRate ? lineGross / 1.18 : lineGross;
    const unitTaxable = taxRate ? Number(it.price) / 1.18 : Number(it.price);
    const desc = `${it.name || "Piece"}${[it.colorLabel, it.size].filter(Boolean).length ? " (" + [it.colorLabel, it.size].filter(Boolean).join(", ") + ")" : ""}${it.batchNumber ? " · " + it.batchNumber : ""}`;
    return [
      String(idx + 1),
      desc,
      String(it.hsn || "3924"),
  ...
  // Taxable subtotal for display (before tax)
  const taxableSubtotal = taxRate
    ? (Number(subtotal) - Number(discount)) / 1.18
    : Number(subtotal) - Number(discount);

  // —— TAX CALCULATION (left) + totals (right) ——
  const taxBoxTop = y;
  doc.setFillColor(...TEAL);
  doc.rect(12, taxBoxTop, 70, 6, "F");
  doc.setTextColor(...WHITE);
  doc.setFont("NotoSans", "bold");
  doc.setFontSize(7.5);
  doc.text("TAX CALCULATION", 14, taxBoxTop + 4);
  doc.setDrawColor(...LINE);
  doc.setFillColor(252, 249, 243);
  doc.rect(12, taxBoxTop + 6, 70, 22, "FD");
  doc.setFont("NotoSans", "normal");
  doc.setFontSize(7);
  doc.setTextColor(...MUTED);
  doc.text("Intra-State · CGST + SGST @ 9% each", 14, taxBoxTop + 11);
  doc.setTextColor(...TEAL);
  doc.setFont("NotoSans", "bold");
  doc.text("Taxable", 14, taxBoxTop + 17);
  doc.text("CGST 9%", 36, taxBoxTop + 17);
  doc.text("SGST 9%", 54, taxBoxTop + 17);
  doc.setFont("NotoSans", "normal");
  doc.setTextColor(40, 45, 42);
  doc.text(`Rs.${taxableSubtotal.toFixed(2)}`, 14, taxBoxTop + 23);
  doc.text(`Rs.${Number(cgst).toFixed(2)}`, 36, taxBoxTop + 23);
  doc.text(`Rs.${Number(sgst).toFixed(2)}`, 54, taxBoxTop + 23);

  // Right totals
  const totX = 110;
  const rows = [];
  rows.push(["Subtotal (Before Tax):", `Rs.${taxableSubtotal.toFixed(2)}`]);
  if (discount > 0) rows.push([`Discount${order.couponCode ? ` (${order.couponCode})` : ""}:`, `- Rs.${Number(discount).toFixed(2)}`]);
  rows.push([`CGST @ 9%:`, `Rs.${Number(cgst).toFixed(2)}`]);
  rows.push([`SGST @ 9%:`, `Rs.${Number(sgst).toFixed(2)}`]);
  ...
  rows.forEach((r, i) => {
    const yy = taxBoxTop + i * 5.5;
    doc.setFont("NotoSans", "normal");
    doc.setFontSize(7.5);
    doc.setTextColor(...MUTED);
    doc.text(r[0], totX, yy + 4);
    doc.setTextColor(40, 45, 42);
    doc.text(r[1], 198, yy + 4, { align: "right" });
  });
  const grandY = taxBoxTop + rows.length * 5.5 + 2;
  doc.setFillColor(...TEAL);
  doc.rect(totX, grandY, 88, 9, "F");
  doc.setTextColor(...WHITE);
  doc.setFont("NotoSans", "bold");
  doc.setFontSize(9);
  doc.text("GRAND TOTAL:", totX + 3, grandY + 6);
  doc.text(`Rs.${Number(grandTotal).toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, 196, grandY + 6, { align: "right" });

  // Amount in words (simple)
  const wordsY = grandY + 14;
  doc.setFont("NotoSans", "normal");
  doc.setFontSize(7);
  doc.setTextColor(...MUTED);
  doc.text(`Total tax: Rs.${totalTax.toFixed(2)}  ·  Inclusive of GST where applicable`, 12, wordsY);
  ...
  if (cancelled) {
    ...
    doc.text("CANCELLED", 105, 160, { align: "center", angle: 28 });
  } else if (paid) {
    doc.setDrawColor(80, 140, 100);
    doc.setLineWidth(1.2);
    doc.setTextColor(80, 140, 100);
    doc.setFont("NotoSans", "bold");
    doc.setFontSize(22);
    // approximate stamp near table center
    doc.text("PAID", 105, 118, { align: "center", angle: 18 });
    doc.setFontSize(8);
    doc.text(`AS OF ${invoiceDate}`, 105, 126, { align: "center", angle: 18 });
  }

  // —— Payment + Terms ——
  let footY = Math.max(wordsY + 8, grandY + 18);
  if (footY > 230) footY = 230;
  doc.setFillColor(...TEAL);
  doc.rect(12, footY, 93, 6, "F");
  doc.rect(105, footY, 93, 6, "F");
  doc.setTextColor(...WHITE);
  doc.setFont("NotoSans", "bold");
  doc.setFontSize(7.5);
  doc.text("PAYMENT DETAILS", 14, footY + 4);
  doc.text("TERMS & CONDITIONS", 107, footY + 4);
  footY += 6;
  const payH = 28;
  doc.setFillColor(252, 249, 243);
  doc.setDrawColor(...LINE);
  doc.rect(12, footY, 93, payH, "FD");
  doc.rect(105, footY, 93, payH, "FD");
  doc.setFont("NotoSans", "normal");
  doc.setFontSize(7);
  doc.setTextColor(40, 45, 42);
  const payLines = [
    ...
    order.paymentMethod === "cod" ? "Method: Cash on Delivery" : "Method: Online (Razorpay)",
  ];
  payLines.forEach((l, i) => doc.text(l, 14, footY + 4.5 + i * 3.8));
  const terms = [
    ...
    paid ? "5. Payment received." : (order.paymentMethod === "cod" ? "5. Collect on delivery." : "5. Awaiting payment confirmation."),
  ];
  terms.forEach((l, i) => doc.text(l, 107, footY + 4.5 + i * 3.8));

  // —— Declaration ——
  footY += payH + 6;
  doc.setFillColor(...CREAM_BAND);
  doc.rect(12, footY, 186, 18, "F");
  doc.setFont("NotoSans", "bold");
  doc.setFontSize(7);
  doc.setTextColor(...TEAL);
  doc.text("DECLARATION", 14, footY + 5);
  doc.setFont("NotoSans", "normal");
  doc.setFontSize(6.5);
  doc.setTextColor(...MUTED);
  doc.text("We declare that this invoice shows the actual price of the goods described and that all particulars are true and correct.", 14, footY + 10);
  doc.text("Studio piece · Printed to order in Mumbai · Not white-labelled stock.", 14, footY + 14.5);
  ...
  doc.text("This is a digitally generated invoice and does not require a physical signature.", 14, footY + 17.5);
  ...
  doc.setFontSize(6.5);
  doc.setTextColor(...MUTED);
  doc.text("www.akaraonline.co.in  |  support@akaraonline.co.in  |  +91-8278085572", 105, 289, { align: "center" });
...
function InvoiceView({ navigate, order }){
  ...
  return <div className="px-4 sm:px-6 md:px-14 py-10 max-w-[860px] mx-auto" style={{backgroundColor:"#F5F0E8"}}>
    <div className="no-print flex justify-between items-center mb-8 flex-wrap gap-3">
      ...
      <div className="flex gap-3">
        ...
        {/* Packing slip is admin/ops only — not shown to customers */}
      </div>
    </div>
    <div className="invoice-print-area p-8 md:p-12 relative" style={{backgroundColor:"white",boxShadow:ELEVATION.raised,borderRadius:RADIUS.md,overflow:"hidden"}}>
      ...
      <p className="text-center text-[26px] mb-8" style={{fontFamily:"'Fraunces',serif",fontStyle:"italic",color:T.teal,letterSpacing:"0.04em"}}><Mac>A</Mac>K<Mac>A</Mac>RA</p>
      <div className="flex justify-between items-start mb-8 pb-6 flex-wrap gap-6 relative" style={{borderBottom:`1px solid ${T.teal}`,zIndex:2}}>
        <div>
          <p className="text-[18px] mb-1" style={{fontFamily:"'Fraunces',serif",fontStyle:"italic",color:T.teal}}><Mac>A</Mac>K<Mac>A</Mac>RA</p>
          <p className="text-[12px]" style={{color:"rgba(36,62,65,0.7)"}}>Precision Forge Labs</p>
          <p className="text-[12px]" style={{color:"rgba(36,62,65,0.7)"}}>Thane, Maharashtra 400601</p>
          <p className="text-[12px]" style={{color:"rgba(36,62,65,0.7)"}}>GSTIN: 27GZCPS9353H1ZQ</p>
          <p className="text-[12px]" style={{color:"rgba(36,62,65,0.7)"}}>support@akaraonline.co.in · +91 82780 85572</p>
        </div>
        <div className="text-right">
          <p className="text-[11px] uppercase tracking-[0.12em] mb-2" style={{color:T.teal}}>Tax Invoice</p>
...
// ============================================================================
// SHIPMENT TRACKING MAP — CSP-safe SVG route (no Leaflet / no external map tiles).
// WHAT: Studio (Mumbai) → customer destination with progress from order + courier status.
// WHY: Real-time feel using Shiprocket scans + status without violating CSP or paid map keys.
// HOW: Approximate India projection; progress 0–1; optional live refresh via POST /api/orders/tracking/refresh.
// CONNECTED TO: order.courier*; server/shiprocket fetchTrackingStatus; webhook courier_events.
// ============================================================================
const AKARA_ORIGIN = { lat: 19.076, lng: 72.8777, label: "Ākāra studio · Mumbai" };
const STATE_COORDS = {
  "Maharashtra":[19.75,75.7],"Delhi":[28.61,77.21],"Karnataka":[12.97,77.59],"Tamil Nadu":[13.08,80.27],
  "Telangana":[17.39,78.49],"Gujarat":[23.02,72.57],"Rajasthan":[26.91,75.79],"West Bengal":[22.57,88.36],
  "Uttar Pradesh":[26.85,80.95],"Madhya Pradesh":[23.26,77.41],"Kerala":[8.52,76.94],"Punjab":[30.9,75.86],
  "Haryana":[28.46,77.03],"Bihar":[25.59,85.14],"Odisha":[20.3,85.82],"Assam":[26.14,91.74],
  "Andhra Pradesh":[16.51,80.63],"Chhattisgarh":[21.25,81.63],"Jharkhand":[23.34,85.31],"Goa":[15.49,73.83],
};
