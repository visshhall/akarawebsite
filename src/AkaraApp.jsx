...
function enrichProduct(p) {
  ...
  if(hasQty && p.status!=="draft" && p.status!=="hidden" && p.status!=="pre-order" && !hasRealSizes){
    if(qtyRaw<=0) cardStatus="sold-out";
    else if(qtyRaw<=10){ cardStatus="low-stock"; stockRemaining=qtyRaw; }
    else if(qtyRaw<=20) cardStatus="low-stock";
...
function CartDrawer({ open, onClose, cart, setCart, navigate }) {
  ...
  const updateQty=(id,size,color,d)=>setCart(c=>c.map(i=>sameLine(i,id,size,color)?{...i,qty:Math.min(99,Math.max(0,i.qty+d))}:i).filter(i=>i.qty>0));
  return <>
...
function NewsletterForm() {
  ...
  return <div>
    <form onSubmit={submit} className="flex" style={{border:"1px solid rgba(255,255,255,0.15)"}}>
      ...
      <button type="submit" disabled={submitting} className="px-4 py-3 text-[11px] tracking-[0.1em] uppercase shrink-0 transition-opacity hover:opacity-90 disabled:opacity-50" style={{backgroundColor:T.cream,color:T.teal,borderRadius:6,fontWeight:500}}>{submitting?"…":"Join"}</button>
    </form>
...
function Footer({ navigate }) {
  ...
  return <footer className="no-print" style={{backgroundColor:T.teal}}>
    <div className="px-8 md:px-14 pt-14 pb-10 max-w-[1800px] mx-auto">
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-x-8 gap-y-10 items-start">
        <div className="sm:col-span-2 md:col-span-1">
          ...
          <p className="text-[13.5px] leading-[1.85] mb-6 max-w-[220px]" style={{color:"rgba(227,218,201,0.75)"}}>
            {fb.tagline}
          </p>
...
function ShopView({ navigate, cart, setCart, wishlist, toggleWishlist, initCategory }) {
  ...
  const filtered=(()=>{
    ...
    // "featured" keeps API/catalogue order
    return list;
  })();
    ...
    <section className="px-6 md:px-14 pt-14 pb-8 text-center">
      ...
      {activeCat&&activeCat!=="All"&&(
        <p className="max-w-[480px] mx-auto text-[12.5px] mt-4 leading-relaxed" style={{color:"rgba(36,62,65,0.65)"}}>
          Each piece is printed and finished to order in our Mumbai studio — typically 2–3 weeks. Not warehouse stock.
        </p>
      )}
    </section>
    <section className="px-6 md:px-10 xl:px-16 max-w-[1800px] mx-auto">
      <div className="flex flex-nowrap gap-2.5 pb-7 overflow-x-auto scrollbar-thin -mx-1 px-1" style={{borderBottom:"1px solid rgba(36,62,65,0.1)",WebkitOverflowScrolling:"touch"}}>
        {["All",...categoryNames].map(c=><button key={c} onClick={()=>setActiveCat(c)}
          className="px-5 py-2.5 text-[11px] tracking-[0.1em] uppercase transition-all duration-200 shrink-0 whitespace-nowrap"
          style={activeCat===c?{backgroundColor:T.teal,color:T.cream,borderRadius:999,boxShadow:"0 8px 20px -12px rgba(24,54,48,0.45)"}:{border:"1px solid rgba(36,62,65,0.16)",borderRadius:999,color:T.teal,background:"transparent"}}>{c}</button>)}
      </div>
      <div className="pt-5 pb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-10">
        ...
        filtered.length===0
          ?<div className="col-span-full text-center py-20 px-4">
              ...
              <p className="italic text-[22px] mb-2" style={{fontFamily:"'Fraunces',serif",color:T.teal}}>No pieces in this view</p>
              <p className="text-[13.5px] mb-6 max-w-sm mx-auto" style={{color:"rgba(36,62,65,0.65)"}}>Try another category, or see the full collection.</p>
...
function ProductDetailView({ productId, navigate, cart, setCart, wishlist, toggleWishlist }) {
  ...
  const addToCart=()=>{ if(soldOut||displayPrice==null) return;
    ...
    const ex=base.find(i=>i.id===product.id&&i.size===size&&(i.color||null)===(color||null));
    if(ex) return base.map(i=>i.id===product.id&&i.size===size&&(i.color||null)===(color||null)?{...i,qty:i.qty+1}:i);
    ...
    {tab==="Wishlist"&&(wishedProducts.length===0
      ...
      :<div className="akara-product-grid grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
        {wishedProducts.map(p=>{
          ...
          <div className="relative flex items-center justify-center mb-3 overflow-hidden" style={{aspectRatio:"4/5",backgroundColor:T.card,boxShadow:ELEVATION.raised,borderRadius:RADIUS.md}}>
            <StockBadge status={p.cardStatus}/>
            {firstImage
              ...
              :(()=>{ const Art=p.Art||PlanterArt; return <Art className="w-1/3 h-1/3 transition-transform group-hover:scale-110 duration-500" style={{color:T.teal,opacity:0.85}}/>; })()}
          </div>
...
function loadStoredCart(products){
  ...
  for(const entry of stored){
    ...
    if(!product) continue;
    let qty=Number.isInteger(entry.qty)&&entry.qty>0&&entry.qty<=99?entry.qty:1;
    ...
    if(merged.has(key)){
      ...
      prev.qty=Math.min(99, Math.max(prev.qty, qty));
      continue;
    }
...
function AkaraAppRoot() {
  ...
  const logout=useCallback(()=>{
    ...
    setUser(null);
    try{ sessionStorage.removeItem("akara_last_order_number"); }catch{}
    navigate("home");
