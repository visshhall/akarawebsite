...
function ProductGallery({ media, Art, name, soldOut }){
  ...
  return <div>
    <div className="relative mx-auto flex items-center justify-center overflow-hidden"
      ...
      {media.length>1&&(
        <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5 px-2 py-1.5" style={{background:"rgba(24,54,48,0.35)",borderRadius:999,backdropFilter:"blur(6px)"}} aria-hidden>
          {media.map((_,i)=><span key={i} className="block rounded-full transition-all duration-300" style={{width:i===active?14:5,height:5,backgroundColor:i===active?T.gold:"rgba(227,218,201,0.55)"}}/>)}
        </div>
      )}
    </div>
    ...
    {lightboxOpen&&item.type!=="video"&&item.src&&createPortal(
    ...
    {tab==="Wishlist"&&(wishedProducts.length===0
      ...
      :<div className="akara-product-grid grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
        {wishedProducts.map(p=>{
          ...
          <div className="relative flex items-center justify-center mb-3 overflow-hidden" style={{aspectRatio:"4/5"}} style={{backgroundColor:T.card,boxShadow:ELEVATION.raised,borderRadius:RADIUS.md}}>
            <StockBadge status={p.cardStatus}/>
            {firstImage
