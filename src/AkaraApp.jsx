    ...
    <div className="px-8 md:px-14 pt-14 pb-10 max-w-[1800px] mx-auto">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-10 lg:gap-x-12 gap-y-10 items-start">
        ...
        <div>
          ...
          {[["About","about"],["The Craft","craft"],["Contact","contact"],["FAQ","faq"],["Care Guide","care-guide"],["Interior projects & studios","bulk-orders"],["Track Your Order","track-order"],["Android App","android-app"],["My Account","account"]].map(([l,v])=><button key={v} onClick={()=>navigate(v)}
...
function CartView({ navigate, cart, setCart, appliedCoupon, setAppliedCoupon, user }) {
  ...
  const applyCoupon=async()=>{
    ...
    try{
      const res=await fetch(`/api/coupons/validate/${encodeURIComponent(code)}`,{credentials:"include"});
      const data=await res.json();
      if(data.valid){ setAppliedCoupon({code:data.code,discountPercent:data.discountPercent,maxDiscountAmount:data.maxDiscountAmount??null}); setCouponErr(""); }
...
async function downloadInvoicePDF(order){
  ...
  // Outer soft page margin background
  doc.setFillColor(...CREAM);
  doc.rect(0, 0, 210, 297, "F");
  doc.setFillColor(...WHITE);
  doc.roundedRect(8, 8, 194, 281, 2, 2, "F");

  // —— Header band ——
  doc.setFillColor(...TEAL);
  doc.rect(8, 8, 194, 22, "F");
  doc.setTextColor(...WHITE);
  doc.setFont("NotoSans", "bold");
  doc.setFontSize(18);
  doc.text("ĀKĀRA", 105, 18, { align: "center" });
  ...
  doc.rect(8, 30, 194, 18, "F");
  doc.setTextColor(...TEAL);
  doc.setFont("NotoSans", "bold");
  doc.setFontSize(9);
  doc.text("by PRECISION FORGE LABS", 105, 36, { align: "center" });
  doc.setFont("NotoSans", "normal");
  doc.setFontSize(7);
  doc.setTextColor(...MUTED);
  doc.text("Artifacts for Modern Spaces", 105, 40.5, { align: "center" });
  doc.text("Thane, Maharashtra - 400605 | Ph: +91-8278085572 | info@akaraonline.co.in", 105, 44.5, { align: "center" });
  doc.setFont("NotoSans", "bold");
  doc.setFontSize(6.5);
  doc.setTextColor(...TEAL);
  doc.text("GSTIN: 27GZCPS9353H1ZQ  |  PAN: GZCPS9353H  |  State: Maharashtra (27)", 105, 48, { align: "center" });
  ...
  const metaLeft = [
    ...
    ["Place of Supply:", [order.city, order.state].filter(Boolean).join(", ") || "—"],
  ];
  const metaRight = [
    ["Date:", invoiceDate || "—"],
  ...
  metaLeft.forEach((row, i) => {
    const yy = y + i * 6;
    doc.setFillColor(i % 2 === 0 ? 250 : 255, i % 2 === 0 ? 248 : 255, i % 2 === 0 ? 242 : 255);
    doc.rect(12, yy, 93, 6, "F");
    doc.rect(105, yy, 93, 6, "F");
    doc.setFont("NotoSans", "bold"); doc.setFontSize(7); doc.setTextColor(...TEAL);
    doc.text(row[0], 14, yy + 4);
    doc.setFont("NotoSans", "normal"); doc.setTextColor(40, 45, 42);
    doc.text(String(row[1]).slice(0, 42), 38, yy + 4);
    const r = metaRight[i];
    doc.setFont("NotoSans", "bold"); doc.setTextColor(...TEAL);
    doc.text(r[0], 107, yy + 4);
    doc.setFont("NotoSans", "normal"); doc.setTextColor(40, 45, 42);
    doc.text(String(r[1]).slice(0, 42), 135, yy + 4);
  });
  y += 20;
  ...
  const shipLines = billLines; // same destination unless separate ship fields exist later

  doc.setFillColor(...TEAL);
  doc.rect(12, y, 93, 6, "F");
  doc.rect(105, y, 93, 6, "F");
  doc.setTextColor(...WHITE);
  doc.setFont("NotoSans", "bold");
  doc.setFontSize(8);
  doc.text("BILL TO", 14, y + 4);
  ...
  doc.rect(12, y, 93, boxH, "FD");
  doc.rect(105, y, 93, boxH, "FD");
  doc.setFont("NotoSans", "normal");
  doc.setFontSize(7.5);
  doc.setTextColor(40, 45, 42);
  billLines.forEach((line, i) => doc.text(String(line).slice(0, 48), 14, y + 5 + i * 4));
  shipLines.forEach((line, i) => doc.text(String(line).slice(0, 48), 107, y + 5 + i * 4));
  y += boxH + 6;
