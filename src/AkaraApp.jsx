...
function ProductGallery({ media, Art, name, soldOut }){
  ...
  return <div>
    <div className="relative mx-auto flex items-center justify-center overflow-hidden"
      style={{backgroundColor:T.card,boxShadow:ELEVATION.raised,borderRadius:RADIUS.lg,aspectRatio:"4/5",width:"100%",maxWidth:"min(100%,512px)"}}>
      {/* maxWidth 512px keeps 4:5 height ≤640px without breaking aspect (old maxHeight+width:100% caused crop on wide screens) */}
      {item&&item.type==="video"
        ? (item.src
            ? <video src={item.src} controls playsInline className="w-full h-full object-contain" style={{opacity:soldOut?0.5:1,backgroundColor:T.card}}/>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-14">
      ...
      <div className="h-fit p-7" style={{backgroundColor:T.card,boxShadow:ELEVATION.raised,borderRadius:RADIUS.lg}}>
        ...
        <div className="mb-5">
          ...
          {!user&&!appliedCoupon&&!couponErr&&<p className="text-[11.5px] mt-2" style={{color:"rgba(36,62,65,0.55)"}}>Coupons are for signed-in customers — the field is visible so you can sign in and apply at checkout.</p>}
        </div>
...
function CheckoutView({ navigate, cart, setCart, setOrder, appliedCoupon, setAppliedCoupon, user, setPostLoginRedirect }) {
  ...
  const selectSavedAddress=id=>{
    ...
    if(a) setForm(f=>({...f, address:a.line, landmark:a.landmark||"", city:a.city, state:a.state||"", pin:a.pin, phone:a.phone, gstin:a.gstin||""}));
  };
  const subtotal=cart.reduce((s,i)=>s+i.price*i.qty,0);
...
function shipmentProgress(order) {
  ...
  if (/in transit|transit|reached|hub|departed|connected/.test(cs)) return 0.58;
  if (/picked|pickup|manifest|shipped|booked|assigned/.test(cs) || st === "dispatched") return 0.32;
  if (st === "qc") return 0.18;
  if (st === "production") return 0.12;
  if (st === "confirmed") return 0.06;
...
function ShipmentTrackingMap({ order, setOrder, guestToken }) {
  ...
  const stageLabel =
    ...
    progress >= 0.9 ? "Out for delivery" :
    progress >= 0.5 ? "In transit" :
    progress >= 0.3 ? "Dispatched" :
    progress >= 0.1 ? "In the atelier" : "Confirmed";

  // Named journey steps — always full-width, never a blank map with overlapping dots.
  const steps = [
    { key: "confirmed", label: "Confirmed", min: 0.05 },
    { key: "atelier", label: "Atelier", min: 0.12 },
    { key: "qc", label: "QC", min: 0.18 },
    { key: "dispatch", label: "Dispatched", min: 0.32 },
    { key: "transit", label: "In transit", min: 0.58 },
    { key: "ofd", label: "Out for delivery", min: 0.9 },
    { key: "delivered", label: "Delivered", min: 1 },
  ];
    ...
    <div className="mb-8 overflow-hidden" style={{ backgroundColor: T.card, borderRadius: RADIUS.lg, boxShadow: ELEVATION.raised, border: "1px solid rgba(36,62,65,0.08)" }}>
      ...
      {!order?.courierTrackingId && (
        <p className="px-5 pb-5 text-[12px]" style={{ color: "rgba(36,62,65,0.6)" }}>
          Progress follows atelier status until dispatch. After Shiprocket assigns an AWB, refresh for live courier scans.
        </p>
      )}
    </div>
  );
...
function OrderStatusView({ navigate, order, setOrder }) {
  ...
  return <div className="px-6 md:px-14 py-16 max-w-[820px] mx-auto">
    ...
    :<>
      ...
      {currentIdx>=3?<div className="p-5 md:p-6 mb-6" style={{backgroundColor:T.card,boxShadow:ELEVATION.raised,borderRadius:RADIUS.md,border:"1px solid rgba(24,54,48,0.08)"}}>
        <div className="flex items-center justify-between gap-3 flex-wrap mb-3">
          <div className="flex items-center gap-3"><Truck size={18} style={{color:T.teal}}/><div><p className="text-[14px]" style={{fontFamily:"'Fraunces',serif",color:T.teal}}>In transit</p><p className="text-[12px]" style={{color:"rgba(36,62,65,0.65)"}}>Courier handoff complete</p></div></div>
          {order.courierTrackingUrl
            ?<a href={order.courierTrackingUrl} target="_blank" rel="noopener noreferrer" className="text-[11.5px] uppercase tracking-[0.08em] px-3 py-2" style={{color:"#F5F0E8",backgroundColor:T.teal,borderRadius:RADIUS.xs}}>Track with courier</a>
            :<span className="text-[11.5px]" style={{color:"rgba(36,62,65,0.75)"}}>Link appears when courier assigns AWB</span>}
        </div>
        {(order.courierTrackingId||order.courierStatus)?(
          <div className="pt-3 text-[12.5px]" style={{borderTop:"1px solid rgba(36,62,65,0.1)",color:T.teal}}>
            {order.courierTrackingId&&<p className="flex flex-wrap items-center gap-2"><span style={{color:"rgba(36,62,65,0.55)"}}>AWB</span><span className="font-medium tracking-wide">{order.courierTrackingId}</span>
              <button type="button" className="text-[11px] uppercase tracking-[0.06em] underline" style={{color:T.gold}} onClick={()=>{try{navigator.clipboard.writeText(order.courierTrackingId); }catch{}}}>Copy</button></p>}
            {order.courierStatus&&<p className="mt-1.5"><span style={{color:"rgba(36,62,65,0.55)"}}>Status · </span>{order.courierStatus}</p>}
            {Array.isArray(order.courierEvents)&&order.courierEvents.length>0&&(
              <ul className="mt-3 space-y-1.5 max-h-32 overflow-y-auto text-[11.5px]" style={{color:"rgba(36,62,65,0.7)"}}>
                {order.courierEvents.slice(0,8).map((ev,i)=><li key={i}>{typeof ev==="string"?ev:(ev.status||ev.activity||JSON.stringify(ev))}</li>)}
              </ul>
            )}
          </div>
        ):<p className="text-[12px] pt-2" style={{color:"rgba(36,62,65,0.6)"}}>AWB will show here as soon as Shiprocket assigns one.</p>}
      </div>:<p className="text-[12.5px] mb-6" style={{color:"rgba(36,62,65,0.75)"}}>Production → QC → dispatch. Courier tracking (AWB) appears after dispatch.</p>}
...
function MyAccountView({ navigate, wishlist, user, setUser, order, setOrder, initTab }) {
  ...
  const [addrForm,setAddrForm]=useState({name:"",line:"",landmark:"",city:"",state:"",pin:"",phone:"",gstin:""});
  ...
  const saveAddress=async()=>{
    ...
    try{
      ...
      if(res.ok){
        ...
        setAddrForm({name:"",line:"",landmark:"",city:"",state:"",pin:"",phone:"",gstin:""});
  ...
  const startEditAddress=a=>{
    ...
    setAddrForm({name:a.name||"",line:a.line||"",landmark:a.landmark||"",city:a.city||"",state:a.state||"",pin:String(a.pin||""),phone:String(a.phone||""),gstin:String(a.gstin||"")});
    setAddrErrors({});
    ...
    {tab==="Addresses"&&<div className="w-full max-w-[900px]">
      {showAddrForm?<div id="akara-addr-form" className="p-6 mb-6" style={{backgroundColor:T.card,boxShadow:ELEVATION.raised,borderRadius:RADIUS.md,border:"1px solid rgba(24,54,48,0.08)"}}>
        ...
        <div className="flex gap-3">
          ...
          <SweepButton onClick={()=>{setShowAddrForm(false);setEditingAddrId(null);setAddrForm({name:"",line:"",landmark:"",city:"",state:"",pin:"",phone:"",gstin:""});setAddrErrors({});}}>Cancel</SweepButton>
        </div>
      </div>:<SweepButton onClick={()=>{setShowAddrForm(true);setEditingAddrId(null);setAddrForm({name:"",line:"",landmark:"",city:"",state:"",pin:"",phone:"",gstin:""});}}>+ Add New Address</SweepButton>}
