...
function ProductCard({ product, navigate, cart, setCart, wishlist, toggleWishlist }) {
  ...
  return <div className="group relative flex flex-col cursor-pointer transition-transform duration-500 ease-out"
    ...
    <div className="relative aspect-square flex items-center justify-center overflow-hidden transition-shadow duration-500"
      ...
      {firstImage
        ?<img src={firstImage} alt={name} loading="lazy" decoding="async" className="w-full h-full object-cover transition-transform duration-700 ease-out" style={{transform:hover?"scale(1.03)":"scale(1)"}}/>
    ...
    <div className="pt-4 flex items-start justify-between gap-2">
      <div>
        ...
        <p className="text-[10.5px] mt-1" style={{color:"rgba(36,62,65,0.55)"}}>{isSoldOut?"Currently unavailable":"Made to order · 2–3 weeks from print"}</p>
      </div>
  ...
  return <div>
    ...
    <section ref={heroRef}
      ...
      <div
        ...
        style={{background:"linear-gradient(to top, rgba(24,54,48,0.88) 0%, rgba(24,54,48,0.62) 45%, rgba(24,54,48,0.42) 100%)"}}
        aria-hidden
      />
      <div
        className="absolute inset-0 pointer-events-none hidden md:block"
        style={{background:"linear-gradient(to top, rgba(24,54,48,0.72) 0%, rgba(24,54,48,0.45) 42%, rgba(24,54,48,0.28) 100%)"}}
        aria-hidden
      />
    ...
    <section className="px-6 md:px-10 xl:px-16 max-w-[1800px] mx-auto">
      <div className="flex flex-nowrap gap-2.5 pb-7 overflow-x-auto scrollbar-thin -mx-1 px-1" style={{borderBottom:"1px solid rgba(36,62,65,0.1)",WebkitOverflowScrolling:"touch"}}>
        {["All",...categoryNames].map(c=><button key={c} onClick={()=>setActiveCat(c)}
          className="px-5 py-2.5 text-[12.5px] transition-all duration-200 shrink-0 whitespace-nowrap"
          style={activeCat===c?{backgroundColor:T.teal,color:"white",borderRadius:RADIUS.xs}:{border:"1px solid rgba(36,62,65,0.18)",borderRadius:RADIUS.xs,color:T.teal}}>{c}</button>)}
      </div>
...
function CheckoutView({ navigate, cart, setCart, setOrder, appliedCoupon, setAppliedCoupon, user, setPostLoginRedirect }) {
  ...
  if(cart.length===0){
    return <div className="px-6 py-24 text-center max-w-[420px] mx-auto">
      ...
      <p className="text-[13.5px] mb-8" style={{color:"rgba(36,62,65,0.75)"}}>Add a piece from the collection before checking out.</p>
      <SweepButton filled onClick={()=>navigate("shop")}>Explore the collection</SweepButton>
    </div>;
  }
...
function PushEnableButton(){
  ...
  const enable=async()=>{
    ...
    try{
      ...
      else if(result?.error==="denied"){ setStatus("denied"); showToast("Notifications blocked in browser settings.","error"); }
