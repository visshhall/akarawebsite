        ...
        return <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 xl:gap-12 items-center mb-10 md:mb-12">
            <button type="button" onClick={()=>navigate("product",hero.id)}
              ...
              {/* WHAT: Magazine hero fills the frame edge-to-edge (object-cover).
                  WHY: object-contain left wide cream sidebars; user wants full-bleed editorial.
                  HOW: cover + center; 4:5 box matches studio product crops. Upload 4:5 for best result. */}
              {heroImg
...
function CheckoutView({ navigate, cart, setCart, setOrder, appliedCoupon, setAppliedCoupon, user, setPostLoginRedirect }) {
  ...
  if(cart.length===0 && stage!=="processing"){
    return <div className="px-6 py-24 text-center max-w-[420px] mx-auto">
      <ShoppingBag size={36} strokeWidth={1} style={{color:"rgba(36,62,65,0.45)",margin:"0 auto 16px"}}/>
      <div className="w-16 h-16 mx-auto mb-5 flex items-center justify-center" style={{borderRadius:"50%",background:"rgba(24,54,48,0.06)",border:"1px solid rgba(24,54,48,0.1)"}}><ShoppingBag size={26} style={{color:T.teal,opacity:0.65}}/></div>
...
function ShipmentTrackingMap({ order, setOrder, guestToken }) {
  ...
  return (
    <div className="mb-8 overflow-hidden" style={{ backgroundColor: T.card, borderRadius: RADIUS.lg, boxShadow: ELEVATION.raised, border: "1px solid rgba(36,62,65,0.08)" }}>
      ...
      {/* Full-width progress rail — works even when origin & destination are the same city */}
      <div className="px-5 pb-4">
        <div className="relative pt-2 pb-1">
          <div className="absolute left-0 right-0 top-[18px] h-[3px]" style={{ backgroundColor: "rgba(36,62,65,0.12)" }} />
          <div
            className="absolute left-0 top-[18px] h-[3px] transition-all duration-700 ease-out"
            style={{
              // Width from step index so the bar ends under the active stage (QC → QC, not Atelier)
              width: `${Math.max(4, (activeIdx / Math.max(1, steps.length - 1)) * 100)}%`,
              ...
              return (
                <div key={s.key} className="flex flex-col items-center flex-1 min-w-0">
                  <div
                    ...
                    style={{
                      backgroundColor: done ? (current ? T.gold : T.teal) : T.cream,
                      borderColor: done ? (current ? T.gold : T.teal) : "rgba(36,62,65,0.25)",
                      boxShadow: current ? `0 0 0 4px rgba(196,163,90,0.25)` : "none",
                    }}
                  />
                  <span
                    className="text-[9px] sm:text-[10px] tracking-[0.04em] text-center leading-tight px-0.5"
