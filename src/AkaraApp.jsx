...
function ProductCard({ product, navigate, cart, setCart, wishlist, toggleWishlist }) {
  ...
  return <div className="group relative flex flex-col cursor-pointer transition-transform duration-500 ease-out"
    ...
    <div className="relative aspect-square flex items-center justify-center overflow-hidden transition-shadow duration-500"
      ...
      {firstImage
        ?<img src={firstImage} alt={name} loading="lazy" className="w-full h-full object-cover transition-transform duration-700 ease-out" style={{transform:hover?"scale(1.06)":"scale(1)"}}/>
...
function ProductGallery({ media, Art, name, soldOut }){
  ...
  // Real click-to-zoom for customers — directly requested. object-contain
  // (this component's own real fix, see the change right above) already
  // stops real photos from losing their edges to cropping, but a shopper
  // genuinely deciding on a real purchase still benefits from seeing a
  // photo larger than this fixed, 520px-max gallery box allows.
  ...
  return <div>
    <div className="relative flex items-center justify-center" style={{backgroundColor:T.card,boxShadow:ELEVATION.raised,borderRadius:RADIUS.lg,aspectRatio:"1/1",maxHeight:"520px",width:"100%",overflow:"hidden"}}>
      {item.type==="video"
        ? (item.src
            ? <video src={item.src} controls className="w-full h-full object-contain" style={{opacity:soldOut?0.5:1}}/>
        ...
        : (item.src
            ? <img src={item.src} alt={`${name} — photo ${active+1}`} className="w-full h-full object-contain cursor-zoom-in" style={{opacity:soldOut?0.5:1}} onClick={()=>!soldOut&&setLightboxOpen(true)}/>
  ...
  return <div>
    ...
    <section className="px-6 md:px-14 pt-8 pb-16 max-w-[1600px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-14">
      ...
      <div>
        ...
        <p className="text-[11.5px] mb-6" style={{color:"rgba(36,62,65,0.75)"}}>Excl. GST · added at checkout</p>
