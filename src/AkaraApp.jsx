                  ...
                  {pair.map(p=>{
                    ...
                    return (
                      <button key={p.id} type="button" onClick={()=>navigate("product",p.id)} className="text-left group">
                        ...
                        <p className="text-[12px]" style={{color:T.muted}}>{p.hasRealPriceRange?"From ":""}₹{Number(p.gstInclusiveDisplayPrice??p.price).toLocaleString("en-IN")}</p>
                      </button>
                    );
                  })}
                </div>
...
function OrderStatusView({ navigate, order, setOrder }) {
  ...
  return <div className="px-6 md:px-14 py-16 max-w-[820px] mx-auto">
    ...
    :<>
      ...
      {currentIdx>=3?<div className="p-5 mb-6" style={{backgroundColor:T.card,boxShadow:ELEVATION.raised,borderRadius:RADIUS.md}}>
        <div className="flex items-center justify-between gap-3 flex-wrap mb-3">
          ...
          {order.courierTrackingUrl
            ?<a href={order.courierTrackingUrl} target="_blank" rel="noopener noreferrer" className="text-[11.5px] uppercase tracking-[0.08em]" style={{color:T.teal}}>Open courier tracking →</a>
        ...
        {(order.courierTrackingId||order.courierStatus)&&(
          <div className="text-[12.5px] mb-2" style={{color:"rgba(36,62,65,0.8)"}}>
            {order.courierTrackingId&&!String(order.courierTrackingId).startsWith("SR-")&&(
              <p className="mb-1"><span style={{color:"rgba(36,62,65,0.55)"}}>AWB / Tracking no.</span> <strong style={{color:T.teal}}>{order.courierTrackingId}</strong></p>
            )}
            {order.courierStatus&&(
              <p><span style={{color:"rgba(36,62,65,0.55)"}}>Courier status</span> <strong style={{color:T.teal}}>{order.courierStatus}</strong></p>
            )}
          </div>
        )}
        {Array.isArray(order.courierEvents)&&order.courierEvents.length>0&&(
          <div className="mt-4 pt-3" style={{borderTop:"1px solid rgba(36,62,65,0.08)"}}>
            <p className="text-[11px] uppercase tracking-[0.08em] mb-2" style={{color:"rgba(36,62,65,0.55)"}}>Shipment updates</p>
            <ul className="space-y-2">
              {[...order.courierEvents].reverse().slice(0,12).map((ev,i)=>(
                <li key={i} className="text-[12.5px] flex gap-2" style={{color:T.teal}}>
                  <span style={{color:"rgba(36,62,65,0.45)",minWidth:118}}>{ev.at?new Date(ev.at).toLocaleString("en-IN",{day:"numeric",month:"short",hour:"2-digit",minute:"2-digit"}):""}</span>
                  <span>{ev.status}</span>
                </li>
              ))}
            </ul>
      ...
      </div>:<p className="text-[12.5px] mb-6" style={{color:"rgba(36,62,65,0.75)"}}>A courier tracking number and link will appear here once your order is dispatched.</p>}
