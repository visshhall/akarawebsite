...
function CartDrawer({ open, onClose, cart, setCart, navigate }) {
  ...
  return <>
    ...
    <aside aria-label="Shopping cart" aria-modal={open} role="dialog" className="fixed top-0 right-0 bottom-0 z-[70] flex flex-col transition-transform duration-500"
      ...
      {cart.length>0&&      <div className="p-5 sm:p-6 shrink-0" style={{ borderTop:"1px solid rgba(36,62,65,0.1)", backgroundColor:T.cream, boxShadow:"0 -12px 32px -16px rgba(36,62,65,0.28)", position:"sticky", bottom:0 }}>
...
function ProductCard({ product, navigate, cart, setCart, wishlist, toggleWishlist }) {
  ...
  return <div className="group relative flex flex-col cursor-pointer transition-transform duration-500 ease-out akara-press"
    ...
    <div className="relative aspect-[4/5] flex items-center justify-center overflow-hidden transition-shadow duration-500"
      ...
      {firstImage
        ?<img src={firstImage} alt={name} loading="lazy" decoding="async" className="w-full h-full object-contain transition-all duration-700 ease-out" sizes="(max-width:640px) 50vw, (max-width:1024px) 33vw, 25vw" style={{backgroundColor:T.cream,transform:hover&&!isSoldOut?"scale(1.03)":"scale(1)",filter:isSoldOut?"grayscale(0.2) brightness(0.96)":"none"}}/>
        ...
        return <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 xl:gap-12 items-center mb-10 md:mb-12">
            <button type="button" onClick={()=>navigate("product",hero.id)}
              ...
              {heroImg
                ? <img src={heroImg} alt={hero.name} className="w-full h-full object-contain transition-transform duration-[1.1s] ease-out group-hover:scale-[1.02]" loading="eager" style={{backgroundColor:T.cream}} fetchPriority="high" decoding="async" width={800} height={1000}/>
                  ...
                  return (
                    <button key={p.id} type="button" onClick={()=>navigate("product",p.id)}
                      ...
                      <div className="relative overflow-hidden mb-4 max-h-[220px] sm:max-h-[240px]" style={{aspectRatio:"4/5",backgroundColor:T.card,boxShadow:ELEVATION.raised,borderRadius:RADIUS.md}}>
                        ...
                        {img
                          ? <img src={img} alt={p.name} loading="lazy" className="w-full h-full object-contain transition-transform duration-700 ease-out group-hover:scale-[1.02]" style={{backgroundColor:T.cream}}/>
  ...
  return <div className="px-6 md:px-14 py-16 max-w-[1100px] mx-auto">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-14">
        ...
        {cart.map(item=>{
          ...
          <div className="flex-1 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-5">
              <div className="inline-flex items-center" style={{border:"1px solid rgba(36,62,65,0.2)",borderRadius:RADIUS.xs}}>
                <button onClick={()=>updateQty(item.id,item.size,item.color,-1)} className="w-11 h-11 flex items-center justify-center" style={{color:T.teal}} aria-label={`Decrease quantity of ${item.name}`}><Minus size={15}/></button>
                <span className="w-8 text-center text-[14px]" style={{fontFamily:"'Fraunces',serif"}}>{item.qty}</span>
                <button onClick={()=>updateQty(item.id,item.size,item.color,1)} className="w-11 h-11 flex items-center justify-center" style={{color:T.teal}} aria-label={`Increase quantity of ${item.name}`}><Plus size={15}/></button>
              </div>
      ...
      <div className="h-fit p-7" style={{backgroundColor:T.card,boxShadow:ELEVATION.raised,borderRadius:RADIUS.lg}}>
        ...
        {[["Subtotal (incl. GST)",`₹${Math.round(subtotal*(1+GST_RATE)).toLocaleString("en-IN")}`],...(discount>0?[["Discount",`−₹${Math.round(discount*(1+GST_RATE)).toLocaleString("en-IN")}`]]:[]),["Shipping",shipping===0?"Free":`₹${shipping}`],["GST on shipping & extras",shipping>0?`₹${Math.round(shipping*GST_RATE).toLocaleString("en-IN")}`:"₹0"]].map(([l,v])=><div key={l} className="flex justify-between mb-3 text-[13.5px]" style={{color:l==="Discount"?T.success:"rgba(36,62,65,0.7)"}}><span>{l}</span><span>{v}</span></div>)}
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-14">
      ...
      <div className="h-fit p-7" style={{backgroundColor:T.card,boxShadow:ELEVATION.raised,borderRadius:RADIUS.lg}}>
        ...
        <div className="pt-4 mt-3 flex flex-col gap-2.5 text-[13.5px]" style={{borderTop:"1px solid rgba(36,62,65,0.12)",color:"rgba(36,62,65,0.75)"}}>
          <div className="flex justify-between"><span>Subtotal (incl. GST)</span><span>₹{Math.round(subtotal*(1+GST_RATE)).toLocaleString("en-IN")}</span></div>
          ...
          {[["Shipping",shipping===0?"Free":`₹${shipping}`],...(codFee>0?[["COD Handling Fee",`₹${codFee}`]]:[]),["GST on ship / COD",`₹${Math.round((shipping+codFee)*GST_RATE).toLocaleString("en-IN")}`]].map(([l,v])=><div key={l} className="flex justify-between"><span>{l}</span><span>{v}</span></div>)}
          <p className="text-[11px] mt-1" style={{color:"rgba(36,62,65,0.55)"}}>Product prices include 18% GST — same as the catalogue.</p>
        </div>
...
function ShipmentTrackingMap({ order, setOrder, guestToken }) {
  ...
  const o = projectIndia(AKARA_ORIGIN.lat, AKARA_ORIGIN.lng);
  const d = projectIndia(dest.lat, dest.lng);
  // Quadratic control point for a gentle arc
  const cx = (o.x + d.x) / 2;
  const cy = Math.min(o.y, d.y) - 40;
  const t = progress;
  // Approximate point on quadratic Bezier
  const px = (1 - t) * (1 - t) * o.x + 2 * (1 - t) * t * cx + t * t * d.x;
  const py = (1 - t) * (1 - t) * o.y + 2 * (1 - t) * t * cy + t * t * d.y;
  ...
  const refreshLive = async () => {
    ...
    try {
      ...
      if (data.refreshed) showToast("Live courier status updated.", "success");
      else showToast(data.reason || "No new courier update yet.", "info");
    } catch {
      showToast("Could not reach tracking service.", "error");
    } finally {
      setRefreshing(false);
    }
  };

  // Soft auto-refresh once when map mounts if already dispatched
  useEffect(() => {
    if (!order?.orderNumber) return;
    if (!order.courierTrackingId || String(order.courierTrackingId).startsWith("SR-")) return;
    if (!["dispatched", "delivered"].includes(String(order.status || "").toLowerCase()) && !order.courierStatus) return;
    let cancelled = false;
    (async () => {
      try {
        const body = { orderNumber: order.orderNumber };
      ...
      } catch { /* silent — map still shows cached status */ }
    })();
    return () => { cancelled = true; };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [order?.orderNumber]);
  ...
  return (
    <div className="mb-8 overflow-hidden" style={{ backgroundColor: T.card, boxShadow: ELEVATION.raised, borderRadius: RADIUS.lg, border: "1px solid rgba(36,62,65,0.08)" }}>
      <div className="flex flex-wrap items-start justify-between gap-3 px-5 pt-5 pb-2">
        <div>
          <p className="text-[11px] tracking-[0.14em] uppercase mb-1" style={{ color: "rgba(36,62,65,0.55)" }}>Live route</p>
          <h3 className="italic text-[20px]" style={{ fontFamily: "'Fraunces',serif", color: T.teal }}>Shipment path</h3>
          <p className="text-[12.5px] mt-1" style={{ color: "rgba(36,62,65,0.7)" }}>
            {stageLabel}
      ...
      <div className="px-3 sm:px-5 pb-2">
        <svg viewBox="0 0 400 420" className="w-full h-auto" role="img" aria-label={`Shipment from Mumbai to ${dest.label}`}>
          <defs>
            <linearGradient id="akRouteGrad" x1="0%" y1="0%" x2="100%" y2="0%">
              <stop offset="0%" stopColor="#183630" />
              <stop offset="100%" stopColor="#C4A35A" />
            </linearGradient>
            <filter id="akSoft" x="-20%" y="-20%" width="140%" height="140%">
              <feDropShadow dx="0" dy="2" stdDeviation="3" floodColor="#183630" floodOpacity="0.25" />
            </filter>
          </defs>
          {/* Soft India plate */}
          <rect x="8" y="8" width="384" height="404" rx="12" fill="#F7F3EB" stroke="rgba(36,62,65,0.1)" />
          <path d={pathD} fill="none" stroke="rgba(36,62,65,0.15)" strokeWidth="6" strokeLinecap="round" />
          <path d={pathD} fill="none" stroke="url(#akRouteGrad)" strokeWidth="2.5" strokeLinecap="round" strokeDasharray="6 6">
            <animate attributeName="stroke-dashoffset" from="24" to="0" dur="1.2s" repeatCount="indefinite" />
          </path>
          {/* Origin */}
          <circle cx={o.x} cy={o.y} r="7" fill="#183630" filter="url(#akSoft)" />
          <circle cx={o.x} cy={o.y} r="12" fill="none" stroke="#183630" strokeOpacity="0.25" strokeWidth="1.5" />
          <text x={o.x} y={o.y - 16} textAnchor="middle" fill="#183630" fontSize="10" fontFamily="Space Grotesk,sans-serif">Studio</text>
          {/* Destination */}
          <circle cx={d.x} cy={d.y} r="7" fill="#C4A35A" filter="url(#akSoft)" />
          <text x={d.x} y={d.y + 22} textAnchor="middle" fill="#183630" fontSize="10" fontFamily="Space Grotesk,sans-serif">You</text>
          {/* Parcel marker */}
          <g transform={`translate(${px},${py})`}>
            <circle r="11" fill="#2A5A55" filter="url(#akSoft)" />
            <circle r="4" fill="#F7F3EB" />
          </g>
        </svg>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 px-5 pb-4 text-[12.5px]">
        <div className="p-3" style={{ background: "rgba(24,54,48,0.04)", borderRadius: RADIUS.sm }}>
          <p className="text-[10px] tracking-[0.1em] uppercase mb-1" style={{ color: "rgba(36,62,65,0.5)" }}>From</p>
          <p style={{ color: T.teal }}>{AKARA_ORIGIN.label}</p>
        </div>
        <div className="p-3" style={{ background: "rgba(24,54,48,0.04)", borderRadius: RADIUS.sm }}>
          <p className="text-[10px] tracking-[0.1em] uppercase mb-1" style={{ color: "rgba(36,62,65,0.5)" }}>To</p>
          <p style={{ color: T.teal }}>{dest.label}</p>
        </div>
      </div>
      ...
      {!order?.courierTrackingId && (
        <p className="px-5 pb-5 text-[12px]" style={{ color: "rgba(36,62,65,0.6)" }}>
          Map follows atelier progress until dispatch. After Shiprocket assigns an AWB, refresh for live courier scans.
        </p>
      )}
    </div>
  );
...
function loadStoredCart(products){
  ...
  for(const entry of stored){
    ...
    // Only keep size if product actually has size variants
    const hasRealSizes=Array.isArray(product.variants)&&product.variants.some(v=>v&&v.size!=null);
    const size=hasRealSizes&&VALID_SIZES.includes(entry.size)?entry.size:null;
...
function AkaraAppRoot() {
  ...
  useEffect(()=>{
    ...
    setCart(c=>syncCartWithCatalog(c, products));
  ...
  // Multi-tab: when another tab updates cart/wishlist in localStorage (or via BC), refresh this tab.
  useEffect(()=>{
    if(!hasHydrated||!products.length) return;
    const applyCart= (items)=>{
      if(!Array.isArray(items)) return;
      const next=loadStoredCart(products); // re-read from storage after other tab wrote
      // Prefer explicit payload if shapes match loadStoredCart needs — reload from storage is safer
      setCart(loadStoredCart(products));
    };
    const onStorage=(e)=>{
      if(e.key==="akara_cart") setCart(loadStoredCart(products));
    ...
    try{
      if(typeof BroadcastChannel!=="undefined"){
        ...
        bc.onmessage=(ev)=>{
          ...
          if(d.tab&&window.__akaraTabId&&d.tab===window.__akaraTabId) return; // own tab
  ...
  useEffect(()=>{
    ...
    apiFetch("/api/cart/merge",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({items:Array.isArray(localItems)?localItems:[]})})
      ...
      .catch(()=>{ if(!cancelled) setCartServerReady(true); });
    return ()=>{ cancelled=true; };
  },[user,hasHydrated]);

  useEffect(()=>{
    if(!user){ cartMergedForUser.current=null; setCartServerReady(false); }
  },[user]);
