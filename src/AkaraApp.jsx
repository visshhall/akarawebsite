  ...
  return <header className={"sticky top-0 z-40 grid items-center h-[76px] px-6 md:px-12 relative transition-[background-color,box-shadow,border-color] duration-300 "+className} style={{ gridTemplateColumns:"auto 1fr auto", gap:"16px", backgroundColor:scrolled?"rgba(255,242,223,0.98)":"rgba(255,242,223,0.88)", backdropFilter:scrolled?"blur(24px)":"blur(16px)", borderBottom:scrolled?"1px solid rgba(36,62,65,0.12)":"1px solid rgba(36,62,65,0.06)", boxShadow:scrolled?"0 8px 28px -18px rgba(36,62,65,0.22)":"none" }}>
    ...
    <div className="justify-self-end flex items-center gap-0.5">
      <div ref={searchWrapRef} className="relative">
        ...
        {searchOpen && <div className="absolute right-0 top-full mt-2 z-50" style={{ width:"min(360px,calc(100vw - 3rem))" }}>
...
function ShopView({ navigate, cart, setCart, wishlist, toggleWishlist, initCategory }) {
  ...
  return <div>
    <section className="px-6 md:px-14 pt-14 pb-8 text-center">
      ...
      {/* Refined filters — quiet, URL-synced */}
      <div className="max-w-[920px] mx-auto mt-10 pt-9 px-1 sm:px-2 text-left" style={{borderTop:"1px solid rgba(36,62,65,0.08)"}}>
        <div className="flex flex-wrap items-end justify-between gap-3 mb-6">
          <div>
            <p className="text-[10px] tracking-[0.18em] uppercase mb-1" style={{color:"rgba(36,62,65,0.45)"}}>Refine the collection</p>
            <p className="text-[12px]" style={{color:"rgba(36,62,65,0.55)"}}>Quiet filters — shareable in the link.</p>
          </div>
          {hasActiveFilters&&(
            <button type="button" onClick={clearFilters} className="text-[11px] tracking-[0.08em] uppercase" style={{color:T.teal,borderBottom:"1px solid rgba(24,54,48,0.25)"}}>Clear all</button>
          )}
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          <div>
            <p className="text-[10px] tracking-[0.12em] uppercase mb-2" style={{color:"rgba(36,62,65,0.45)"}}>Scale</p>
            <div className="flex flex-wrap gap-1.5">
              {chip(!filterSize, ()=>setFilterSize(""), "Any")}
              {sizeOptions.map(s=>chip(filterSize===s, ()=>setFilterSize(filterSize===s?"":s), s))}
            </div>
          </div>
          <div>
            <p className="text-[10px] tracking-[0.12em] uppercase mb-2" style={{color:"rgba(36,62,65,0.45)"}}>Finish colour</p>
            <select value={filterColour} onChange={e=>setFilterColour(e.target.value)}
              className="w-full text-[12.5px] px-3 py-2 outline-none"
              style={{border:"1px solid rgba(36,62,65,0.18)",borderRadius:RADIUS.xs,color:T.teal,background:T.cream}}>
          ...
          <div>
            <p className="text-[10px] tracking-[0.12em] uppercase mb-2" style={{color:"rgba(36,62,65,0.45)"}}>Availability</p>
            <div className="flex flex-wrap gap-1.5">
              {chip(!filterAvail, ()=>setFilterAvail(""), "Any")}
              {chip(filterAvail==="in", ()=>setFilterAvail(filterAvail==="in"?"":"in"), "In studio")}
              {chip(filterAvail==="low", ()=>setFilterAvail(filterAvail==="low"?"":"low"), "Few left")}
            </div>
          </div>
          <div>
            <p className="text-[10px] tracking-[0.12em] uppercase mb-2" style={{color:"rgba(36,62,65,0.45)"}}>Price (₹, incl. GST)</p>
            <div className="flex items-center gap-2">
              <input inputMode="numeric" placeholder={`₹${priceFloor}`} value={priceMin} maxLength={7}
                onChange={e=>{
                ...
                className="w-full text-[12.5px] px-2.5 py-2 outline-none"
                style={{border:"1px solid rgba(36,62,65,0.18)",borderRadius:RADIUS.xs,color:T.teal,background:T.cream}}
                aria-label={`Minimum price, from ${priceFloor}`}/>
              <span style={{color:"rgba(36,62,65,0.35)"}}>—</span>
              <input inputMode="numeric" placeholder={`₹${priceCeil}`} value={priceMax} maxLength={7}
                ...
                onBlur={()=>{
                  ...
                  n=Math.max(priceFloor, Math.min(priceCeil, n));
                  // Round displayed max up to nearest ₹1,000 when user leaves field empty of exact intent — only clamp
                  setPriceMax(String(n));
                }}
                className="w-full text-[12.5px] px-2.5 py-2 outline-none"
                style={{border:"1px solid rgba(36,62,65,0.18)",borderRadius:RADIUS.xs,color:T.teal,background:T.cream}}
                aria-label={`Maximum price, up to ${priceCeil}`}/>
            </div>
            <p className="text-[10.5px] mt-1.5" style={{color:"rgba(36,62,65,0.45)"}}>Range of this collection: ₹{priceFloor.toLocaleString("en-IN")} – ₹{priceCeil.toLocaleString("en-IN")}</p>
          </div>
        </div>
      </div>

      <div className="pt-5 pb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 max-w-[900px] mx-auto">
        <p className="text-[11.5px] tracking-[0.06em] uppercase" style={{color:"rgba(36,62,65,0.75)"}}>{productsLoading?"Loading…":`${filtered.length} ${filtered.length===1?"piece":"pieces"}`}</p>
        {!productsLoading&&(
