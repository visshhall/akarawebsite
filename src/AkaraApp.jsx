    ...
    <div className="flex flex-col md:flex-row gap-12 md:gap-20">
      {sections.length>0&&<nav aria-label="Sections on this page" className="md:w-[220px] shrink-0 md:sticky md:top-28 md:self-start order-2 md:order-1">
        ...
        <div className="flex flex-col gap-3 md:max-h-[65vh] md:overflow-y-auto md:pr-2 border-l" style={{borderColor:"rgba(184,147,90,0.35)"}}>
          {sections.map(s=><a key={s.id} href={`#${s.id}`} className="text-[12.5px] leading-snug pl-3 transition-colors hover:text-[#183630]" style={{color:"rgba(36,62,65,0.6)"}}>{s.text}</a>)}
        </div>
      </nav>}
...
function AkaraAppRoot() {
  ...
  useEffect(()=>{
    fetch("/api/products")
      ...
      .then(data=>{ const list=Array.isArray(data?.products)?data.products:[]; const enriched=list.map(enrichProduct).filter(Boolean); setProducts(enriched); setProductsLoading(false); setCart(c=>syncCartWithCatalog(c, enriched)); })
