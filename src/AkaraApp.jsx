...
import { T, FONTS, sanitize, apiFetch, getCsrfToken, Mac, SweepButton, InputField, ToastProvider, useToast, Skeleton, Modal, ELEVATION, RADIUS, ICON, trackAkara, TurnstileWidget } from "./shared.jsx";
...
function ProductCard({ product, navigate, cart, setCart, wishlist, toggleWishlist }) {
  ...
  return <div className="group relative flex flex-col cursor-pointer transition-transform duration-500 ease-out akara-press"
    ...
    <div className="relative aspect-[4/5] flex items-center justify-center overflow-hidden transition-shadow duration-500"
      ...
      {firstImage
        ?<img src={firstImage} alt={name} loading="lazy" decoding="async" className="w-full h-full object-cover transition-all duration-700 ease-out" style={{transform:hover&&!isSoldOut?"scale(1.045)":"scale(1)",filter:isSoldOut?"grayscale(0.2) brightness(0.96)":"none"}}/>
        ...
        return <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14 xl:gap-16 items-center mb-14 md:mb-20">
            <button type="button" onClick={()=>navigate("product",hero.id)}
              ...
              {heroImg
                ? <img src={heroImg} alt={hero.name} className="w-full h-full object-cover transition-transform duration-[1.1s] ease-out group-hover:scale-[1.025]" loading="eager" fetchPriority="high" decoding="async" width={800} height={1000}/>
    ...
    {trimmed.length<=1
      ...
      ?<div className="px-6 py-16 text-center max-w-[480px] mx-auto">
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          <SweepButton filled onClick={()=>navigate("shop")}>{homeSections.cta_explore||"Explore the Collection"}</SweepButton>
  ...
  return <div>
    <div className="relative mx-auto flex items-center justify-center overflow-hidden"
      ...
      {item.type==="video"
        ...
        : (item.src
            ? <img src={item.src} alt={`${name} — photo ${active+1}`} className="w-full h-full object-contain cursor-zoom-in" style={{opacity:soldOut?0.5:1,backgroundColor:T.card}} onClick={()=>!soldOut&&setLightboxOpen(true)}/>
    ...
    <section className="px-6 md:px-14 pt-8 pb-16 max-w-[1800px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-14">
      ...
      <div>
        ...
        <div className="flex flex-col gap-2 text-[12.5px]" style={{color:"rgba(36,62,65,0.75)"}}>
          ...
          <div className="flex items-center gap-2"><MapPin size={13}/> Studio-made in Mumbai — not white-labelled</div>
        </div>
      </div>
    </section>
...
function MyAccountView({ navigate, wishlist, user, setUser, order, setOrder, initTab }) {
  ...
  const requestPhoneOTP=async e=>{
    ...
    try{
      ...
      const data=await res.json();
      if(res.ok){ setPhoneChangeStep("verifying"); showToast(data.channel==="email"?"Code sent to your email":"Code sent on WhatsApp to the new number","success"); }
  ...
  const saveAddress=async()=>{
    ...
    if(Object.keys(errs).length) return;
    try{
      const body={...addrForm,phone:normalizePhone(addrForm.phone)};
      ...
      if(res.ok){
        ...
        setAddrForm({name:"",line:"",landmark:"",city:"",state:"",pin:"",phone:""});
  ...
  const startEditAddress=a=>{
    ...
    setAddrForm({name:a.name||"",line:a.line||"",landmark:a.landmark||"",city:a.city||"",state:a.state||"",pin:String(a.pin||""),phone:String(a.phone||"")});
    setAddrErrors({});
    ...
    {tab==="Addresses"&&<div className="w-full max-w-[900px]">
      {showAddrForm?<div id="akara-addr-form" className="p-6 mb-6" style={{backgroundColor:T.card,boxShadow:ELEVATION.raised,borderRadius:RADIUS.md,border:"1px solid rgba(24,54,48,0.08)"}}>
        ...
        <div className="flex gap-3">
          ...
          <SweepButton onClick={()=>{setShowAddrForm(false);setEditingAddrId(null);setAddrForm({name:"",line:"",landmark:"",city:"",state:"",pin:"",phone:""});setAddrErrors({});}}>Cancel</SweepButton>
        </div>
      </div>:<SweepButton onClick={()=>{setShowAddrForm(true);setEditingAddrId(null);setAddrForm({name:"",line:"",landmark:"",city:"",state:"",pin:"",phone:""});}}>+ Add New Address</SweepButton>}
...
function LegalShell({title,updated,icon:Icon,children}){
  ...
  return <div className="px-6 md:px-14 py-16 md:py-20 max-w-[1140px] mx-auto">
    <div className="mb-12 md:mb-16 max-w-[40rem]">
      <p className="text-[10px] tracking-[0.28em] uppercase mb-4" style={{color:T.gold}}>Ākāra atelier</p>
      ...
      <p className="text-[12px] tracking-[0.06em]" style={{color:"rgba(36,62,65,0.55)"}}>Last updated · {updated}</p>
    </div>
    <div className="flex flex-col md:flex-row gap-12 md:gap-20">
      {sections.length>0&&<nav aria-label="Sections on this page" className="md:w-[220px] shrink-0 md:sticky md:top-28 md:self-start order-2 md:order-1">
        <p className="text-[10px] tracking-[0.16em] uppercase mb-4" style={{color:T.gold}}>On this page</p>
...
function AkaraAppRoot() {
  ...
  useEffect(()=>{ if(hasHydrated) safeStorageSet("akara_cart",cart.map(i=>({id:i.id,size:i.size||null,color:i.color||null,qty:i.qty}))); },[cart,hasHydrated]);
...
class ErrorBoundary extends Component {
  ...
  componentDidCatch(error,info){ console.error("ĀKĀRA app error:",error,info); }
...
class SoftBoundary extends Component {
  constructor(props){ super(props); this.state={hasError:false, message:""}; }
  static getDerivedStateFromError(error){ return {hasError:true, message:(error&&error.message)||""}; }
  componentDidCatch(error,info){ console.error("ĀKĀRA route error:",error,info); }
  componentDidUpdate(prevProps){
    if(this.state.hasError && this.props.resetKey!==prevProps.resetKey){
      this.setState({hasError:false, message:""});
    }
