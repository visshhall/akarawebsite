...
function NewsletterForm() {
  ...
  return <form onSubmit={e=>{e.preventDefault();if(validEmail(email))setDone(true);}} className="flex" style={{border:"1px solid rgba(255,255,255,0.15)"}}>
    ...
    <button type="submit" className="px-4 py-3 text-[11px] tracking-[0.1em] uppercase shrink-0" style={{backgroundColor:"rgba(255,255,255,0.1)",color:T.cream}}>Join</button>
  </form>;
}
        ...
        <div className="sm:col-span-2 md:col-span-1">
          ...
          <div className="flex flex-col gap-2.5">
            {[[Mail,"support@akaraonline.co.in","mailto:support@akaraonline.co.in"],
              ...
              <Icon size={13} style={{color:T.cream,flexShrink:0}}/>
...
function ProductCard({ product, navigate, cart, setCart, wishlist, toggleWishlist }) {
  ...
  // Drives the brief "Added" confirmation on the persistent cart-icon
  // button below — without this, clicking Add to Cart gave literally no
  // feedback at all (no toast, no state change), so it wasn't obvious
  // anything happened, especially for a second/third click on the same card.
  ...
  return <div className="group relative flex flex-col cursor-pointer transition-transform duration-500 ease-out"
    ...
    <div className="relative aspect-square flex items-center justify-center overflow-hidden transition-shadow duration-500"
      ...
      {/* Persistent, always-tappable cart button — deliberately NOT
          hover-gated (unlike the full-width bar below), since hover
          doesn't meaningfully exist on a touch screen. This is what
          actually makes "add to cart" reachable on mobile without
          opening the product page first. Briefly swaps to a checkmark
          + gold background on click as the missing confirmation feedback. */}
      <button onClick={addToCart} aria-label="Add to cart"
        className="absolute top-3.5 right-14 w-8 h-8 flex items-center justify-center border transition-colors duration-300"
        style={{borderColor:justAdded?T.gold:"rgba(36,62,65,0.1)",backgroundColor:justAdded?T.gold:"rgba(255,255,255,0.75)"}}>
        {justAdded?<Check size={14} style={{color:"white"}}/>:<ShoppingBag size={14} style={{color:T.teal}}/>}
      </button>
      <button onClick={addToCart}
        ...
        <span className="text-[11px] tracking-[0.14em] uppercase font-medium text-white">{justAdded?"Added ✓":"Add to Cart"}</span>
      </button>
    </div>
  ...
  const submit=async e=>{
    ...
    try{
      ...
      if(typeof window.Razorpay!=="function"){
        setErrors({form:"Payment couldn't load. Please check your connection and try again."}); setStage("form"); return;
      }
  ...
  return <div className="px-6 md:px-14 py-16 max-w-[1100px] mx-auto">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-14">
      <form onSubmit={submit} noValidate>
        ...
        <button type="submit" disabled={stage==="processing"} className="w-full py-4 text-[12px] tracking-[0.14em] uppercase font-medium text-white disabled:opacity-60" style={{backgroundColor:T.teal}}>
...
function OrderStatusView({ navigate, order, setOrder }) {
  ...
  const confirmCancel=async()=>{
    setCancelling(true);
    try{
      const res=await apiFetch(`/api/orders/${order.orderNumber}/cancel`,{method:"PATCH"});
      const data=await res.json();
      if(res.ok){ setOrder(data.order); showToast("Order cancelled","success"); }
      else showToast(data.error||"Couldn't cancel this order.","error");
    }catch{
  ...
  return <div className="px-6 md:px-14 py-16 max-w-[820px] mx-auto">
    ...
    <Modal open={confirmCancelOpen} onClose={()=>setConfirmCancelOpen(false)} title="Cancel this order?" danger confirmLabel={cancelling?"Cancelling…":"Cancel Order"} onConfirm={confirmCancel}>
      This can't be undone. If you've already paid, your refund will be processed separately — we'll be in touch.
    </Modal>
  </div>;
}
...
function ContactView() {
  const [form,setForm]=useState({name:"",email:"",message:""}); const [sent,setSent]=useState(false); const [errors,setErrors]=useState({});
  const submit=e=>{
    e.preventDefault();
    const errs={};
    if(!sanitize(form.name).trim()) errs.name="Required";
    if(!validEmail(form.email)) errs.email="Valid email required";
    if(!sanitize(form.message).trim()) errs.message="Required";
    setErrors(errs);
...
function MyAccountView({ navigate, wishlist, user, setUser, order, setOrder, initTab }) {
  ...
  return <div className="px-6 md:px-14 py-16 max-w-[1000px] mx-auto">
    ...
    {tab==="Orders"&&(!user
      ?<div className="text-center py-16"><p className="text-[14px] mb-6" style={{color:"rgba(36,62,65,0.5)"}}>Sign in to see your order history.</p><SweepButton filled onClick={()=>navigate("login")}>Sign In</SweepButton></div>
      :ordersLoading
...
function ReturnRequestView({ navigate, order, user }){
  ...
  const submit=async e=>{
    ...
    if(!sanitize(description).trim()) errs.description="Please describe what happened";
    setErrors(errs);
    if(Object.keys(errs).length) return;
    setSubmitting(true);
    try{
