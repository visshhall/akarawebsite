...
function SignupView({ navigate, onLogin, postLoginRedirect, setPostLoginRedirect }) {
  ...
  const requestOTP=async e=>{
    ...
    try{
      ...
      if(!res.ok){
        ...
        setOtpChannel(data.channel==="email"?"email":data.channel==="both"?"both":"whatsapp");
        setStep("verify");
      }
    }catch{
      setErrors({form:"Couldn't reach the server. Please check your connection and try again."});
  ...
  if(step==="verify") return <AuthLayout eyebrow="Almost there" title={otpChannel==="email"?"Verify your email":"Verify your number"} subtitle={otpChannel==="email"?`Enter the 6-digit code we sent to ${form.email}.`:otpChannel==="both"?`Enter the 6-digit code sent on WhatsApp to +91 ${normalizePhone(form.phone)} and to ${form.email}.`:`Enter the 6-digit code we sent on WhatsApp to +91 ${normalizePhone(form.phone)}.`}>
    ...
    <p className="text-[13px] text-center mt-6" style={{color:"rgba(36,62,65,0.75)"}}>{otpChannel==="email"?"Wrong email?":otpChannel==="both"?"Wrong email or number?":"Wrong phone number?"} <button type="button" onClick={()=>{setStep("form");setOtpCode("");setErrors({});}} className="underline" style={{color:T.teal}}>Go back</button></p>
  </AuthLayout>;
