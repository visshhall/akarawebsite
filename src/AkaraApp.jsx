  ...
  return <div>
    ...
    {!productsLoading&&featured.length>0&&<section className="px-5 sm:px-8 md:px-12 lg:px-16 xl:px-20 2xl:px-24 py-20 md:py-28 max-w-[2000px] mx-auto w-full">
      <div className="mb-12 md:mb-16 max-w-xl">
        <p className="text-[11px] tracking-[0.28em] uppercase mb-3" style={{color:"rgba(36,62,65,0.55)"}}>This season</p>
        ...
        return <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14 xl:gap-16 items-center mb-14 md:mb-20">
            <button type="button" onClick={()=>navigate("product",hero.id)}
              ...
              style={{aspectRatio:"4/5",maxHeight:"680px",backgroundColor:T.card,boxShadow:ELEVATION.raised,borderRadius:RADIUS.lg}}
            ...
            <div className="lg:col-span-5 flex flex-col justify-center lg:pl-2">
              {heroCat&&<p className="text-[11px] tracking-[0.22em] uppercase mb-4" style={{color:T.gold}}>{heroCat}</p>}
              ...
              <div className="mb-8">
                <p className="text-[24px] md:text-[26px]" style={{fontFamily:"'Fraunces',serif",color:T.teal}}>₹{Number(hero.price).toLocaleString("en-IN")}</p>
                <p className="text-[11.5px] mt-1" style={{color:"rgba(36,62,65,0.5)"}}>Excl. GST · printed after you order</p>
              </div>
          ...
          {supports.length>0&&(
            <div>
              <div className="flex items-end justify-between mb-8 gap-4">
                <p className="text-[11px] tracking-[0.2em] uppercase" style={{color:"rgba(36,62,65,0.5)"}}>Also in this edit</p>
                ...
                {supports.map((p,i)=>{
                  ...
                  return (
                    <button key={p.id} type="button" onClick={()=>navigate("product",p.id)}
                      ...
                      <p className="text-[10.5px] tracking-[0.18em] uppercase mb-1.5" style={{color:"rgba(36,62,65,0.45)"}}>{String(i+2).padStart(2,"0")} · {p.cat||p.category||""}</p>
                      <p className="italic text-[18px] md:text-[20px] leading-snug mb-1" style={{fontFamily:"'Fraunces',serif",color:T.teal}}>{p.name}</p>
                      <p className="text-[14px]" style={{fontFamily:"'Fraunces',serif",color:"rgba(36,62,65,0.75)"}}>₹{Number(p.price).toLocaleString("en-IN")}</p>
                    </button>
                  );
                })}
              </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          <div key={title} className="p-7 text-center" style={{backgroundColor:T.card,boxShadow:ELEVATION.raised,borderRadius:RADIUS.md}}>
            ...
            <p className="text-[11px] tracking-[0.2em] uppercase mb-2" style={{color:T.gold}}>{num}</p>
            <h3 className="text-[15px] mb-2" style={{fontFamily:"'Fraunces',serif",fontStyle:"italic",color:T.teal}}>{title}</h3>
            <p className="text-[13px] leading-[1.6]" style={{color:"rgba(36,62,65,0.75)"}}>{desc}</p>
          </div>
        ))}
  ...
  return (
    <div className="mt-5">
      <div className="flex items-center gap-3 mb-4">
        ...
        <span className="text-[11px] tracking-[0.12em] uppercase" style={{ color: "rgba(36,62,65,0.5)" }}>or</span>
        <div className="flex-1 h-px" style={{ backgroundColor: "rgba(36,62,65,0.12)" }} />
      </div>
      <div ref={btnRef} className="w-full flex justify-center min-h-[44px]" style={{ opacity: busy ? 0.5 : 1, pointerEvents: busy ? "none" : "auto" }} />
      {!ready && <p className="text-[12px] text-center mt-2" style={{ color: "rgba(36,62,65,0.55)" }}>Loading Google…</p>}
    </div>
  );
}
  ...
  return <div className="px-6 md:px-14 py-16 max-w-[1000px] mx-auto">
    <div className="flex items-end justify-between mb-10 flex-wrap gap-4">
      ...
      <button type="button" onClick={async()=>{const r=await enableCustomerPush(); alert(r.ok?"Order status alerts enabled.":(r.error||"Could not enable"));}}
        className="mt-3 text-[11px] uppercase tracking-[0.1em] px-3 py-2" style={{border:`1px solid rgba(36,62,65,0.15)`,borderRadius:RADIUS.xs,color:T.teal}}>Enable order alerts</button>
