...
async function downloadInvoicePDF(order){
  ...
  doc.text("FORM · MADE TO ORDER",14,24);
  doc.setTextColor(255,255,255);
  doc.setFont("NotoSans","normal"); doc.setFontSize(8);
  doc.text("Precision Forge Labs · Thane 400601  ·  GSTIN 27GZCPS9353H1ZQ",14,36);
  ...
  autoTable(doc,{
    ...
    body:order.items.map(i=>[`${i.name}${[i.colorLabel,i.size].filter(Boolean).length?" ("+[i.colorLabel,i.size].filter(Boolean).join(", ")+")":""}${i.batchNumber?" · "+i.batchNumber:""}`,i.hsn,String(i.qty),`Rs ${i.price.toLocaleString("en-IN")}`,`Rs ${(i.price*i.qty).toLocaleString("en-IN")}`]),
  ...
  doc.setTextColor(...WARMGRAY);
  doc.text("Studio piece · Printed to order in Mumbai · Not white-labelled stock",14,278);
  doc.text("Computer-generated tax invoice · akaraonline.co.in",14,284);
  doc.setTextColor(...TEAL);
  doc.setFont("NotoSans","bold");
  doc.text("ĀKĀRA",196,284,{align:"right"});

  
  // Collector detail — form identity, not just a SKU line
  try {
    const fy = (doc.lastAutoTable && doc.lastAutoTable.finalY) ? doc.lastAutoTable.finalY + 10 : 260;
    doc.setFont("NotoSans", "bold");
    doc.setFontSize(9);
    doc.setTextColor(24, 54, 48);
    doc.text("Collector detail", 14, fy);
    doc.setFont("NotoSans", "normal");
    doc.setFontSize(8);
    doc.setTextColor(90);
    const detailLines = (order.items || []).slice(0, 6).map((it) => {
      const v = [it.colorLabel, it.size].filter(Boolean).join(" · ");
      return `${it.name || "Piece"}${v ? ` · ${v}` : ""} · studio batch · Mumbai`;
    });
    let dy = fy + 5;
    for (const line of detailLines) {
      doc.text(String(line).slice(0, 95), 14, dy);
      dy += 4.5;
    }
  } catch (_) { /* non-fatal */ }
