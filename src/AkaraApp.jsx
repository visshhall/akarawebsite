...
function ContactView() {
  ...
  return <div>
    ...
    <section className="px-6 max-w-[1000px] mx-auto pb-14 grid grid-cols-1 md:grid-cols-3 overflow-hidden" style={{boxShadow:ELEVATION.raised,borderRadius:RADIUS.md}}>
      {[
      ...
      ].map(([label,title,desc,linkLabel,href])=><div key={label} className="p-8 md:p-9" style={{backgroundColor:T.teal,borderRight:"1px solid rgba(255,255,255,0.08)"}}>
        <p className="text-[11px] tracking-[0.1em] uppercase mb-4" style={{color:T.cream}}>{label}</p>
        <h2 className="italic text-[20px] md:text-[21px] mb-4 text-white" style={{fontFamily:"'Fraunces',serif"}}>{title}</h2>
        <p className="text-[13.5px] leading-[1.7] mb-5" style={{color:"rgba(255,255,255,0.6)"}}>{desc}</p>
        <a href={href} target={href.startsWith("http")?"_blank":undefined} rel={href.startsWith("http")?"noopener noreferrer":undefined} className="text-[12.5px] hover:opacity-70 transition-opacity underline-offset-2" style={{color:T.gold}}>{linkLabel}</a>
      </div>)}
    </section>
...
function ScrollToTopFab(){
  const [visible,setVisible]=useState(false);
  useEffect(()=>{
    const onScroll=()=>setVisible((window.scrollY||document.documentElement.scrollTop||0)>420);
    onScroll();
  ...
  return (
    <button
      ...
      className="fixed z-[90] flex items-center justify-center transition-opacity hover:opacity-95"
      style={{
        right:16, bottom:20, width:48, height:48, borderRadius:999,
        backgroundColor:T.teal, color:T.cream, border:"1px solid rgba(229,198,144,0.45)",
        boxShadow:"0 8px 28px rgba(24,54,48,0.28)",
      }}
    >
