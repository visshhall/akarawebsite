...
function SectionRule({ label }) {
  return <div className="flex items-center justify-center gap-4 px-6 py-2" aria-hidden>
...
function SearchResultsView({ navigate, cart, setCart, wishlist, toggleWishlist, initQuery }){
  ...
  useEffect(()=>{ setQ(initQuery||""); setCatFilter("all"); setAvailFilter("all"); },[initQuery]);
...
function ProductDetailView({ productId, navigate, cart, setCart, wishlist, toggleWishlist }) {
  ...
  // Defaults for colour / size — in effects, never during render (setState
  // while rendering crashed SoftBoundary on some products after admin edits).
  useEffect(()=>{
    if(hasColors&&color===null&&product.colors?.[0]?.variantKey){
      setColor(product.colors[0].variantKey);
    }
  },[hasColors,color,product.colors]);
  useEffect(()=>{
    if(!hasRealSizes||autoSizeAppliedRef.current) return;
    autoSizeAppliedRef.current=true;
    const sid=hasColors?product.colors.find(c=>c.variantKey===color)?.id:null;
    const currentSizeVariant=product.variants.find(v=>(sid==null||v.colorId===sid)&&(v.size||"Medium")===size);
    if(!currentSizeVariant||currentSizeVariant.status==="sold-out"){
      const firstAvailable=["Small","Medium","Large"].find(s=>{
        const v=product.variants.find(v=>(sid==null||v.colorId===sid)&&(v.size||"Medium")===s);
        return v&&v.status!=="sold-out";
      });
      if(firstAvailable&&firstAvailable!==size) setSize(firstAvailable);
    }
  },[hasRealSizes,hasColors,color,size,product.variants,product.colors]);
      ...
      <div>
        ...
        {hasColors&&<div className="mb-5">
          ...
          <div className="flex gap-2.5">
            {(Array.isArray(product.colors)?product.colors:[]).map(c=>{ const optionSoldOut=isColorSoldOut(c.variantKey); return <button key={c.variantKey} onClick={()=>setColor(c.variantKey)} disabled={optionSoldOut} title={optionSoldOut?`${c.label} — Sold Out`:c.label}
...
function LegalShell({title,updated,icon:Icon,children}){
  ...
  return <div className="px-6 md:px-14 py-16 md:py-20 max-w-[1140px] mx-auto">
...
function Lp({c}){return <p className="text-[15px] leading-[1.9] mb-5 first:text-[16.5px] first:leading-[1.85]" style={{color:"rgba(36,62,65,0.78)",textAlign:"justify",textJustify:"inter-word",maxWidth:"38rem"}}>{c}</p>;}
...
function CmsSectionBlocks({blocks, navigate}){
  return <>{blocks.map(b=>{
    if(b.blockType==="hero"){
      ...
      return <section key={b.id} className="relative px-6 py-28 md:py-36 text-center max-w-[880px] mx-auto">
        <div className="pointer-events-none absolute left-1/2 top-16 -translate-x-1/2 w-40 h-px" style={{background:`linear-gradient(90deg,transparent,${T.gold},transparent)`}} aria-hidden/>
        <p className="text-[11px] tracking-[0.32em] uppercase mb-8" style={{color:T.gold}}>{eyebrow}</p>
        <h1 className="italic text-[34px] md:text-[52px] leading-[1.15] mb-8" style={{fontFamily:"'Fraunces',serif",fontWeight:400,color:T.teal}}>
          {heading.split(/(\*\*[^*]+\*\*)/).map((part,i)=>part.startsWith("**")?<span key={i} style={{color:T.gold}}>{part.slice(2,-2)}</span>:part)}
        </h1>
        <p className="text-[16.5px] leading-[1.85] max-w-[520px] mx-auto" style={{color:"rgba(36,62,65,0.72)"}}>{subtext}</p>
      </section>;
    }
