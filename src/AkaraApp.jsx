...
function Header({ navigate, onOpenDrawer, onOpenSearch, onOpenCart, cartCount, wishCount, user, className="" }) {
  ...
  return <header className={"sticky top-0 z-40 grid items-center h-[68px] px-6 md:px-12 "+className} style={{ gridTemplateColumns:"auto 1fr auto", gap:"16px", backgroundColor:scrolled?"rgba(255,242,223,0.97)":"rgba(255,242,223,0.88)", backdropFilter:"blur(20px)", borderBottom:"1px solid rgba(36,62,65,0.09)" }}>
...
function SearchPanel({ open, onClose, navigate }) {
  ...
  // Redesigned — found during a live-site sweep: this used to be a full
  // page-width rectangular strip sliding down from under the header, with
  // hard corners and a heavy border. Replaced with a compact, rounded
  // popover anchored near the header's search icon (same responsive-width
  // pattern already used by CartDrawer elsewhere in this file), so it
  // reads as belonging to the header rather than a separate slab of page.
  return <>
    {open&&<div onClick={onClose} className="fixed inset-0 z-[35] transition-opacity duration-300" style={{ backgroundColor:"rgba(36,62,65,0.15)" }}/>}
    <div role="search" aria-label="Search products" aria-hidden={!open} className="fixed top-[78px] right-4 md:right-10 z-[60] overflow-hidden transition-all duration-300 origin-top-right"
      style={{ width:"min(92vw,380px)", maxHeight:open?"70vh":"0px", opacity:open?1:0, transform:open?"scale(1)":"scale(0.96)", backgroundColor:T.cream, borderRadius:RADIUS.md, boxShadow:open?"0 24px 60px -16px rgba(36,62,65,0.35)":"none", pointerEvents:open?"auto":"none" }}>
      <form onSubmit={e=>{e.preventDefault();goToResults();}} className="px-5 py-4 flex items-center gap-3" style={{borderBottom:"1px solid rgba(36,62,65,0.08)"}}>
      ...
      {matches.length>0 && <div className="px-5 pb-3 overflow-y-auto" style={{maxHeight:"50vh"}}>
      ...
      {trimmed.length>1&&allMatches.length===0&&<div className="px-5 pb-4">
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center justify-between gap-3 mb-2">
              <div className="inline-flex items-center" style={{border:"1px solid rgba(36,62,65,0.2)"}}>
                <button onClick={()=>updateQty(item.id,item.size,-1)} className="w-7 h-7 flex items-center justify-center" style={{color:T.teal}} aria-label="Decrease quantity"><Minus size={11}/></button>
                <span className="w-6 text-center text-[12.5px]" style={{fontFamily:"'Fraunces',serif"}}>{item.qty}</span>
                <button onClick={()=>updateQty(item.id,item.size,1)} className="w-7 h-7 flex items-center justify-center" style={{color:T.teal}} aria-label="Increase quantity"><Plus size={11}/></button>
              </div>
              <p className="text-[13px]" style={{ color:T.gold }}>₹{item.price*item.qty}</p>
            </div>
...
function ProductCard({ product, navigate, cart, setCart, wishlist, toggleWishlist }) {
  ...
  return <div className="group relative flex flex-col cursor-pointer transition-transform duration-500 ease-out"
    ...
    <div className="relative aspect-square flex items-center justify-center overflow-hidden transition-shadow duration-500"
      ...
      {cartQty===0
        ?<button onClick={addToCart} aria-label="Add to cart" className="absolute bottom-3.5 right-3.5 w-9 h-9 flex items-center justify-center" style={{backgroundColor:T.teal,borderRadius:RADIUS.sm}}>
        ...
        :<div onClick={e=>e.stopPropagation()} className="absolute bottom-3.5 right-3.5 flex items-center" style={{backgroundColor:T.teal,borderRadius:RADIUS.sm}}>
          <button onClick={e=>updateQty(e,-1)} aria-label="Decrease quantity" className="w-8 h-9 flex items-center justify-center"><Minus size={12} style={{color:"white"}}/></button>
          <span className="text-[12.5px] text-white px-0.5 min-w-[16px] text-center">{cartQty}</span>
          <button onClick={e=>updateQty(e,1)} aria-label="Increase quantity" className="w-8 h-9 flex items-center justify-center"><Plus size={12} style={{color:"white"}}/></button>
        </div>}
    </div>
...
function HomeView({ navigate, cart, setCart, wishlist, toggleWishlist }) {
  ...
  return <div>
    ...
    <section className="px-6 md:px-14 py-14 md:py-20 max-w-[1600px] mx-auto">
      <div className="text-center mb-12">
        <p className="text-[12px] tracking-[0.3em] uppercase mb-4" style={{color:T.gold}}>Why ĀKĀRA</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        ].map(([Icon,title,desc])=><div key={title} className="p-7 text-center sm:text-left" style={{backgroundColor:T.card,boxShadow:ELEVATION.raised,borderRadius:RADIUS.md}}>
          <div className="w-11 h-11 flex items-center justify-center mb-5 mx-auto sm:mx-0" style={{backgroundColor:"rgba(184,147,90,0.12)",borderRadius:RADIUS.sm}}>
...
function ProductDetailView({ productId, navigate, cart, setCart, wishlist, toggleWishlist }) {
  ...
  // check here.
  const buyNow=()=>{ if(soldOut) return; setCart(c=>{ const ex=c.find(i=>i.id===product.id&&i.size===size); if(ex) return c.map(i=>i.id===product.id&&i.size===size?{...i,qty:i.qty+qty}:i); return [...c,{...product,size,qty}]; }); navigate("checkout"); };
  ...
  return <div>
    ...
    <section className="px-6 md:px-14 pt-8 pb-16 max-w-[1600px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-14">
      ...
      <div>
        ...
        <p className="text-[28px] mb-1" style={{fontFamily:"'Fraunces',serif",color:T.teal}}>₹{(product.price*qty).toLocaleString("en-IN")}</p>
        {qty>1&&<p className="text-[12px] mb-1" style={{color:"rgba(36,62,65,0.5)"}}>₹{product.price.toLocaleString("en-IN")} each × {qty}</p>}
        <p className="text-[11.5px] mb-6" style={{color:"rgba(36,62,65,0.45)"}}>Excl. GST · added at checkout</p>
        {cartQtyForSize>0&&<div className="flex items-center gap-2 px-4 py-2.5 mb-6 text-[12.5px]" style={{backgroundColor:"rgba(59,110,82,0.08)",color:T.success}}>
          <Check size={13}/> {cartQtyForSize} already in your cart{size?` (${size})`:""}
        </div>}
        ...
        <div className="hidden lg:flex gap-3 mb-5">
          <SweepButton onClick={addToCart} disabled={soldOut} className="flex-1">{soldOut?"Sold Out":"Add to Cart"}</SweepButton>
    ...
    <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 flex items-center gap-3 px-4 py-3" style={{backgroundColor:T.card,boxShadow:"0 -8px 24px -14px rgba(36,62,65,0.2)",borderTop:"1px solid rgba(36,62,65,0.1)"}}>
      <div className="shrink-0">
        ...
        <p className="text-[16px]" style={{fontFamily:"'Fraunces',serif",color:T.teal}}>₹{(product.price*qty).toLocaleString("en-IN")}</p>
      </div>
      <SweepButton filled onClick={addToCart} disabled={soldOut} className="flex-1">{soldOut?"Sold Out":"Add to Cart"}</SweepButton>
      <button onClick={()=>toggleWishlist(product.id)} aria-label="Wishlist" className="w-11 h-11 flex items-center justify-center shrink-0" style={{border:"1px solid rgba(36,62,65,0.2)"}}>
...
function CheckoutView({ navigate, cart, setCart, setOrder, appliedCoupon, setAppliedCoupon, user, setPostLoginRedirect }) {
  ...
  const selectSavedAddress=id=>{
    ...
    if(a) setForm(f=>({...f, address:a.line, city:a.city, state:a.state||"", pin:a.pin, phone:a.phone}));
  };
  const subtotal=cart.reduce((s,i)=>s+i.price*i.qty,0);
  const [shipSettings,setShipSettings]=useState({shippingCost:150,freeShippingThreshold:2500});
  useEffect(()=>{ fetch("/api/coupons/shipping").then(r=>r.json()).then(setShipSettings).catch(()=>{}); },[]);
    ...
    try{
      const res=await apiFetch("/api/orders/checkout",{
        ...
        body:JSON.stringify({
          ...
          address:{ name:sanitize(form.name), line:sanitize(form.address), city:sanitize(form.city), state:sanitize(form.state), pin:form.pin },
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-14">
      <form onSubmit={submit} noValidate>
        ...
        {savedAddresses.length>0&&<div className="flex flex-wrap gap-2 mb-5">
          {savedAddresses.map(a=><button key={a.id} type="button" onClick={()=>selectSavedAddress(a.id)}
            className="text-left px-4 py-2.5 text-[12.5px]" style={{border:`1px solid ${selectedAddressId===a.id?T.teal:"rgba(36,62,65,0.2)"}`,backgroundColor:selectedAddressId===a.id?"rgba(36,62,65,0.05)":"transparent",color:T.teal}}>
            {a.name} — {a.line.slice(0,30)}{a.line.length>30?"…":""}
          </button>)}
          <button type="button" onClick={()=>{setSelectedAddressId(null);setForm(f=>({...f,address:"",city:"",state:"",pin:""}));}}
            className="text-left px-4 py-2.5 text-[12.5px]" style={{border:`1px solid ${selectedAddressId===null?T.teal:"rgba(36,62,65,0.2)"}`,color:T.teal}}>+ New Address</button>
        </div>}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
        ...
        <SweepButton filled type="submit" disabled={stage==="processing"} className="w-full">
          {stage==="processing"?"Opening secure payment…":`Place Order — ₹${total.toLocaleString("en-IN")}`}
        </SweepButton>
      ...
      <div className="h-fit p-7" style={{backgroundColor:T.card,boxShadow:ELEVATION.raised,borderRadius:RADIUS.lg}}>
        ...
        <div className="pt-4 mt-3 flex flex-col gap-2.5 text-[13.5px]" style={{borderTop:"1px solid rgba(36,62,65,0.12)",color:"rgba(36,62,65,0.7)"}}>
          ...
          {discount>0&&<div className="flex justify-between" style={{color:T.gold}}><span>Discount ({appliedCoupon?.code})</span><span>−₹{discount.toLocaleString("en-IN")}</span></div>}
          {[["Shipping",shipping===0?"Free":`₹${shipping}`],["GST (18%)",`₹${gst.toLocaleString("en-IN")}`]].map(([l,v])=><div key={l} className="flex justify-between"><span>{l}</span><span>{v}</span></div>)}
        </div>
...
function OrderConfirmedView({ navigate, order }) {
  ...
  useEffect(()=>{
    ...
    fetch(`/api/orders/${order.orderNumber}`,{credentials:"include"})
      ...
      .then(data=>setVerified(data?.order?.paymentStatus==="paid"))
...
async function downloadInvoicePDF(order){
  ...
  const doc=new jsPDF();
  const { subtotal=0, discount=0, shippingCost:shipCost=0, cgst=0, sgst=0, total:grandTotal=0 } = order;
  ...
  doc.setFont("helvetica","bold"); doc.setFontSize(11);
  doc.text("Total Paid",140,y); doc.text(`Rs ${grandTotal.toLocaleString("en-IN")}`,196,y,{align:"right"});
    ...
    <div className="invoice-print-area p-8 md:p-12" style={{backgroundColor:"white",boxShadow:ELEVATION.raised,borderRadius:RADIUS.md}}>
      ...
      <div className="flex justify-end mb-10">
        <div className="w-full sm:w-[280px]">
          ...
          {discount>0&&<div className="flex justify-between text-[12.5px] mb-2" style={{color:T.gold}}><span>Discount{order.couponCode?` (${order.couponCode})`:""}</span><span>−₹{discount.toLocaleString("en-IN")}</span></div>}
          {[["Shipping",shipCost],["CGST (9%)",cgst],["SGST (9%)",sgst]].map(([l,v])=><div key={l} className="flex justify-between text-[12.5px] mb-2" style={{color:"rgba(36,62,65,0.65)"}}><span>{l}</span><span>{v===0?"Free":`₹${v.toLocaleString("en-IN")}`}</span></div>)}
    ...
    :<>
      ...
      {currentIdx>=3?<div className="flex items-center justify-between p-5 mb-6" style={{backgroundColor:T.card,boxShadow:ELEVATION.raised,borderRadius:RADIUS.md}}>
        ...
        {order.courierTrackingUrl
          ?<a href={order.courierTrackingUrl} target="_blank" rel="noopener noreferrer" className="text-[11.5px] uppercase tracking-[0.08em]" style={{color:T.gold}}>Track with Courier →</a>
  ...
  return <div className="min-h-[80vh] flex items-center justify-center px-4 py-10 md:px-8">
    <div className="w-full max-w-[860px] grid grid-cols-1 md:grid-cols-[1fr_1.15fr]" style={{borderRadius:RADIUS.lg,overflow:"hidden",boxShadow:ELEVATION.modal}}>
      <div className="hidden md:flex flex-col justify-between p-10 relative overflow-hidden" style={{backgroundColor:T.teal}}>
        ...
        <p className="text-[12px] relative z-10" style={{color:"rgba(255,255,255,0.4)"}}>Mumbai studio</p>
      ...
      <div className="p-8 md:p-11 flex flex-col justify-center" style={{backgroundColor:T.cream}}>
        ...
        {eyebrow&&<p className="text-[11px] tracking-[0.2em] uppercase mb-2" style={{color:T.gold}}>{eyebrow}</p>}
...
function LoginView({ navigate, onLogin, postLoginRedirect, setPostLoginRedirect }) {
  ...
  return <AuthLayout eyebrow="Welcome back" title="Sign In" subtitle="New here? — sign up takes a minute.">
    ...
    <p className="text-[13px] text-center mt-6" style={{color:"rgba(36,62,65,0.55)"}}>Don't have an account? <button onClick={()=>navigate("signup")} className="underline" style={{color:T.gold}}>Register</button></p>
  </AuthLayout>;
}
...
function SignupView({ navigate, onLogin, postLoginRedirect, setPostLoginRedirect }) {
  ...
  return <AuthLayout eyebrow="Join Us" title="Create Account" subtitle="Track orders, save addresses, and check out faster.">
    ...
    <p className="text-[13px] text-center mt-6" style={{color:"rgba(36,62,65,0.55)"}}>Already have one? <button onClick={()=>navigate("login")} className="underline" style={{color:T.gold}}>Sign in</button></p>
  </AuthLayout>;
}
...
// flow once a password-reset email/token backend exists.
      ...
      :<form onSubmit={submit} noValidate className="flex flex-col gap-4">
        ...
        <div>
          ...
          <textarea required rows={5} value={form.message} onChange={e=>setForm(f=>({...f,message:sanitize(e.target.value)}))} maxLength={2000}
            className="w-full bg-transparent outline-none text-[14px]" style={{border:`1px solid ${errors.message?T.error:"rgba(36,62,65,0.22)"}`,padding:"13px 14px",color:T.teal,fontFamily:"'Space Grotesk',sans-serif",resize:"vertical"}}/>
...
function MyAccountView({ navigate, wishlist, user, setUser, order, setOrder, initTab }) {
  ...
  const saveAddress=async()=>{
    ...
    try{
      ...
      const data=await res.json();
      if(res.ok){ setAddrForm({name:"",line:"",city:"",state:"",pin:"",phone:""}); setShowAddrForm(false); loadAddresses(); showToast("Address saved","success"); }
      ...
      :<div className="flex flex-col gap-4">
        {orders.map(o=><div key={o.orderNumber} className="p-6" style={{backgroundColor:T.card,boxShadow:ELEVATION.raised,borderRadius:RADIUS.md}}>
          <div className="flex items-start justify-between mb-4 flex-wrap gap-3">
            ...
            <span className="text-[10.5px] uppercase tracking-[0.08em] px-3 py-1.5" style={o.paymentStatus==="paid"?{backgroundColor:"rgba(184,147,90,0.14)",color:T.gold}:o.paymentStatus==="refunded"?{backgroundColor:"rgba(36,62,65,0.07)",color:T.teal}:{backgroundColor:"rgba(192,57,43,0.08)",color:T.error}}>
              {o.paymentStatus==="paid"?(stageIndexFromOrder(o)>=4?"Delivered":stageIndexFromOrder(o)>=3?"In Transit":"Processing"):o.paymentStatus==="refunded"?"Refunded":o.paymentStatus==="failed"?"Payment Failed":"Payment Pending"}
            </span>
          </div>
          ...
          {o.paymentStatus==="paid"&&<div className="flex flex-col gap-2 pt-4" style={{borderTop:"1px solid rgba(36,62,65,0.08)"}}>
            {o.items.map(i=><div key={i.id+(i.size||"")} className="flex items-center justify-between gap-3">
              ...
              <button onClick={()=>setReviewTarget({orderNumber:o.orderNumber,productId:i.id,productName:i.name})} className="text-[11.5px] uppercase tracking-[0.05em] underline shrink-0" style={{color:T.gold}}>Write a Review</button>
            </div>)}
          </div>}
        </div>)}
      </div>
    ...
    {tab==="Addresses"&&<div className="max-w-[560px]">
      ...
      {showAddrForm?<div className="p-6" style={{backgroundColor:T.card,boxShadow:ELEVATION.raised,borderRadius:RADIUS.md}}>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
          ...
          <InputField label="PIN Code" value={addrForm.pin} onChange={addrUpd("pin")} error={addrErrors.pin} maxLength={6} required/>
        </div>
        <div className="flex gap-3">
    ...
    {tab==="Profile"&&<div className="max-w-[420px]">
      <form onSubmit={saveProfile} className="flex flex-col gap-5 mb-8">
        ...
        <div>
          ...
          <input value={user?.email||""} disabled className="w-full bg-transparent outline-none text-[14px] cursor-not-allowed" style={{border:"1px solid rgba(36,62,65,0.14)",padding:"13px 14px",color:"rgba(36,62,65,0.5)"}}/>
    ...
    :<form onSubmit={submit} noValidate className="flex flex-col gap-4">
      ...
      <div>
        ...
        <textarea rows={4} value={description} onChange={e=>setDescription(sanitize(e.target.value))} maxLength={1500}
          className="w-full bg-transparent outline-none text-[14px]" style={{border:`1px solid ${errors.description?T.error:"rgba(36,62,65,0.22)"}`,padding:"13px 14px",color:T.teal,fontFamily:"'Space Grotesk',sans-serif",resize:"vertical"}}/>
      ...
      <div>
        ...
        {photoUrl?<div className="flex items-center gap-3 p-3" style={{border:"1px solid rgba(36,62,65,0.22)"}}>
...
// NOTE: still UI-only — no real backend, unlike the Footer's newsletter
// form (see NewsletterForm above), which was fixed to genuinely store
// subscriptions. This page's 3 separate preference categories and
// unsubscribe flow are a bigger, still-open piece of work — the
// newsletter_subscribers table this could eventually read/write already
// has the right columns for it (new_arrivals/promotions/journal), so no
// schema change needed when this gets built for real.
    ...
    return <div className="min-h-screen w-full flex flex-col items-center justify-center px-6 text-center" style={{backgroundColor:T.cream,fontFamily:"'Space Grotesk',system-ui,sans-serif"}}>
      ...
      {productsError
        ...
        : <div className="flex flex-col items-center">
            ...
            <p className="akara-splash-tagline text-[11px] tracking-[0.25em] uppercase mt-5" style={{color:T.gold}}>Artifacts for Modern Spaces</p>
          </div>}
    </div>;
  }
