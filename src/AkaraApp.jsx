            ...
            <div className="mt-2 overflow-hidden" style={{ backgroundColor:T.cream, borderRadius:RADIUS.md, boxShadow:"0 24px 60px -16px rgba(36,62,65,0.35)" }}>
              {matches.length>0 ? <div className="px-5 py-2 overflow-y-auto" style={{maxHeight:"50vh"}}>
                {matches.map(m=><button key={m.id} onClick={()=>{ navigate("product",m.id); closeSearch(); }}
                  ...
                  <span>{m.name}</span><span style={{ color:T.teal }}>₹{m.gstInclusiveBasePrice??m.price}</span>
                </button>)}
    ...
    <div className="px-8 md:px-14 pt-14 pb-10 max-w-[1800px] mx-auto">
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-x-8 gap-y-10">
        ...
        <div>
          ...
          {[["About","about"],["The Craft","craft"],["Contact","contact"],["FAQ","faq"],["Care Guide","care-guide"],["Bulk & Corporate Orders","bulk-orders"],["Android App","android-app"],["My Account","account"]].map(([l,v])=><button key={v} onClick={()=>navigate(v)}
...
function ProductCard({ product, navigate, cart, setCart, wishlist, toggleWishlist }) {
  const {id,name,cat,price,gstInclusiveBasePrice,Art,media}=product;
  ...
  return <div className="group relative flex flex-col cursor-pointer transition-transform duration-500 ease-out"
    ...
    <div className="pt-4 flex items-start justify-between gap-2">
      ...
      <p className="text-[14px] shrink-0 pt-0.5" style={{fontFamily:"'Fraunces',serif",color:T.teal}}>₹{gstInclusiveBasePrice??price}</p>
    </div>
  </div>;
}
                  ...
                  {pair.map(p=>{
                    ...
                    return (
                      <button key={p.id} type="button" onClick={()=>navigate("product",p.id)} className="text-left group">
                        ...
                        <p className="text-[12px]" style={{color:T.muted}}>₹{Number(p.gstInclusiveBasePrice??p.price).toLocaleString("en-IN")}</p>
                      </button>
                    );
                  })}
                </div>
        ...
        return <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14 xl:gap-16 items-center mb-14 md:mb-20">
            ...
            <div className="lg:col-span-5 flex flex-col justify-center lg:pl-2">
              ...
              <div className="mb-8">
                <p className="text-[24px] md:text-[26px]" style={{fontFamily:"'Fraunces',serif",color:T.teal}}>₹{Number(hero.gstInclusiveBasePrice??hero.price).toLocaleString("en-IN")}</p>
                ...
                {supports.map((p,i)=>{
                  ...
                  return (
                    <button key={p.id} type="button" onClick={()=>navigate("product",p.id)}
                      ...
                      <p className="text-[14px]" style={{fontFamily:"'Fraunces',serif",color:"rgba(36,62,65,0.75)"}}>₹{Number(p.gstInclusiveBasePrice??p.price).toLocaleString("en-IN")}</p>
                    </button>
                  );
                })}
              </div>
...
function ProductDetailView({ productId, navigate, cart, setCart, wishlist, toggleWishlist }) {
  ...
  // Real, GST-inclusive figure actually SHOWN to the customer — kept
  // deliberately separate from displayPrice above, which must stay the
  // real, GST-exclusive base (it's used to build the real cart line
  // that checkout prices from; inflating it here would double-charge
  // GST at checkout, a genuine, real pricing bug). Computed with the
  // exact same gstInclusivePrice() helper enrichProduct() uses, so the
  // number on screen always mathematically matches what checkout
  // actually charges for this same real base price.
  const customerFacingPrice=gstInclusivePrice(displayPrice);
  ...
  return <div>
    ...
    <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 flex items-center gap-3 px-4 py-3" style={{backgroundColor:T.card,boxShadow:"0 -8px 24px -14px rgba(36,62,65,0.2)",borderTop:"1px solid rgba(36,62,65,0.1)"}}>
      <div className="shrink-0">
        ...
        <p className="text-[16px]" style={{fontFamily:"'Fraunces',serif",color:T.teal}}>₹{customerFacingPrice.toLocaleString("en-IN")}</p>
      </div>
...
async function downloadInvoicePDF(order){
  ...
  doc.setFont("NotoSans","bold"); doc.setFontSize(18);
  doc.text("ĀKĀRA",14,20);
  doc.setFont("NotoSans","normal"); doc.setFontSize(9);
  doc.text("AKARA is a brand of Precision Forge Labs",14,27);
  ...
  autoTable(doc,{
    ...
    headStyles:{fillColor:[36,62,65],textColor:255,font:"NotoSans"},
  ...
  doc.setFont("NotoSans","normal"); doc.setFontSize(8); doc.setTextColor(120);
    ...
    {tab==="Wishlist"&&(wishedProducts.length===0
      ...
      :<div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
        {wishedProducts.map(p=><div key={p.id} onClick={()=>navigate("product",p.id)} className="cursor-pointer group">
          ...
          <p className="text-[13px]" style={{color:T.teal}}>₹{p.gstInclusiveBasePrice??p.price}</p>
        </div>)}
      </div>
    )}
