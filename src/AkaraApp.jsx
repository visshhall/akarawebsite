...
function SignupView({ navigate, onLogin, postLoginRedirect, setPostLoginRedirect }) {
  ...
  const requestOTP=async e=>{
    ...
    try{
      ...
      if(!res.ok){
        ...
        setOtpChannel(data.channel==="email"?"email":"whatsapp");
        setStep("verify");
      }
    }catch{
      setErrors({form:"Couldn't reach the server. Please check your connection and try again."});
  ...
  if(step==="verify") return <AuthLayout eyebrow="Almost there" title="Verify your number" subtitle={otpChannel==="email"?`Enter the 6-digit code we sent to ${form.email}.`:`Enter the 6-digit code we sent on WhatsApp to +91 ${normalizePhone(form.phone)}.`}>
