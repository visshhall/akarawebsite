...
function enrichProduct(p) {
  ...
  // Product-level stock_qty (admin) overrides derived variant status for badges when set.
  if(hasQty && p.status!=="draft" && p.status!=="hidden" && p.status!=="pre-order"){
    ...
    <button type="button" onClick={()=>navigate("home")}
      ...
      <span
        style={{
          ...
          fontWeight:500,
          /* Match visual height of the ~36–42px wordmark */
...
function CartView({ navigate, cart, setCart, appliedCoupon, setAppliedCoupon, user }) {
  ...
  useEffect(()=>{ fetch("/api/coupons/featured").then(r=>r.ok?r.json():null).then(d=>{ if(d?.coupons?.length) setFeaturedOffer(d.coupons[0]); }).catch(()=>{}); },[]);
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-14">
      ...
      <div className="h-fit p-7" style={{backgroundColor:T.card,boxShadow:ELEVATION.raised,borderRadius:RADIUS.lg}}>
        <h2 className="text-[12px] tracking-[0.1em] uppercase mb-5" style={{color:T.teal}}>Order Summary</h2>
          ...
          <p className="text-[10px] tracking-[0.12em] uppercase mb-2" style={{color:T.teal,fontWeight:600}}>Have a coupon?</p>
          {featuredOffer && !appliedCoupon && (
            <p className="text-[12.5px] mb-2 leading-relaxed" style={{color:"rgba(36,62,65,0.75)"}}>
              {featuredOffer.label || "Special offer"} — use code{" "}
              <button type="button" className="underline font-medium" style={{color:T.gold}} onClick={()=>{ setCoupon(featuredOffer.code); setCouponErr(""); }}>{featuredOffer.code}</button>
              {" "}({featuredOffer.discountPercent}% off). Also shown on the top site banner when featured.
            </p>
          )}
