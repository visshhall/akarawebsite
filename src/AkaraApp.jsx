  ...
  return <div className="group relative flex flex-col cursor-pointer transition-transform duration-500 ease-out"
    ...
    <div className="pt-4 flex items-start justify-between gap-2">
      <div>
        ...
        <p className="text-[10.5px] mt-1" style={{color:"rgba(36,62,65,0.55)"}}>{isSoldOut?"Currently unavailable":"Made to order · 2–3 weeks"}</p>
      </div>
...
function MyAccountView({ navigate, wishlist, user, setUser, order, setOrder, initTab }) {
  ...
  const saveAddress=async()=>{
    ...
    try{
      ...
      const res=editingAddrId
        ? await apiFetch(`/api/addresses/${editingAddrId}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)})
      ...
      if(res.ok){
        ...
        loadAddresses();
        showToast(editingAddrId?"Address updated":"Address saved","success");
  ...
  const startEditAddress=a=>{
    ...
    setAddrForm({name:a.name||"",line:a.line||"",landmark:a.landmark||"",city:a.city||"",state:a.state||"",pin:a.pin||"",phone:a.phone||""});
    setAddrErrors({});
  ...
  return <div className="px-6 md:px-14 py-12 md:py-16 max-w-[1100px] mx-auto">
    ...
    {tab==="Addresses"&&<div className="w-full max-w-[900px]">
      ...
      {addresses.map(a=><div key={a.id} className="flex items-start gap-4 p-5" style={{backgroundColor:T.card,boxShadow:ELEVATION.raised,borderRadius:RADIUS.md,border:"1px solid rgba(24,54,48,0.06)"}}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex gap-3 mt-3">
            <button type="button" onClick={()=>startEditAddress(a)} className="text-[11px] uppercase tracking-[0.08em]" style={{color:T.teal}}>Edit</button>
      ...
      {showAddrForm?<div className="p-6 mb-6" style={{backgroundColor:T.card,boxShadow:ELEVATION.raised,borderRadius:RADIUS.md,border:"1px solid rgba(24,54,48,0.08)"}}><p className="text-[14px] mb-4" style={{fontFamily:"'Fraunces',serif",color:T.teal}}>{editingAddrId?"Edit address":"Add address"}</p>
        ...
        <div className="flex gap-3">
          ...
          <SweepButton onClick={()=>{setShowAddrForm(false);setEditingAddrId(null);setAddrForm({name:"",line:"",landmark:"",city:"",state:"",pin:"",phone:""});}}>Cancel</SweepButton>
        </div>
