...
export const T = {
  cream: "#E3DAC9", card: "#E3DAC9",
  teal: "#183630",
  gold: "#E5C690",
  success: "#3B6E52", warning: "#B5651D", error: "#A83B32",
  // Readable secondary text on cream (avoid 0.45–0.55 opacity — too faint)
  muted: "rgba(24,54,48,0.78)",
  mutedSoft: "rgba(24,54,48,0.68)",
};
...
:focus-visible {
  outline: none;
  box-shadow: 0 0 0 3px rgba(229,198,144,0.4);
}
:focus:not(:focus-visible) {
  outline: none;
  box-shadow: none;
}
/* REAL FIX — browser autofill (a saved password/email being filled in
   automatically) forces its own background color on an input, which a
   normal CSS background-color rule genuinely cannot override — this is
   real, longstanding browser behavior, not a bug in this app's own
   styling. It was never addressed anywhere in this codebase before,
   which is why a saved-credential login showed a stark white (or pale
   yellow) box against the site's cream background, on every single
   input across both the customer site and the admin panel. The real
   fix is this specific -webkit-box-shadow inset trick — an enormous
   inset shadow of the real intended background color, painted OVER
   the browser's forced autofill background, plus forcing the text
   color so autofilled text stays readable. -webkit-text-fill-color is
   required here specifically because autofill also overrides a plain
   color rule on the text itself, not just the background. Applies
   globally, once, rather than needing to be repeated per input.
   30000s transition-delay is the real, standard technique to prevent
   the color from visibly "flashing" back to the browser's own default
   the moment the field is focused/edited after being autofilled. */
input:-webkit-autofill,
input:-webkit-autofill:hover,
input:-webkit-autofill:focus {
  -webkit-box-shadow: 0 0 0 1000px #E3DAC9 inset !important;
  -webkit-text-fill-color: #183630 !important;
...
/* Scroll-row arrow buttons (ScrollableProductRow, AkaraApp.jsx) — hidden
   by default and faded in only on hovering the row's parent container,
   so they read as a subtle, discoverable affordance rather than
   permanent visual clutter sitting over the product photos at all
   times. The (hover: none) media query covers touch devices, where
   there's no real hover state to reveal them WITH — those need the
   arrows visible unconditionally, since the whole reveal-on-hover
   mechanism doesn't apply to touch at all. */
.akara-scroll-arrow {
  opacity: 0;
  transition: opacity 0.25s ease;
}
.akara-scroll-hover-area:hover .akara-scroll-arrow {
  opacity: 1;
}
@media (hover: none) {
  .akara-scroll-arrow {
...
@keyframes akaraSplashLetter { from { opacity:0; transform:translate3d(0,14px,0); } to { opacity:1; transform:translate3d(0,0,0); } }
@keyframes akaraSplashTagline { from { opacity:0; transform:translate3d(0,4px,0); } to { opacity:1; transform:translate3d(0,0,0); } }
...
/** Post a client-side error to /api/client-errors (rate-limited). Fire-and-forget. */
export function reportClientError(err, extra = {}) {
  try {
    ...
    const body = {
      ...
      url: typeof window !== "undefined" ? String(window.location.href).slice(0, 400) : "",
      view: extra.view || "",
    };
    // Prefer sendBeacon so unload crashes still leave a trail
...
export async function apiFetch(url, options={}){
  ...
  // Stale CSRF after long sessions — refresh once and retry
  if (res.status === 403) {
...
export function InputField({ label, type="text", value, onChange, onBlur, error, placeholder, maxLength=200, required }) {
  ...
  return <div>
    <label className="block text-[11px] tracking-[0.08em] uppercase mb-2" style={{ color:"rgba(36,62,65,0.75)" }}>{label}{required && " *"}</label>
    <div className="relative">
      ...
      {isPass && <button type="button" onClick={()=>setShow(s=>!s)} className="absolute right-3 top-1/2 -translate-y-1/2" style={{ color:"rgba(36,62,65,0.75)" }}>{show?<EyeOff size={15}/>:<Eye size={15}/>}</button>}
    </div>
  ...
  return <ToastContext.Provider value={showToast}>
    ...
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[200] flex flex-col gap-2 items-center pointer-events-none">
      {toasts.map(t => {
        ...
        return <div key={t.id} className="flex items-center gap-2.5 px-5 py-3.5 text-[13.5px] pointer-events-auto"
          style={{ backgroundColor: T.teal, color: T.cream, borderRadius: RADIUS.sm, boxShadow: ELEVATION.modal, borderLeft: `3px solid ${t.type==="error"?T.error:T.gold}` }}>
          <Icon size={ICON.sm} style={{ color: t.type==="error"?T.error:T.gold, flexShrink: 0 }}/>
          <span>{t.message}</span>
        </div>;
      })}
    </div>
...
export function Modal({ open, onClose, title, children, danger=false, confirmLabel="Confirm", onConfirm, cancelLabel="Cancel" }) {
  ...
  return <div className="fixed inset-0 z-[300] flex items-center justify-center px-6" style={{ backgroundColor: "rgba(36,62,65,0.45)" }} onClick={onClose}>
    <div className="w-full max-w-[400px] p-7" style={{ backgroundColor: T.card, borderRadius: RADIUS.md, boxShadow: ELEVATION.modal }} onClick={e => e.stopPropagation()}>
      ...
      <div className="text-[14px] mb-7" style={{ color:"rgba(36,62,65,0.75)", lineHeight: 1.6 }}>{children}</div>
...
export function EmptyState({ icon: Icon = Inbox, title, message, actionLabel, onAction }) {
  return <div className="flex flex-col items-center text-center py-16 px-6">
    <div className="w-16 h-16 flex items-center justify-center mb-5" style={{ backgroundColor: "rgba(24,54,48,0.06)", borderRadius: "50%", border: "1px solid rgba(24,54,48,0.1)" }}>
      <Icon size={ICON.lg} style={{ color: T.teal, opacity: 0.7 }}/>
    </div>
    {title && <p className="text-[16px] mb-1.5" style={{ fontFamily: "'Fraunces',serif", fontStyle: "italic", color: T.teal }}>{title}</p>}
    {message && <p className="text-[13.5px] mb-6 max-w-[320px]" style={{ color:"rgba(36,62,65,0.75)" }}>{message}</p>}
...
// ---------------------------------------------------------------------------
// Cloudflare Turnstile — loads only when server reports enabled + siteKey.
// Parent reads token via onToken; pass turnstileToken on API bodies.
// ---------------------------------------------------------------------------
...
export function TurnstileWidget({ onToken, className = "" }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/auth/turnstile-config", { credentials: "include" })
      ...
      .then((d) => {
        if (cancelled) return;
        if (d?.enabled && d?.siteKey) {
          setEnabled(true);
          setSiteKey(d.siteKey);
        } else {
          setEnabled(false);
          try { onTokenRef.current?.(""); } catch { /* */ }
        }
      })
      .catch(() => {
        if (!cancelled) {
      ...
      .then(() => {
        ...
        try {
          widgetId.current = window.turnstile.render(boxRef.current, {
            sitekey: siteKey,
            theme: "light",
            callback: (token) => { try { onTokenRef.current?.(token || ""); } catch { /* */ } },
            "expired-callback": () => { try { onTokenRef.current?.(""); } catch { /* */ } },
            "error-callback": () => {
              try { onTokenRef.current?.(""); } catch { /* */ }
              setErr("Security check error — refresh the page.");
            },
          });
        } catch (e) {
          setErr("Could not start security check.");
        }
      })
      .catch(() => setErr("Could not load security check."));
    return () => {
      cancelled = true;
  ...
  return (
    <div className={className}>
      <div ref={boxRef} className="flex justify-center min-h-[65px]" />
      {err && <p className="text-[11.5px] mt-1.5 text-center" style={{ color: T.error }}>{err}</p>}
    </div>
  );
}

// ANALYTICS — GA4 (if present) + first-party beacon to /api/analytics/event
// ============================================================================
function getAnalyticsSessionId() {
  try {
    let id = sessionStorage.getItem("akara_sid");
    if (!id) {
      id = (crypto.randomUUID && crypto.randomUUID()) || `s_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`;
      sessionStorage.setItem("akara_sid", id);
    }
    return id;
  } catch {
    return null;
...
/** Fire a behaviour event to GA4 (best-effort) and our own DB. */
export function trackAkara(event, meta = {}) {
    ...
    if (typeof window.gtag === "function") {
      if (event === "page_view") {
        window.gtag("event", "page_view", {
          ...
          page_title: document.title || "ĀKĀRA",
        });
      } else if (event === "view_item" && safeMeta.productId) {
        window.gtag("event", "view_item", {
          currency: "INR",
          value: Number(safeMeta.price) || 0,
          items: [{
            item_id: String(safeMeta.productId),
            item_name: String(safeMeta.name || safeMeta.productId),
            price: Number(safeMeta.price) || 0,
            quantity: 1,
          }],
        });
        ...
        window.gtag("event", "add_to_cart", {
          currency: "INR",
          value: (Number(safeMeta.price) || 0) * (Number(safeMeta.qty) || 1),
          items: [{
            item_id: String(safeMeta.productId || ""),
            item_name: String(safeMeta.name || ""),
            price: Number(safeMeta.price) || 0,
            quantity: Number(safeMeta.qty) || 1,
          }],
        });
      } else if (event === "begin_checkout") {
        const items = Array.isArray(safeMeta.itemsList)
          ? safeMeta.itemsList
          : undefined;
        window.gtag("event", "begin_checkout", {
          currency: "INR",
          value: Number(safeMeta.value) || 0,
          ...(items ? { items } : { items: [{ item_id: "cart", quantity: Number(safeMeta.items) || 1 }] }),
        });
      } else if (event === "purchase") {
        // transaction_id required for Ads purchase conversion de-dupe
        window.gtag("event", "purchase", {
          transaction_id: String(safeMeta.orderNumber || safeMeta.transaction_id || ""),
          currency: "INR",
          value: Number(safeMeta.value) || 0,
          ...(Array.isArray(safeMeta.itemsList) ? { items: safeMeta.itemsList } : {}),
        });
        // Optional Google Ads conversion (set window.__AKARA_ADS_SEND_TO = "AW-XXXX/label")
        if (typeof window.__AKARA_ADS_SEND_TO === "string" && window.__AKARA_ADS_SEND_TO) {
          window.gtag("event", "conversion", {
            send_to: window.__AKARA_ADS_SEND_TO,
            value: Number(safeMeta.value) || 0,
            currency: "INR",
            transaction_id: String(safeMeta.orderNumber || ""),
          });
        }
      } else if (event === "search") {
        window.gtag("event", "search", { search_term: String(safeMeta.q || "") });
      } else {
        window.gtag("event", event, safeMeta);
      }
    }
  } catch { /* GA blocked or unavailable */ }
  ...
  try {
    ...
    // Prefer fetch: sendBeacon is easy to rate-limit silently and some browsers
    // drop JSON blobs; keepalive still runs during page unload.
