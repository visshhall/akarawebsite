  ...
  return <div className="p-5 md:p-8">
      ...
      {statCards.map(({label,val,icon:Icon,tint,iconColor})=>
        <div key={label} className="p-5" style={{backgroundColor:T.card,borderRadius:RADIUS.md,boxShadow:ELEVATION.raised}}>
          <div className="flex items-center gap-2.5 mb-3">
            ...
            <p className="text-[12.5px] font-medium" style={{color:"rgba(36,62,65,0.75)"}}>{label}</p>
          </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
      <div className="p-5" style={{backgroundColor:T.card,borderRadius:RADIUS.md,boxShadow:ELEVATION.raised}}>
        ...
        {data.lowStock.length===0?<p className="text-[13.5px]" style={{color:"rgba(36,62,65,0.75)"}}>Nothing low or out of stock.</p>:
      ...
      <div className="p-5" style={{backgroundColor:T.card,borderRadius:RADIUS.md,boxShadow:ELEVATION.raised}}>
        ...
        {data.recentOrders.length===0?<p className="text-[13.5px]" style={{color:"rgba(36,62,65,0.75)"}}>No orders yet.</p>:
        <div className="flex flex-col gap-2.5">{data.recentOrders.map(o=><div key={o.orderNumber} className="flex justify-between items-center text-[13.5px] py-1.5" style={{borderBottom:"1px solid rgba(36,62,65,0.06)"}}><span style={{color:T.teal}}>#{o.orderNumber}</span><span className="flex items-center gap-2" style={{color:"rgba(36,62,65,0.75)"}}>₹{o.total.toLocaleString("en-IN")}<Badge variant={o.paymentStatus==="paid"?"success":o.paymentStatus==="refunded"?"neutral":o.paymentStatus==="failed"?"error":"warning"}>{o.paymentStatus}</Badge></span></div>)}</div>}
      </div>
    </div>
  </div>;
}
