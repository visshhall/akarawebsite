...
function AdminSettings({ admin }){
  ...
  const [newCoupon,setNewCoupon]=useState({code:"",discountPercent:"10",expiresAt:"",maxRedemptions:"",usageMode:"once",offerKind:"percent",maxDiscountAmount:"",featured:false,showOnCheckout:true,label:""});
  ...
  const createCoupon=async e=>{
    ...
    const data=await res.json().catch(()=>({}));
    if(res.ok){
      setNewCoupon({code:"",discountPercent:"10",expiresAt:"",maxRedemptions:"",usageMode:"once",offerKind:"percent",maxDiscountAmount:"",featured:false,showOnCheckout:true,label:""});
      loadCoupons();
      ...
      <div className="flex flex-col gap-2 mb-5 max-h-[220px] overflow-y-auto pr-1">
        ...
        coupons.map(c=>(
          <div key={c.code} className="flex flex-wrap items-center gap-x-3 gap-y-2 py-2.5 px-3" style={{border:"1px solid rgba(36,62,65,0.08)",borderRadius:8,background:"rgba(255,255,255,0.35)"}}>
            ...
            <span className="text-[12.5px]" style={{color:"rgba(36,62,65,0.7)"}}>{c.discountPercent}% off{c.maxDiscountAmount!=null?` upto ₹${c.maxDiscountAmount}`:""}</span>
