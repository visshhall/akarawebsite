      ...
      <div className="p-5" style={ATELIER_PANEL}>
        ...
        {payPie.length===0?(
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-3">
            <div style={{width:"100%",height:200,flex:"1 1 50%"}}>
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={payPie} dataKey="value" nameKey="name" innerRadius={48} outerRadius={78} paddingAngle={3}>
            ...
            <ul className="flex flex-col gap-2 flex-1 text-[13px]">
              {payPie.map((row,i)=>(
                <li key={row.name} className="flex items-center gap-2">
                  <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{backgroundColor:PIE_COLORS[i%PIE_COLORS.length]}}/>
                  <span style={{color:T.teal}}>{row.name}</span>
                  <span className="ml-auto tabular-nums" style={{color:"rgba(36,62,65,0.7)"}}>₹{Number(row.value).toLocaleString("en-IN")} · {row.orders} ord.</span>
                </li>
              ))}
            </ul>
          </div>
