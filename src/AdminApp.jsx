...
function AdminDashboard({ setView }){
  ...
  return <div>
    ...
    <div className="p-5 mt-5 overflow-hidden relative z-0" style={{...ATELIER_PANEL, isolation:"isolate"}}>
      ...
      <p className="text-[11.5px] mb-4 leading-relaxed" style={{color:T.mutedSoft}}>All-time figures (not limited to the date range above). Only products with a cost breakdown are listed.</p>
          ...
          return <>
            <div style={{width:"100%",height:300,overflow:"hidden"}}>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={top} margin={{top:8,right:8,left:8,bottom:48}}>
                  <CartesianGrid strokeDasharray="3 3" stroke="rgba(36,62,65,0.08)"/>
                  <XAxis dataKey="name" tick={{fontSize:10,fill:"#183630"}} angle={-30} textAnchor="end" interval={0} height={64}/>
    ...
    <div onClick={e=>e.stopPropagation()} className="w-full max-w-[720px]" style={{backgroundColor:"#F7F3EB",borderRadius:0,boxShadow:"0 24px 80px rgba(24,54,48,0.28)",borderTop:"3px solid #E5C690"}}>
      ...
      <div className="p-6 flex flex-col gap-5 max-h-[70vh] overflow-y-auto">
        ...
        <div className="flex flex-col gap-6">
          ...
          <section className="p-4" style={{border:"1px solid rgba(24,54,48,0.12)",borderRadius:RADIUS.xs,background:"rgba(255,255,255,0.35)"}}>
            <p className="text-[11px] tracking-[0.12em] uppercase mb-2" style={{color:T.teal,fontWeight:600}}>Finish swatches</p>
            <p className="text-[12.5px] mb-3 leading-relaxed" style={{color:"rgba(36,62,65,0.78)"}}>
              Colour chips on the product page (name + optional hex).
            </p>
            {finishes.length>0 && (
              <ul className="flex flex-col gap-2 mb-3">
                {finishes.map((f,i)=>(
                  <li key={i} className="flex items-center gap-3 px-3 py-2.5" style={{border:"1px solid rgba(24,54,48,0.12)",borderRadius:RADIUS.xs,background:T.cream}}>
                    <span className="w-6 h-6 rounded-full border shrink-0" style={{backgroundColor:f.hex||T.cream,borderColor:"rgba(24,54,48,0.25)"}}/>
                    <span className="text-[14px] flex-1" style={{color:T.teal}}>{f.name}{f.hex?` · ${f.hex}`:""}</span>
                    <button type="button" onClick={()=>setFinishes(finishes.filter((_,j)=>j!==i))} className="text-[11px] uppercase tracking-wide shrink-0" style={{color:T.error}}>Remove</button>
                  </li>
                ))}
              </ul>
            )}
            <div className="grid grid-cols-1 sm:grid-cols-[1fr_110px_auto] gap-2 items-end">
              ...
              <div>
                ...
                <input value={newFinishHex} onChange={e=>setNewFinishHex(e.target.value)} className="w-full text-[13px] px-2 py-2.5 min-h-[44px]" style={{border:"1px solid rgba(36,62,65,0.22)",borderRadius:RADIUS.xs,color:T.teal}}/>
              </div>
              <button type="button" className="px-4 py-2.5 text-[12px] tracking-[0.06em] uppercase min-h-[44px]" style={{border:`1px solid ${T.teal}`,borderRadius:RADIUS.xs,color:T.teal}} onClick={()=>{
