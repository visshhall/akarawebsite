...
function AdminOrders(){
  ...
  return <div className="p-5 md:p-8">
    ...
    <div className="flex flex-col gap-2.5 md:gap-0">
      {orders.map(o=><div key={o.orderNumber} className="flex flex-col md:flex-row md:items-center gap-2.5 md:gap-4 p-4 md:px-5 md:py-3" style={{backgroundColor:T.card,borderRadius:RADIUS.sm,boxShadow:ELEVATION.raised,marginBottom:"2px"}}>
        <div className="flex items-center justify-between md:contents">
          <span className="text-[14px] md:w-24" style={{color:T.teal}}>#{o.orderNumber}</span>
          <span className="text-[13px] md:flex-1 md:min-w-[140px]" style={{color:"rgba(36,62,65,0.6)"}}>{o.email}</span>
        </div>
        <div className="flex items-center justify-between md:contents">
    ...
    <Modal open={!!cancelTarget} onClose={()=>setCancelTarget(null)} title="Cancel this order?" danger confirmLabel="Cancel Order" onConfirm={confirmCancel}>
      Order #{cancelTarget} will be marked cancelled. This doesn't automatically process a refund — if the customer already paid, that needs to be handled separately via the Razorpay dashboard.
    </Modal>
  </div>;
}
