...
/** Hover / focus / tap explainers — works on desktop hover and mobile PWA tap */
function AdminInfoTip({ text, align = "center" }) {
  ...
  return (
    <span className={"relative inline-flex group align-middle ml-1.5" + (open ? " is-open" : "")} style={{ verticalAlign: "middle" }}>
      <button
        type="button"
      ...
      <span
        role="tooltip"
        className={"absolute " + pos + " bottom-full mb-2 w-60 sm:w-72 p-3 text-[11.5px] leading-snug transition-opacity z-[80] " + (open ? "opacity-100 visible pointer-events-auto" : "opacity-0 invisible pointer-events-none group-hover:opacity-100 group-hover:visible group-focus-within:opacity-100 group-focus-within:visible")}
        style={{ background: "#1a2e2c", color: "#F7F3EB", borderRadius: 8, boxShadow: "0 10px 28px rgba(0,0,0,0.22)", border: "1px solid rgba(196,163,90,0.25)" }}
      >
        {text}
      </span>
    </span>
  ...
  return <div>
    <div className="mb-8 pb-6 flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4"
      ...
      <div className="min-w-0">
        <p className="text-[10px] tracking-[0.22em] uppercase mb-2" style={{color:T.gold}}>
          {refreshing?"Refreshing…":lastUpdated?`Live · ${lastUpdated.toLocaleTimeString("en-IN",{hour:"2-digit",minute:"2-digit"})}`:"Live"}
          <span style={{color:"rgba(36,62,65,0.45)"}}> · auto 1 min</span>
        </p>
    ...
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-5 mb-5">
      <div className="xl:col-span-2 p-5" style={ATELIER_PANEL}>
        <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
          <p className="text-[13px] uppercase tracking-[0.06em] font-medium" style={{color:T.teal}}>Revenue trajectory <AdminInfoTip text="Paid + COD revenue by day in the selected range. Confirmed money only — not pending checkouts." /></p>
      ...
      <div className="p-5" style={ATELIER_PANEL}>
        <p className="text-[13px] uppercase tracking-[0.06em] mb-3 font-medium" style={{color:T.teal}}>Category mix <AdminInfoTip text="Share of revenue (or units) by product category for the selected range." /></p>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
      ...
      <div className="p-5" style={ATELIER_PANEL}>
        ...
        <p className="text-[11px] mt-4 leading-relaxed" style={{color:"rgba(36,62,65,0.55)"}}>
          “Needs attention” is the sum of pending returns + low/sold-out stock + failed/pending payments (14d). It is not “orders stuck in production.” Click a row above to open the matching section.
        </p>
      </div>
    </div>
    ...
    <div className="p-5" style={ATELIER_PANEL}>
      <div className="flex items-center justify-between mb-1">
        <p className="text-[13px] uppercase tracking-[0.06em] font-medium" style={{color:T.teal}}>Cost &amp; profit per product <AdminInfoTip text="Uses Material/Labor/… cost fields you enter on each product × units sold. Not shown on the customer site." /></p>
        <p className="text-[12px] mb-2" style={{color:"rgba(36,62,65,0.7)"}}>Live from admin cost fields + paid orders — <strong>not</strong> on the customer homepage. Enter Material/Labor/… under each product → Basics/Cost, then numbers appear here after sales.</p>
...
function ProductEditorPanel({ product, onClose, onSaved }){
  ...
  const save=async()=>{
    ...
    setSaving(true);
    try{
      const stockQtyNum=form.stockQty===""||form.stockQty==null?null:Number(form.stockQty);
      let statusToSave=form.status;
      if(stockQtyNum!=null&&Number.isFinite(stockQtyNum)&&!["draft","hidden","pre-order"].includes(form.status)){
  ...
  return <div className="fixed inset-0 z-[100] flex items-start justify-center overflow-y-auto py-8 px-4" style={{backgroundColor:"rgba(36,62,65,0.45)"}} onClick={onClose}>
    <div onClick={e=>e.stopPropagation()} className="w-full max-w-[720px]" style={{backgroundColor:"#F7F3EB",borderRadius:0,boxShadow:"0 24px 80px rgba(24,54,48,0.28)",borderTop:"3px solid #E5C690"}}>
      ...
      <div className="p-6 flex flex-col gap-5 max-h-[70vh] overflow-y-auto">
        {editorSection==="basics" && <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              <InputField label="Stock quantity (optional)" value={form.stockQty} onChange={v=>upd("stockQty")(v.replace(/\D/g,""))} placeholder="e.g. 90"/>
              <p className="text-[11px] mt-1.5 leading-relaxed" style={{color:"rgba(36,62,65,0.55)"}}>When set: &gt;20 in stock · 11–20 low stock · 1–10 shows “N remaining” · 0 sold out. Leave blank to use status only. Saved in the database.</p>
            </div>
          </div>
        </div>}
...
function AdminProducts(){
  ...
  return <div className="p-5 md:p-8">
    <AdminPageHeader
      ...
      subtitle="Collection ledger — form, status, stock, and media for every piece."
...
function AdminOrders(){
  ...
  const toggleOrder=n=>setExpandedOrders(m=>({...m,[n]:!m[n]}));
  ...
  return <div className="p-5 md:p-8">
    ...
    <div className="flex flex-col gap-3">
      {orders.map((o,oi)=>{
        ...
        return <div key={o.orderNumber} className="flex flex-col gap-4 p-4 md:p-5" style={ATELIER_PANEL}>
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between gap-2">
            ...
            <span className="text-[10px] tracking-[0.12em] mr-2 hidden sm:inline" style={{color:"rgba(24,54,48,0.45)"}}>{String(oi+1).padStart(2,"0")}</span>
            <span className="text-[16px] md:shrink-0" style={{color:T.teal,fontFamily:"'Fraunces',serif",fontWeight:400}}>#{o.orderNumber}</span>
...
function AdminActivityLog(){
  ...
  return <div className="p-5 md:p-8">
    <AdminPageHeader eyebrow="System" title="Activity log"
      info="Audit trail of admin actions (product/order/settings changes) for this panel." subtitle="Every product and order change from this panel — most recent first." />
    ...
    <div className="flex flex-col gap-2">
      {entries.map(e=>{ const isFailure=e.action.includes("failed")||e.action.includes("skipped"); return <div key={e.id} className="flex items-center justify-between gap-4 py-4 px-1" style={{...ATELIER_ROW, borderLeft:isFailure?`2px solid ${T.error}`:"2px solid transparent", backgroundColor:isFailure?"rgba(168,59,50,0.05)":"transparent", paddingLeft:12}}>
        <div className="min-w-0">
...
function AdminSettings({ admin }){
  ...
  return <div className="p-5 md:p-8">
    ...
    <div className="p-6 flex flex-col h-full min-h-[280px]" style={ATELIER_PANEL}>
      <p className="text-[10px] tracking-[0.16em] uppercase mb-2" style={{color:T.teal,fontWeight:600}}>04 · Offers</p>
      ...
      <form onSubmit={createCoupon} className="grid grid-cols-2 gap-3">
        ...
        <div className="col-span-2 pt-2"><SweepButton filled type="submit" className="w-full sm:w-auto">Save coupon</SweepButton><p className="text-[11px] mt-2" style={{color:"rgba(36,62,65,0.55)"}}>Creates the coupon in the database immediately.</p></div>
      </form>
    </div>
    ...
    <div className="p-5 md:p-8">
      ...
      <div className="p-5 mb-6" style={ATELIER_PANEL}>
        ...
        {chartData.length === 0 ? <p className="text-[13px]" style={{ color: T.muted }}>No events in this range yet — open the live site and browse a few pages.</p> : (
          <div style={{ width: "100%", height: 280 }}>
            {(chartData.length===0||chartData.every(d=>!(d.page_views||d.view_item||d.add_to_cart||d.purchase)))&&(
        <p className="text-[13px] mb-4 p-4" style={{...ATELIER_PANEL,color:"rgba(36,62,65,0.75)"}}>
          No behaviour events in this range yet. Open the live site in a browser (or have customers browse) so <code style={{fontSize:11}}>page_view</code> / product views are recorded — then refresh this page. Also confirm <code style={{fontSize:11}}>npm run db:sync</code> created <code style={{fontSize:11}}>analytics_events</code>.
        </p>
      )}
      <ResponsiveContainer>
              <AreaChart data={chartData} margin={{ top: 8, right: 12, left: 0, bottom: 0 }}>
