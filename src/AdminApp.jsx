    ...
    <div className="w-full max-w-[760px] grid grid-cols-1 md:grid-cols-[1fr_1.1fr]" style={{borderRadius:RADIUS.lg,overflow:"hidden",boxShadow:ELEVATION.modal}}>
      ...
      <div className="p-8 md:p-10 flex flex-col justify-center" style={{backgroundColor:T.cream}}>
        ...
        {pendingToken?<>
          ...
          <p className="text-[13.5px] mb-7" style={{color:"rgba(36,62,65,0.55)"}}>Enter the 6-digit code from your authenticator app, or a backup code.</p>
          ...
          <form onSubmit={submitTwoFactor} noValidate className="flex flex-col gap-4">
            ...
            <button type="button" onClick={()=>{setPendingToken(null);setCode("");setErr("");}} className="text-[12.5px] underline" style={{color:"rgba(36,62,65,0.5)"}}>Back to sign in</button>
          </form>
        </>:<>
...
function AdminShell({ admin, view, setView, onLogout, children }){
  ...
  const items=allItems.filter(([,,,roles])=>roles.includes(admin?.role)).map(([k,l,i])=>[k,l,i]);
  const nav=<>
...
// ============================================================================
// PHASE 2 — MEDIA GALLERY EDITOR. Uploads go through a dedicated
// admin-only endpoint (POST /api/admin/products/media/upload,
// requireAdmin) — genuinely separate from the customer-facing upload
// route used by return-request photos, even though both share the same
// validated processUpload()/R2 pipeline underneath. Saving the actual
// media array reuses the product's own existing PUT endpoint (no new
// save endpoint needed) — this component just builds the array
// ({type,src} objects, exactly what ProductGallery on the customer
// side already expects) and hands it to the parent's existing save
// flow, the same as every other field in the editor.
// ============================================================================
// VARIANT EDITOR — size/color variants, confirmed spec: both size and
// color independently affect price and stock, tracked separately per
// combination; colors are custom per product (not a shared palette).
// Genuinely separate save action from the main product form below it —
// same reasoning as MediaGalleryEditor: a product's colors/variants
// live in their own real tables (product_colors/product_variants), not
// as columns on products, so they need their own PUT call
// (/api/admin/products/:id/variants) rather than riding along in the
// main save() below.
//
// Colors are tracked with a stable REACT KEY (tempKey) separate from
// their real database id — a brand-new color being added in this same
// editing session has no database id yet (won't exist until the save
// actually completes), so the UI can't rely on `id` for React's key
// prop or for matching a variant row to its color while editing.
function VariantEditor({ productId }){
  ...
  useEffect(()=>{
    fetch(`/api/admin/products/${productId}`,{credentials:"include"}).then(r=>r.json()).then(d=>{
  ...
  return <div>
    <div className="flex items-center justify-between mb-3">
      <p className="text-[11px] tracking-[0.1em] uppercase" style={{color:"rgba(36,62,65,0.45)"}}>Colors</p>
      <SweepButton onClick={addColor}>+ Add Color</SweepButton>
    </div>
    {colors.length===0
      ? <p className="text-[12.5px] mb-4" style={{color:"rgba(36,62,65,0.5)"}}>No colors — this product shows no color picker to customers. Add one to start giving it real color/size variants.</p>
    ...
    <div className="flex items-center justify-between mb-3">
      <p className="text-[11px] tracking-[0.1em] uppercase" style={{color:"rgba(36,62,65,0.45)"}}>Size / Color / Price / Stock</p>
      <SweepButton onClick={addVariant}>+ Add Combination</SweepButton>
    </div>
    {variants.length===0
      ? <p className="text-[12.5px] mb-4" style={{color:"rgba(36,62,65,0.5)"}}>No size/color combinations yet.</p>
...
function MediaGalleryEditor({ productId, category, media, onChange }){
  ...
  return <div>
    <p className="text-[11px] tracking-[0.1em] uppercase mb-3" style={{color:"rgba(36,62,65,0.45)"}}>Photos & Videos</p>
    {media.length===0
      ? <p className="text-[12.5px] mb-4" style={{color:"rgba(36,62,65,0.5)"}}>No media yet — the customer-facing gallery will show a placeholder icon until at least one photo is added.</p>
      : <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-4">
          {media.map((m,i)=><div key={i} className="relative group">
            <div className="w-full overflow-hidden relative" style={{aspectRatio:"1/1",borderRadius:RADIUS.xs,boxShadow:i===0?`0 0 0 2px ${T.gold}`:"0 0 0 1px rgba(36,62,65,0.15)"}}>
              {m.type==="video"
            ...
            {m.photoId&&<button type="button" onClick={()=>copyId(m.photoId,i)} title="Copy photo ID"
              ...
              <Copy size={10} style={{color:"rgba(36,62,65,0.4)",flexShrink:0}}/>
            </button>}
            <div className="flex items-center justify-between mt-1.5 px-0.5">
              <div className="flex gap-1">
                <button type="button" onClick={()=>moveLeft(i)} disabled={i===0} title="Move earlier" style={{color:"rgba(36,62,65,0.5)",opacity:i===0?0.3:1}}><ChevronLeft size={14}/></button>
                <button type="button" onClick={()=>moveRight(i)} disabled={i===media.length-1} title="Move later" style={{color:"rgba(36,62,65,0.5)",opacity:i===media.length-1?0.3:1}}><ChevronRight size={14}/></button>
                {i!==0&&<button type="button" onClick={()=>makeMain(i)} title="Make main photo" style={{color:T.gold}}><Star size={13}/></button>}
              </div>
              <button type="button" onClick={()=>remove(i)} title="Remove" style={{color:T.error}}><Trash2 size={13}/></button>
            </div>
          </div>)}
        </div>}
    <label className="inline-flex items-center gap-2 px-4 py-2.5 text-[12px] tracking-[0.06em] uppercase cursor-pointer" style={{border:`1px solid ${T.gold}`,borderRadius:RADIUS.sm,color:T.gold,opacity:uploading?0.6:1,pointerEvents:uploading?"none":"auto"}}>
    ...
    <p className="text-[11px] mt-2" style={{color:"rgba(36,62,65,0.4)"}}>The first item (marked ★ Main) is what customers see first — use the star or the arrows to reorder. JPEG/PNG/WebP up to 10MB, MP4/WebM/MOV up to 100MB.</p>
  </div>;
}
  ...
  return <div className="fixed inset-0 z-[100] flex items-start justify-center overflow-y-auto py-8 px-4" style={{backgroundColor:"rgba(36,62,65,0.45)"}} onClick={onClose}>
    <div onClick={e=>e.stopPropagation()} className="w-full max-w-[720px]" style={{backgroundColor:T.card,borderRadius:RADIUS.md,boxShadow:ELEVATION.raised}}>
      ...
      <div className="p-6 flex flex-col gap-5 max-h-[70vh] overflow-y-auto">
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-[11px] tracking-[0.08em] uppercase mb-2" style={{color:"rgba(36,62,65,0.55)"}}>Dimensions</label>
              ...
              <p className="text-[11px] mt-1.5" style={{color:"rgba(36,62,65,0.45)"}}>One dimension per line — shows as a bulleted list on the product page once there's more than one line. Useful for a product with several colors, each with its own real measurements.</p>
            </div>
        ...
        {!isNew&&<div>
          <p className="text-[11px] tracking-[0.1em] uppercase mb-3" style={{color:"rgba(36,62,65,0.45)"}}>Size / Color Variants</p>
          <VariantEditor productId={product.id}/>
        </div>}
        <div>
          <p className="text-[11px] tracking-[0.1em] uppercase mb-3" style={{color:"rgba(36,62,65,0.45)"}}>Key Features</p>
          {keyFeatures.length===0
            ? <p className="text-[12.5px] mb-3" style={{color:"rgba(36,62,65,0.5)"}}>No key features yet — the tab only appears on the product page once at least one is added.</p>
          ...
          {keyFeatures.length<12&&<div className="flex gap-2">
            ...
            <button type="button" onClick={addFeature} className="px-4 py-2.5 text-[12px] tracking-[0.06em] uppercase" style={{border:`1px solid ${T.gold}`,borderRadius:RADIUS.xs,color:T.gold}}>Add</button>
          </div>}
        </div>
        <div>
          <p className="text-[11px] tracking-[0.1em] uppercase mb-3" style={{color:"rgba(36,62,65,0.45)"}}>Accessories Note</p>
          ...
          <p className="text-[11px] mt-1.5" style={{color:"rgba(36,62,65,0.45)"}}>Optional — leave blank for most products. Only appears as a real tab on the customer product page when it has real content, so a lamp needing bulb/wire info won't show it for a planter.</p>
        </div>
        <div>
      ...
      <div className="flex gap-3 justify-end px-6 py-5" style={{borderTop:"1px solid rgba(36,62,65,0.1)"}}>
        {!isNew&&<button onClick={()=>window.open(`/preview/${product.id}`,"_blank","noopener,noreferrer")} className="px-5 py-2.5 text-[12px] tracking-[0.08em] uppercase mr-auto" style={{color:T.gold,border:`1px solid ${T.gold}`,borderRadius:RADIUS.sm}}>Preview as Customer</button>}
...
function AdminFeaturedProducts(){
  ...
  return <div className="p-5 md:p-8 max-w-[900px]">
    ...
    <p className="text-[13.5px] mb-6" style={{color:"rgba(36,62,65,0.5)"}}>Shown in this exact order in the homepage's "Featured pieces" section. Up to 12.</p>

    {loading?<Skeleton height={200}/>:<>
      <p className="text-[11px] tracking-[0.1em] uppercase mb-3" style={{color:"rgba(36,62,65,0.45)"}}>Currently Featured ({featuredIds.length})</p>
      {featuredIds.length===0
        ? <p className="text-[12.5px] mb-8" style={{color:"rgba(36,62,65,0.5)"}}>Nothing featured yet — the homepage section stays hidden until you add at least one product below.</p>
        : <div className="flex flex-col gap-2 mb-8">
            {featuredIds.map((id,i)=>{ const p=byId(id); if(!p) return null; return <div key={id} className="flex items-center gap-3 p-3" style={{backgroundColor:T.card,borderRadius:RADIUS.sm,boxShadow:ELEVATION.raised}}>
              <span className="text-[12px] w-5 text-center" style={{color:"rgba(36,62,65,0.4)"}}>{i+1}</span>
              <div className="flex-1 min-w-0">
                <p className="text-[13.5px] truncate" style={{color:T.teal}}>{p.name}</p>
                <p className="text-[11px]" style={{color:"rgba(36,62,65,0.45)"}}>{p.category} · ₹{p.price}</p>
              </div>
              <button type="button" onClick={()=>moveUp(i)} disabled={i===0} style={{color:"rgba(36,62,65,0.5)",opacity:i===0?0.3:1}}><ChevronLeft size={16} style={{transform:"rotate(90deg)"}}/></button>
              <button type="button" onClick={()=>moveDown(i)} disabled={i===featuredIds.length-1} style={{color:"rgba(36,62,65,0.5)",opacity:i===featuredIds.length-1?0.3:1}}><ChevronRight size={16} style={{transform:"rotate(90deg)"}}/></button>
              <button type="button" onClick={()=>removeFeatured(id)} style={{color:T.error}}><Trash2 size={15}/></button>
            </div>;})}
          </div>}

      <p className="text-[11px] tracking-[0.1em] uppercase mb-3" style={{color:"rgba(36,62,65,0.45)"}}>Add a Product</p>
      <div className="flex flex-col gap-2">
        {notFeatured.length===0
          ? <p className="text-[12.5px]" style={{color:"rgba(36,62,65,0.5)"}}>Every product is already featured.</p>
          : notFeatured.map(p=><div key={p.id} className="flex items-center gap-3 p-3" style={{border:"1px solid rgba(36,62,65,0.14)",borderRadius:RADIUS.sm}}>
              ...
              <button type="button" onClick={()=>addFeatured(p.id)} className="px-3 py-1.5 text-[11px] uppercase tracking-[0.06em]" style={{border:`1px solid ${T.gold}`,borderRadius:RADIUS.xs,color:T.gold}}>Add</button>
            </div>)}
      </div>
    </>}
  </div>;
...
function AdminMediaManager(){
  ...
  return <div className="p-5 md:p-8 max-w-[900px]">
    ...
    <p className="text-[13.5px] mb-6" style={{color:"rgba(36,62,65,0.5)"}}>Pick a category, then a product, to upload photos and videos directly to it — no need to open the full product editor.</p>

    {loading?<Skeleton height={44}/>:<>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-5">
        <div>
          <label className="block text-[11px] tracking-[0.08em] uppercase mb-2" style={{color:"rgba(36,62,65,0.55)"}}>Category</label>
        ...
        <div>
          <label className="block text-[11px] tracking-[0.08em] uppercase mb-2" style={{color:"rgba(36,62,65,0.55)"}}>Product</label>
      ...
      {selected&&<div className="p-5" style={{backgroundColor:T.card,borderRadius:RADIUS.md,boxShadow:ELEVATION.raised}}>
        <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
          <div>
            ...
            <p className="text-[11.5px]" style={{color:"rgba(36,62,65,0.5)"}}>{selected.category} · {selected.id}</p>
          </div>
...
function AdminEnquiries(){
  ...
  return <div className="p-5 md:p-8 max-w-[900px]">
    ...
    <p className="text-[13.5px] mb-6" style={{color:"rgba(36,62,65,0.5)"}}>Contact form submissions and Bulk & Corporate Order enquiries, together, most recent first.</p>
      ...
      {shown.map(e=><div key={`${e.type}-${e.id}`} className="p-4 flex gap-3" style={{backgroundColor:T.card,borderRadius:RADIUS.sm,boxShadow:ELEVATION.raised,opacity:e.handled?0.6:1}}>
        ...
        <div className="flex items-start justify-between gap-3 flex-wrap mb-2">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {e.company&&<span className="text-[12px]" style={{color:"rgba(36,62,65,0.5)"}}>· {e.company}</span>}
          </div>
          <span className="text-[11.5px] whitespace-nowrap" style={{color:"rgba(36,62,65,0.45)"}}>{new Date(e.createdAt).toLocaleString("en-IN",{day:"2-digit",month:"short",hour:"2-digit",minute:"2-digit"})}</span>
        </div>
        <div className="flex flex-wrap gap-x-4 gap-y-1 mb-3 text-[12px]" style={{color:"rgba(36,62,65,0.55)"}}>
        ...
        <p className="text-[13.5px] leading-[1.6] mb-3" style={{color:"rgba(36,62,65,0.7)"}}>{e.message}</p>
        <div className="flex gap-3">
          <a href={`mailto:${e.email}?subject=${encodeURIComponent(`Re: Your enquiry to ĀKĀRA`)}`} className="text-[11.5px] uppercase tracking-[0.06em]" style={{color:T.gold}}>Reply by Email</a>
          <button onClick={()=>toggleHandled(e)} className="text-[11.5px] uppercase tracking-[0.06em]" style={{color:"rgba(36,62,65,0.5)"}}>{e.handled?"Mark as New":"Mark as Handled"}</button>
        </div>
        </div>
      </div>)}
    </div>}
...
function AdminNewsletter(){
  ...
  return <div className="p-5 md:p-8">
    ...
    <p className="text-[13.5px] mb-6" style={{color:"rgba(36,62,65,0.5)"}}>{subscribers.length} total subscriber{subscribers.length!==1?"s":""}{search.trim()&&filtered.length!==subscribers.length?` — ${filtered.length} matching "${search}"`:""}</p>
    ...
    <div className="flex flex-col gap-2.5 md:gap-0">
      {filtered.map(s=><div key={s.email} className="flex flex-col md:flex-row md:items-center gap-2 md:gap-4 p-4 md:px-5 md:py-3" style={{backgroundColor:T.card,borderRadius:RADIUS.sm,boxShadow:ELEVATION.raised,marginBottom:"2px"}}>
        <div className="flex-1">
          ...
          <p className="text-[12px]" style={{color:"rgba(36,62,65,0.45)"}}>Subscribed {new Date(s.subscribedAt).toLocaleDateString("en-IN",{day:"2-digit",month:"short",year:"numeric"})}</p>
        </div>
        <div className="flex items-center justify-between md:contents">
    ...
    <Modal open={!!removeTarget} onClose={()=>setRemoveTarget(null)} title="Remove this subscriber?" danger confirmLabel="Remove" onConfirm={confirmRemove}>
      <p className="text-[13.5px]" style={{color:"rgba(36,62,65,0.6)"}}>{removeTarget} will be permanently removed from the newsletter list. This can't be undone from here — they'd need to sign up again.</p>
    </Modal>
  </div>;
}
      ...
      {entries.map(e=>{ const isFailure=e.action.includes("failed")||e.action.includes("skipped"); return <div key={e.id} className="flex items-center justify-between gap-4 p-4" style={{backgroundColor:isFailure?"rgba(168,59,50,0.06)":T.card,borderRadius:RADIUS.sm,boxShadow:ELEVATION.raised,borderLeft:isFailure?`3px solid ${T.error}`:"none"}}>
        <div className="min-w-0">
          ...
          {/* The backend has always sent adminName/adminEmail (joined
              ...
              is. */}
          <p className="text-[11.5px] mt-0.5" style={{color:"rgba(36,62,65,0.4)"}}>by {e.adminName}</p>
        </div>
...
function AdminCms(){
  ...
  if(selectedPage) return <div className="p-5 md:p-8 max-w-[820px]">
    ...
    <p className="text-[13px] mb-6" style={{color:"rgba(36,62,65,0.5)"}}>Use [text](page-key) for an internal link, [text](mailto:address) for email, and **text** for bold. Every save keeps the previous version — see View History to revert.</p>
    ...
    <div className="flex flex-col gap-3 mb-6">
      {blocks.map((b,i)=><div key={i} className="p-4" style={{backgroundColor:T.card,borderRadius:RADIUS.sm,boxShadow:ELEVATION.raised}}>
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-2">
            <button onClick={()=>moveUp(i)} disabled={i===0} style={{color:"rgba(36,62,65,0.5)",opacity:i===0?0.3:1}}><ChevronLeft size={15} style={{transform:"rotate(90deg)"}}/></button>
            <button onClick={()=>moveDown(i)} disabled={i===blocks.length-1} style={{color:"rgba(36,62,65,0.5)",opacity:i===blocks.length-1?0.3:1}}><ChevronRight size={15} style={{transform:"rotate(90deg)"}}/></button>
        ...
        {b.blockType==="heading"
          ...
          ? (()=>{ let parsed={title:"",points:[]}; try{ parsed=JSON.parse(b.content); }catch{} return <div className="flex flex-col gap-2">
              ...
              <button onClick={()=>addBulletPoint(i)} className="text-[11.5px] uppercase tracking-[0.06em] self-start" style={{color:T.gold}}>+ Add Point</button>
            </div>; })()
          : b.blockType==="hero"
          ...
          ? (()=>{ let parsed={eyebrow:"",heading:"",steps:[]}; try{ parsed=JSON.parse(b.content); }catch{} return <div className="flex flex-col gap-2">
              ...
              <button onClick={()=>addArrayItem(i,"steps",{number:String(parsed.steps.length+1).padStart(2,"0"),title:"",description:""})} className="text-[11.5px] uppercase tracking-[0.06em] self-start" style={{color:T.gold}}>+ Add Step</button>
            </div>; })()
          ...
          ? (()=>{ let parsed={cards:[]}; try{ parsed=JSON.parse(b.content); }catch{} return <div className="flex flex-col gap-2">
              ...
              <button onClick={()=>addArrayItem(i,"cards",{title:"",description:""})} className="text-[11.5px] uppercase tracking-[0.06em] self-start" style={{color:T.gold}}>+ Add Card</button>
            </div>; })()
    ...
    <Modal open={showHistory} onClose={()=>setShowHistory(false)} title="Version History">
      ...
      history.length===0?<p className="text-[13.5px]" style={{color:"rgba(36,62,65,0.5)"}}>No past versions yet — history is saved starting from the next time this page is edited.</p>:
      <div className="flex flex-col gap-2">
        {history.map(v=><div key={v.id} className="flex items-center justify-between gap-3 p-3" style={{border:"1px solid rgba(36,62,65,0.14)",borderRadius:RADIUS.xs}}>
          <div>
            ...
            <p className="text-[11.5px]" style={{color:"rgba(36,62,65,0.5)"}}>by {v.savedByName} · {v.blockCount} block{v.blockCount!==1?"s":""}</p>
          </div>
  ...
  return <div className="p-5 md:p-8">
    ...
    <p className="text-[13.5px] mb-6" style={{color:"rgba(36,62,65,0.5)"}}>Edit any page's real content — live immediately, no code or redeploy needed.</p>
    {loading?<Skeleton height={200}/>:
    <div className="flex flex-col gap-2.5">
      {pages.map(p=><button key={p.pageKey} onClick={()=>openPage(p.pageKey)} className="flex items-center justify-between gap-3 p-4 text-left w-full" style={{backgroundColor:T.card,borderRadius:RADIUS.sm,boxShadow:ELEVATION.raised}}>
        <div>
          ...
          <p className="text-[11.5px]" style={{color:"rgba(36,62,65,0.5)"}}>{p.blockCount} block{p.blockCount!==1?"s":""}{p.lastUpdated?` · updated ${new Date(p.lastUpdated).toLocaleDateString("en-IN",{day:"2-digit",month:"short",year:"numeric"})}`:""}</p>
        </div>
        <ChevronRight size={16} style={{color:"rgba(36,62,65,0.4)"}}/>
      </button>)}
    </div>}
  </div>;
}
...
function AdminManageAccounts({ admin }){
  ...
  return <div className="p-5 md:p-8 max-w-[720px]">
    ...
    <p className="text-[13.5px] mb-6" style={{color:"rgba(36,62,65,0.5)"}}>Staff can manage orders only. Admin can manage products, customers, and settings. Super Admin has full access, including this screen.</p>
    ...
    <div className="flex flex-col gap-2.5">
      {accounts.map(a=><div key={a.id} className="flex items-center gap-3 p-4" style={{backgroundColor:T.card,borderRadius:RADIUS.sm,boxShadow:ELEVATION.raised}}>
        <div className="flex-1 min-w-0">
          <p className="text-[14px]" style={{color:T.teal}}>{a.name}{a.id===admin?.id&&<span className="text-[11px] ml-2" style={{color:"rgba(36,62,65,0.4)"}}>(you)</span>}</p>
          <p className="text-[12px]" style={{color:"rgba(36,62,65,0.5)"}}>{a.email}</p>
        </div>
    ...
    <Modal open={showCreate} onClose={()=>setShowCreate(false)} title="New Account">
      <form onSubmit={create} className="flex flex-col gap-4">
        ...
        <p className="text-[11px]" style={{color:"rgba(36,62,65,0.45)"}}>10+ characters, an uppercase letter, a number, and a special character.</p>
        <div>
          <label className="block text-[11px] tracking-[0.08em] uppercase mb-2" style={{color:"rgba(36,62,65,0.55)"}}>Role</label>
    ...
    <Modal open={!!removeTarget} onClose={()=>setRemoveTarget(null)} title="Remove this account?" danger confirmLabel="Remove" onConfirm={remove}>
      <p className="text-[13.5px]" style={{color:"rgba(36,62,65,0.6)"}}>{removeTarget?.name} ({removeTarget?.email}) will lose access immediately. Their past activity log entries are kept, just no longer linked to a live account.</p>
    </Modal>
  </div>;
}
...
function AdminTwoFactorSettings(){
  ...
  if(newBackupCodes) return <div className="p-6" style={{backgroundColor:T.card,borderRadius:RADIUS.md,boxShadow:ELEVATION.raised}}>
    ...
    <p className="text-[12.5px] mb-4" style={{color:"rgba(36,62,65,0.5)"}}>Each code works once, if you ever lose access to your authenticator app. This is the only time these will be shown — save them somewhere real, like a password manager.</p>
  ...
  if(setupData) return <div className="p-6" style={{backgroundColor:T.card,borderRadius:RADIUS.md,boxShadow:ELEVATION.raised}}>
    ...
    <p className="text-[12.5px] mb-4" style={{color:"rgba(36,62,65,0.5)"}}>Scan this with Google Authenticator, Authy, or any real TOTP app, then enter the 6-digit code it shows to confirm.</p>
    <img src={setupData.qrCodeDataUrl} alt="2FA setup QR code" className="mb-4" style={{width:180,height:180,borderRadius:RADIUS.sm}}/>
    <p className="text-[11px] mb-4" style={{color:"rgba(36,62,65,0.4)"}}>Can't scan? Enter this key manually: <span className="font-mono">{setupData.secret}</span></p>
  ...
  return <div className="p-6" style={{backgroundColor:T.card,borderRadius:RADIUS.md,boxShadow:ELEVATION.raised}}>
    ...
    <p className="text-[12.5px] mb-5" style={{color:"rgba(36,62,65,0.5)"}}>
    ...
    <Modal open={showDisableModal} onClose={()=>{setShowDisableModal(false);setDisablePassword("");}} title="Disable two-factor authentication?" danger confirmLabel={submitting?"Disabling…":"Disable"} onConfirm={confirmDisable}>
      <p className="text-[13.5px] mb-4" style={{color:"rgba(36,62,65,0.6)"}}>This removes the extra login step and your saved backup codes. Enter your password to confirm.</p>
