...
function AdminShell({ admin, view, setView, onLogout, children }){
  ...
  return <div className="h-[100dvh] w-full flex overflow-hidden" style={{backgroundColor:T.cream}}>
    ...
    <main className="flex-1 min-w-0 h-full overflow-x-hidden overflow-y-auto pt-14 md:pt-0" style={ADMIN_PAGE_BG}><div className="w-full max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-5 md:py-8">{children}</div></main>
  </div>;
}
...
function AdminRoomStories(){
  ...
  return <div className="p-5 md:p-8 max-w-[960px] mx-auto w-full">
    ...
    <div className="flex flex-col gap-5">
      {boards.map((b, idx)=>(
        <div key={b.id} className="p-4 md:p-6" style={{backgroundColor:T.card,borderRadius:RADIUS.md,boxShadow:ELEVATION.raised,border:"1px solid rgba(24,54,48,0.08)"}}>
      ...
      <div className="flex flex-wrap gap-2 pt-2">
        <SweepButton type="button" onClick={addBoard}>Add another board</SweepButton>
        <SweepButton type="button" filled onClick={save} disabled={saving}>{saving?"Saving…":"Save all"}</SweepButton>
      </div>
    </div>}
  </div>;
