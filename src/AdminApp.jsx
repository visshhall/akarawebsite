...
function VariantEditor({ productId }){
    ...
    fetch(`/api/admin/products/${productId}`,{credentials:"include"})
      ...
      .then(d=>{
        ...
        setVariants((d.variants||[]).map(v=>{
          ...
          return {tempKey:`db-${v.id}`,colorTempKey:ownerColor?.tempKey||null,size:v.size||"",price:v.price,status:v.status,...parseDims(v.dims)};
        }));
      })
  ...
  const addVariant=()=>{
    ...
    setVariants(v=>[...v,{tempKey,colorTempKey:colors[0]?.tempKey||null,size:"",price:"",status:"in-stock",length:"",width:"",height:"",weightKg:""}]);
  };
  ...
  return <div>
    <div className="mb-5 p-3.5" style={{backgroundColor:"rgba(24,54,48,0.05)",borderRadius:RADIUS.xs}}>
      <p className="text-[12.5px] leading-[1.7]" style={{color:"rgba(36,62,65,0.85)"}}>
        <strong>How this works:</strong> if this product comes in different real colours, list them under "Colors" first. Then, below that, add one real row per colour (and per size, if it genuinely comes in more than one) with its own real price, stock status, and dimensions — <strong>that price is exactly what the customer is charged</strong>, not the Basics tab price. Most products don't have real, distinct sizes — pick "No size" for those; only pick Small/Medium/Large if this specific product genuinely comes in more than one real size, and give each one its own real row here so the size picker shown to customers matches exactly what you set up.
      </p>
    </div>
    <div className="flex items-center justify-between mb-3">
      ...
      : <div className="flex flex-col gap-3 mb-2">
          {variants.map(v=><div key={v.tempKey} className="p-3" style={{border:"1px solid rgba(36,62,65,0.14)",borderRadius:RADIUS.xs}}>
            <div className="flex items-center gap-2 flex-wrap mb-2.5">
              ...
              <input value={v.price} onChange={e=>updateVariant(v.tempKey,"price",e.target.value.replace(/\D/g,""))} placeholder="Price ₹ — what the customer pays" className="w-24 bg-transparent outline-none text-[12.5px] px-2 py-1.5" style={{border:"1px solid rgba(36,62,65,0.18)",borderRadius:RADIUS.xs,color:T.teal}}/>
      ...
      {orders.map(o=>{
        ...
        <div className="flex flex-col md:flex-row md:items-center md:flex-wrap gap-3 md:gap-4">
          <div className="flex items-center justify-between md:contents gap-2">
            <span className="text-[15px] md:w-28" style={{color:T.teal,fontFamily:"'Fraunces',serif"}}>#{o.orderNumber}</span>
...
function AdminAndroidAppSettings(){
  ...
  const uploadApk=async(file)=>{
    ...
    try{
      ...
      if(res.ok){
        ...
        showToast(data.error||"Couldn't upload that file.","error");
      }
    }catch{
      showToast("Couldn't reach the server.","error");
    }finally{
  ...
  return <div className="p-6" style={{backgroundColor:T.card,borderRadius:RADIUS.md,boxShadow:ELEVATION.raised}}>
    ...
    <form onSubmit={publish} className="flex flex-col gap-4">
      ...
      <SweepButton filled type="submit" disabled={publishing||(!pendingApkUrl&&!current?.apkUrl)}>{publishing?"Publishing…":"Publish This Version"}</SweepButton>
    </form>
  </div>;
}
