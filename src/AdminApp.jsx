      ...
      <div className="p-5" style={ATELIER_PANEL}>
        ...
        {payPie.length===0?(
          ...
          <div className="flex flex-col items-stretch gap-4">
            <div className="mx-auto" style={{width:"100%",maxWidth:240,height:200}}>
              <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="p-5" style={ATELIER_PANEL}>
        ...
        {returnsPie.length===0?(
          ...
          <div style={{width:"100%",height:200}}>
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={returnsPie} dataKey="value" nameKey="name" innerRadius={45} outerRadius={75} paddingAngle={2}>
...
function ProductEditorPanel({ product, onClose, onSaved }){
  ...
  const [saving,setSaving]=useState(false);
  const upd=k=>v=>{
    if(k==="id"){ setIdManual(true); setForm(f=>({...f,id:v})); return; }
    if(k==="name"&&isNew&&!idManual){
      setForm(f=>({...f,name:v,id:slugifyProductId(v)}));
      return;
    }
    setForm(f=>({...f,[k]:v}));
  };
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {isNew
              ? <div>
                  ...
                  <p className="text-[11px] mt-1.5" style={{color:"rgba(36,62,65,0.55)"}}>Fills automatically from the product name. Edit only if you need a shorter URL.</p>
                </div>
    ...
    <Modal open={!!refundTarget} onClose={()=>!refunding&&setRefundTarget(null)} title={refundTarget?`Refund #${refundTarget.orderNumber}`:"Refund"} confirmLabel={refunding?"Processing…":"Issue refund"} onConfirm={submitPartialRefund} danger>
      {refundTarget&&(()=>{
        ...
        return <div>
          ...
          <p className="text-[12.5px] mb-3" style={{color:"rgba(36,62,65,0.75)"}}>Online payments refund via Razorpay. COD / unpaid orders cannot use this path — adjust status or settle offline.</p>
          <InputField label="Amount to refund (₹)" value={refundAmount} onChange={setRefundAmount}/>
          {remaining>0&&(
...
function AdminSettings({ admin }){
  ...
  const [newCoupon,setNewCoupon]=useState({code:"",discountPercent:"10",expiresAt:"",maxRedemptions:"",usageMode:"once",featured:false,label:""});
  ...
  const createCoupon=async e=>{
    ...
    const data=await res.json().catch(()=>({}));
    if(res.ok){
      setNewCoupon({code:"",discountPercent:"10",expiresAt:"",maxRedemptions:"",usageMode:"once",featured:false,label:""});
      loadCoupons();
  ...
  return <div className="p-5 md:p-8">
    ...
    <div className="p-6 flex flex-col h-full min-h-[280px]" style={ATELIER_PANEL}>
      ...
      <p className="text-[12.5px] mb-2" style={{color:"rgba(36,62,65,0.75)"}}>Deactivating a code retires it immediately without deleting its history.</p><p className="text-[12px] mb-5 leading-relaxed p-3" style={{color:T.teal,background:"rgba(196,163,90,0.1)",borderRadius:8,border:"1px solid rgba(196,163,90,0.25)"}}><strong>Where customers see coupons:</strong> (1) Top teal banner — only codes with Feature Publicly + label. (2) Cart “Have a coupon?” box. Activate alone is not enough for the banner.</p>
      ...
      <div className="flex flex-col gap-2 mb-5">
        ...
        coupons.map(c=><div key={c.code} className="flex items-center justify-between gap-3 py-2 flex-wrap" style={{borderBottom:"1px solid rgba(36,62,65,0.06)"}}>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {c.onePerCustomer&&<Badge variant="neutral">1 per customer</Badge>}{c.firstOrderOnly&&<Badge variant="neutral">welcome once</Badge>}
      ...
      <form onSubmit={createCoupon} className="grid grid-cols-2 gap-3">
        <InputField label="Code" value={newCoupon.code} onChange={v=>setNewCoupon(f=>({...f,code:v.toUpperCase().replace(/[^A-Z0-9]/g,"").slice(0,24)}))} placeholder="e.g. SAVE10"/>
        <div>
          <label className="block text-[11px] tracking-[0.08em] uppercase mb-2" style={{color:"rgba(36,62,65,0.75)"}}>Discount % off MRP/subtotal</label>
        ...
        <InputField label="Max total uses (optional)" value={newCoupon.maxRedemptions} onChange={v=>setNewCoupon(f=>({...f,maxRedemptions:v.replace(/\D/g,"").slice(0,6)}))} placeholder="Blank = unlimited site-wide"/>
        <div className="col-span-2">
          ...
          <select value={newCoupon.usageMode||"once"} onChange={e=>setNewCoupon(f=>({...f,usageMode:e.target.value}))}
            ...
            <option value="once">One time per customer (any order)</option>
            <option value="first">Welcome-style — once per customer</option>
          ...
          <p className="text-[11px] mt-1.5 leading-relaxed" style={{color:"rgba(36,62,65,0.55)"}}>
            “One time” = each account/email/phone can redeem this code once. “Multiple” = same person can reuse until max total uses or expiry.
          </p>
        </div>
        <label className="col-span-2 flex items-center gap-2 text-[13px] cursor-pointer" style={{color:T.teal}}>
          ...
          Show on site banner (Feature Publicly)
        </label>
        {newCoupon.featured&&<div className="col-span-2"><InputField label="Banner text customers see" value={newCoupon.label} onChange={v=>setNewCoupon(f=>({...f,label:v}))} placeholder='e.g. "10% off — use code SAVE10"'/></div>}
        <div className="col-span-2 pt-2"><SweepButton filled type="submit" className="w-full sm:w-auto">Save coupon</SweepButton><p className="text-[11px] mt-2" style={{color:"rgba(36,62,65,0.55)"}}>Saved to the database immediately. Customers apply it at cart (signed-in).</p></div>
      </form>
    </div>
