...
function AdminShell({ admin, view, setView, onLogout, children }){
  ...
  const allGroups=[
    ...
    { label: "Catalog", items: [
      ...
      ["hero-background","Hero image",ImageIcon,["admin","super_admin"]],
...
function AdminHeroBackground(){
  ...
  const load=()=>{
    setLoading(true);
    fetch("/api/admin/settings/hero-background",{credentials:"include"})
      .then(r=>r.json())
      .then(d=>{ setImageUrl(d.imageUrl||""); setDefaultUrl(d.defaultUrl||"/images/hero-home.jpg"); })
      .catch(()=>showToast("Couldn't load hero image settings.","error"))
      .finally(()=>setLoading(false));
  };
  useEffect(()=>{ load(); },[]);
  ...
  return <div className="p-5 md:p-8 max-w-[720px] mx-auto w-full">
    <AdminPageHeader
      ...
      subtitle="One still full-bleed photo behind “Let there be form.” Not a slider — change it whenever you like (e.g. weekly). Text stays readable via a fixed teal scrim."
    />
    {loading?<Skeleton height={200}/>:
    <div className="flex flex-col gap-5">
      <div className="overflow-hidden relative" style={{aspectRatio:"16/9",borderRadius:RADIUS.md,boxShadow:ELEVATION.raised,backgroundColor:T.card}}>
      ...
      <p className="text-[12.5px]" style={{color:"rgba(36,62,65,0.75)"}}>Recommended: landscape, ~2400×1400 or larger, products toward the sides so centered headline stays clear. Default asset: <code className="text-[12px]">/images/hero-home.jpg</code></p>
      <div className="flex flex-wrap gap-3">
        ...
        <SweepButton type="button" onClick={()=>save("")} disabled={saving||!imageUrl}>Reset to default</SweepButton>
      </div>
      <div>
        ...
        <SweepButton type="button" filled onClick={()=>save(imageUrl)} disabled={saving}>{saving?"Saving…":"Save URL"}</SweepButton>
      </div>
    </div>}
  </div>;
}
