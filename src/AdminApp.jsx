...
import {
  ...
  Image as ImageIcon, Film, ChevronLeft, ChevronRight, Star, Upload, Copy, Shield, UserPlus, FileText, GripVertical, Layers, Search, Filter,
} from "lucide-react";
...
// —— Admin visual system (match Atelier Pulse density & quiet luxury) ——
  ...
  return (
    <div className="mb-6 md:mb-8 flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
      <div>
        ...
        {subtitle && <p className="text-[13px] mt-1.5 max-w-xl" style={{ color: T.muted }}>{subtitle}</p>}
      </div>
...
function AdminShell({ admin, view, setView, onLogout, children }){
  ...
  return <div className="min-h-screen w-full flex" style={{backgroundColor:T.cream}}>
    ...
    <main className="flex-1 overflow-auto pt-14 md:pt-0" style={ADMIN_PAGE_BG}>{children}</main>
  </div>;
}
  ...
  return <div className="p-5 md:p-8" style={{background:"linear-gradient(180deg,#F7F3EB 0%,#EFE8DC 100%)",minHeight:"100%"}}>
    <div className="mb-6 p-4 md:p-5 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4"
      ...
      <div className="min-w-0">
        ...
        <h1 className="italic text-[24px] md:text-[28px] leading-tight" style={{fontFamily:"'Fraunces',serif",color:T.teal}}>Atelier Pulse</h1>
      </div>
...
function VariantEditor({ productId }){
  ...
  return <div>
    <div className="mb-5 p-3.5" style={{backgroundColor:"rgba(24,54,48,0.05)",borderRadius:RADIUS.xs}}>
      <p className="text-[12.5px] leading-[1.7]" style={{color:"rgba(36,62,65,0.85)"}}>
        <strong>How this works:</strong> if this product comes in different real colours, list them under "Colors" first. Then, below that, add one real row per colour (and per size, if it genuinely comes in more than one) with its own real price, stock status, and dimensions — <strong>type the real, final price here including GST</strong> (exactly what a customer pays), and the correct base price is worked out and stored automatically. This overrides the Basics tab price only once a real, distinct Size is set — pick "No size" for a colour-only row, and the Basics tab price correctly stays in charge for that one; only pick Small/Medium/Large if this specific product genuinely comes in more than one real size, and give each one its own real row here so the size picker shown to customers matches exactly what you set up.
      </p>
    </div>
    <div className="flex items-center justify-between mb-3">
          ...
          {variants.map(v=><div key={v.tempKey} className="p-3" style={{border:"1px solid rgba(36,62,65,0.14)",borderRadius:RADIUS.xs}}>
            <div className="flex items-center gap-2 flex-wrap mb-2.5">
              ...
              {/* REAL, DIRECT FIX: Size is now a proper dropdown limited
                  ...
                  option at all. */}
              <select value={v.size} onChange={e=>updateVariant(v.tempKey,"size",e.target.value)} className="text-[12.5px] px-2 py-1.5" style={{border:"1px solid rgba(36,62,65,0.18)",borderRadius:RADIUS.xs,color:T.teal}}>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <p className="text-[10.5px] tracking-[0.06em] uppercase shrink-0" style={{color:"rgba(36,62,65,0.6)"}}>Dimensions (optional)</p>
...
function AdminRoomStories(){
  ...
  return <div className="p-5 md:p-8 max-w-[900px]">
    <AdminPageHeader
      eyebrow="Catalog"
      title="Room stories"
      subtitle="Homepage boards under “Pieces that live together.” You choose the title, blurb, and exact products — nothing is auto-picked."
      actions={<><SweepButton onClick={addBoard}>Add board</SweepButton><SweepButton filled onClick={save} disabled={saving}>{saving?"Saving…":"Save"}</SweepButton></>}
    />
    ...
    <div className="flex flex-col gap-4">
      {boards.map(b=>(
        <div key={b.id} className="p-4 md:p-5" style={{backgroundColor:T.card,borderRadius:RADIUS.md,boxShadow:ELEVATION.raised,border:"1px solid rgba(24,54,48,0.07)"}}>
          <div className="flex flex-col md:flex-row gap-3 md:items-start mb-3">
            <div className="flex-1 flex flex-col gap-2">
              <input value={b.title} onChange={e=>updateBoard(b.id,{title:e.target.value})} placeholder="Board title"
                className="w-full text-[15px] px-3 py-2" style={{border:"1px solid rgba(36,62,65,0.2)",borderRadius:RADIUS.xs,color:T.teal,fontFamily:"'Fraunces',serif"}}/>
              <input value={b.blurb||""} onChange={e=>updateBoard(b.id,{blurb:e.target.value})} placeholder="Short blurb"
                className="w-full text-[13px] px-3 py-2" style={{border:"1px solid rgba(36,62,65,0.2)",borderRadius:RADIUS.xs,color:T.teal}}/>
            </div>
            <label className="flex items-center gap-2 text-[12.5px] shrink-0" style={{color:T.teal}}>
              <input type="checkbox" checked={b.active!==false} onChange={e=>updateBoard(b.id,{active:e.target.checked})}/> Active on homepage
            </label>
            <button type="button" onClick={()=>removeBoard(b.id)} style={{color:T.error}} title="Remove board"><Trash2 size={16}/></button>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {[0,1].map(slot=>(
              <div key={slot}>
                <p className="text-[11px] uppercase tracking-[0.08em] mb-1.5" style={{color:"rgba(36,62,65,0.55)"}}>Product {slot+1}</p>
                <select value={(b.productIds&&b.productIds[slot])||""} onChange={e=>setProductSlot(b.id,slot,e.target.value)}
                  ...
                  {products.map(pr=><option key={pr.id} value={pr.id}>{pr.name} ({pr.id})</option>)}
                </select>
                {(b.productIds&&b.productIds[slot]&&byId(b.productIds[slot]))&&(
                  <p className="text-[12px] mt-1" style={{color:"rgba(36,62,65,0.65)"}}>{byId(b.productIds[slot]).name}</p>
                )}
              </div>
            ))}
          </div>
...
function AdminOrders(){
  ...
  const updateStatus=async(orderNumber,status,pickupLocation)=>{
    ...
    const data=await res.json().catch(()=>({}));
    if(!res.ok){
      showToast(data.error||"Couldn't update that order's status.","error");
      return;
    }
    load();
  ...
  return <div className="p-5 md:p-8">
    ...
    <div className="flex flex-col gap-3">
      {orders.map(o=>{
        ...
        return <div key={o.orderNumber} className="flex flex-col gap-3 p-4 md:p-5" style={{backgroundColor:T.card,borderRadius:RADIUS.md,boxShadow:ELEVATION.raised,border:"1px solid rgba(24,54,48,0.07)"}}>
        <div className="flex flex-col md:flex-row md:items-center md:flex-wrap gap-3 md:gap-4">
          ...
          <div className="flex items-center justify-between md:contents gap-2">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {(o.paymentStatus==="paid"||o.paymentStatus==="partially_refunded")&&o.status!=="cancelled"&&(
                <button onClick={()=>openRefund(o)} className="text-[11px] uppercase tracking-[0.06em] underline" style={{color:T.warning}}>Partial refund</button>
              )}
          ...
          {o.courierTrackingUrl&&<a href={o.courierTrackingUrl} target="_blank" rel="noopener noreferrer" className="text-[11.5px] uppercase tracking-[0.06em]" style={{color:T.teal}}>Open courier →</a>}
        ...
        {(o.courierTrackingId||o.courierStatus||(o.courierEvents&&o.courierEvents.length>0))&&(
          <div className="text-[12.5px] p-3" style={{backgroundColor:"rgba(24,54,48,0.04)",borderRadius:8}}>
            {o.courierTrackingId&&<p style={{color:T.teal}}><span style={{color:"rgba(36,62,65,0.55)"}}>AWB / ID:</span> {o.courierTrackingId}</p>}
    ...
    <Modal open={!!refundTarget} onClose={()=>!refunding&&setRefundTarget(null)} title={refundTarget?`Refund #${refundTarget.orderNumber}`:"Refund"} confirmLabel={refunding?"Refunding…":"Issue refund"} onConfirm={submitPartialRefund} danger>
      <p className="text-[13px] mb-3" style={{color:T.muted}}>Order total ₹{Number(refundTarget?.total||0).toLocaleString("en-IN")} · already refunded ₹{Number(refundTarget?.amountRefunded||0).toLocaleString("en-IN")}. Razorpay will be charged for this amount only.</p>
      <InputField label="Amount (₹)" value={refundAmount} onChange={setRefundAmount}/>
      <div className="mt-3">
        <InputField label="Reason (optional)" value={refundReason} onChange={setRefundReason} placeholder="Damaged in transit / goodwill"/>
      </div>
    </Modal>
  </div>;
}
...
function AdminCms(){
  ...
  return <div className="p-5 md:p-8">
    <AdminPageHeader eyebrow="System" title="Site content" subtitle="Edit page copy that customers see — shipping, about, policies, and more." />
