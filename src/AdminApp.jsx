...
function AdminDashboard({ setView }){
  ...
  if(err||!data) return <EmptyState icon={AlertCircle} title="Couldn't load Atelier Pulse" message="Something went wrong reaching the server. Please refresh and try again." actionLabel="Retry" onAction={load}/>;
  ...
  const kpis=[
    {key:"rev",label:"Paid revenue",val:`₹${Number(data.totalRevenue||0).toLocaleString("en-IN")}`,d:data.revenueDeltaPct,tint:"rgba(229,198,144,0.2)",go:null},
    {key:"ord",label:"Paid orders",val:String(data.paidOrderCount||0),d:data.ordersDeltaPct,tint:"rgba(24,54,48,0.08)",go:()=>setView?.("orders")},
    {key:"aov",label:"AOV",val:`₹${Number(data.aov||0).toLocaleString("en-IN")}`,d:null,tint:"rgba(229,198,144,0.12)",go:null},
    {key:"prod",label:"In production",val:String(data.inProductionCount||0),d:null,tint:"rgba(181,101,29,0.12)",go:()=>openFunnel("production")},
    {key:"att",label:"Needs attention",val:String(data.attentionCount||0),d:null,tint:"rgba(168,59,50,0.1)",go:null},
    {key:"cust",label:"New customers",val:String(data.newCustomers||0),d:null,tint:"rgba(59,110,82,0.12)",go:()=>setView?.("customers")},
  ];
  const flowMax=Math.max(1,...(data.studioFlow||[]).map(s=>s.count));
  const catMax=Math.max(1,...(data.categoryMix||[]).map(c=>c.revenue));

  return <div className="p-5 md:p-8 max-w-[1400px]">
    <div className="flex flex-wrap items-end justify-between gap-4 mb-6">
      <div>
        <p className="text-[11px] tracking-[0.2em] uppercase mb-1" style={{color:"rgba(36,62,65,0.55)"}}>Analytics</p>
        <h1 className="italic text-[26px] md:text-[30px]" style={{fontFamily:"'Fraunces',serif",color:T.teal}}>Atelier Pulse</h1>
        <div className="w-10 h-px mt-2 mb-2" style={{backgroundColor:T.gold}}/>
        <p className="text-[13px]" style={{color:"rgba(36,62,65,0.7)"}}>Real orders, production flow, and catalog signals — no vanity traffic metrics.</p>
      </div>
      <div className="flex flex-wrap items-center gap-2">
        {["7d","30d","90d","all"].map(r=><button key={r} onClick={()=>setRange(r)}
          className="px-3 py-1.5 text-[11px] tracking-[0.08em] uppercase"
          style={{backgroundColor:range===r?T.teal:"transparent",color:range===r?"white":T.teal,border:`1px solid ${range===r?T.teal:"rgba(36,62,65,0.2)"}`,borderRadius:RADIUS.xs}}>
          {r==="all"?"All":r}
        </button>)}
        <button onClick={load} className="px-3 py-1.5 text-[11px] tracking-[0.08em] uppercase" style={{color:T.teal,border:"1px solid rgba(36,62,65,0.2)",borderRadius:RADIUS.xs}}>Refresh</button>
      </div>
    </div>

    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
      {kpis.map(k=><button key={k.key} type="button" onClick={()=>k.go?.()}
        className="text-left p-4 transition-shadow"
        style={{backgroundColor:T.card,borderRadius:RADIUS.md,boxShadow:ELEVATION.raised,cursor:k.go?"pointer":"default"}}>
        <div className="flex items-center gap-2 mb-2">
          <span className="w-7 h-7 flex items-center justify-center" style={{backgroundColor:k.tint,borderRadius:RADIUS.xs}}/>
          <p className="text-[11px] uppercase tracking-[0.06em]" style={{color:"rgba(36,62,65,0.65)"}}>{k.label}</p>
        </div>
        <p className="text-[22px] md:text-[24px] leading-none" style={{fontFamily:"'Fraunces',serif",color:T.teal}}>{k.val}</p>
        {k.d!=null&&<p className="text-[11px] mt-1.5" style={{color:k.d>=0?T.success:T.error}}>{delta(k.d)} vs prior</p>}
      </button>)}
    </div>

    <div className="grid grid-cols-1 lg:grid-cols-5 gap-5 mb-6">
      <div className="lg:col-span-3 p-5" style={{backgroundColor:T.card,borderRadius:RADIUS.md,boxShadow:ELEVATION.raised}}>
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
          <p className="text-[13px] uppercase tracking-[0.06em] font-medium" style={{color:T.teal}}>Revenue canvas</p>
          <div className="flex gap-1">
            {[["revenue","Revenue"],["orderCount","Orders"]].map(([m,l])=><button key={m} onClick={()=>setChartMode(m)}
              className="px-2.5 py-1 text-[11px] uppercase tracking-[0.06em]"
              style={{backgroundColor:chartMode===m?T.teal:"transparent",color:chartMode===m?"white":T.teal,borderRadius:RADIUS.xs,border:`1px solid ${chartMode===m?T.teal:"rgba(36,62,65,0.15)"}`}}>{l}</button>)}
          </div>
        </div>
        {chartData.length===0?<EmptyState icon={Package} title="No paid orders in range" message="Once orders are paid in this period, the chart fills in."/>:
        <ResponsiveContainer width="100%" height={260}>
          <LineChart data={chartData}>
            <CartesianGrid strokeDasharray="3 3" stroke="rgba(36,62,65,0.08)"/>
            <XAxis dataKey="day" tick={{fontSize:11,fill:"rgba(36,62,65,0.65)"}}/>
            <YAxis tick={{fontSize:11,fill:"rgba(36,62,65,0.65)"}}/>
            <Tooltip contentStyle={{background:T.cream,border:`1px solid rgba(36,62,65,0.12)`,borderRadius:8,fontSize:12}}/>
            <Line type="monotone" dataKey={chartMode} stroke={chartMode==="revenue"?T.gold:T.teal} strokeWidth={2.5} dot={false}/>
          </LineChart>
        </ResponsiveContainer>}
      </div>

      <div className="lg:col-span-2 p-5" style={{backgroundColor:T.card,borderRadius:RADIUS.md,boxShadow:ELEVATION.raised}}>
        <p className="text-[13px] uppercase tracking-[0.06em] mb-1 font-medium" style={{color:T.teal}}>Studio flow</p>
        <p className="text-[12px] mb-4" style={{color:"rgba(36,62,65,0.65)"}}>Paid & COD orders by stage — click a stage</p>
        <div className="flex flex-col gap-2.5">
          {(data.studioFlow||[]).map(s=>(
            <button key={s.status} type="button" onClick={()=>openFunnel(s.status)} className="text-left">
              <div className="flex justify-between text-[12px] mb-1">
                <span className="capitalize" style={{color:T.teal}}>{s.status}</span>
                <span style={{color:"rgba(36,62,65,0.7)"}}>{s.count}</span>
              </div>
              <div className="h-2 rounded-full overflow-hidden" style={{backgroundColor:"rgba(36,62,65,0.08)"}}>
                <div className="h-full rounded-full transition-all" style={{width:`${Math.round((s.count/flowMax)*100)}%`,backgroundColor:funnelStatus===s.status?T.gold:T.teal}}/>
              </div>
            </button>
          ))}
        </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-6">
      <div className="p-5" style={{backgroundColor:T.card,borderRadius:RADIUS.md,boxShadow:ELEVATION.raised}}>
        ...
        {(data.bestSellers||[]).length===0?<EmptyState icon={ShoppingCart} title="No paid orders yet" message="Top forms will appear here."/>:
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={data.bestSellers} layout="vertical" margin={{left:8,right:8}}>
            <CartesianGrid strokeDasharray="3 3" stroke="rgba(36,62,65,0.08)"/>
            <XAxis type="number" tick={{fontSize:11}}/>
            <YAxis type="category" dataKey="productName" width={100} tick={{fontSize:10}}/>
            <Tooltip contentStyle={{background:T.cream,border:`1px solid rgba(36,62,65,0.12)`,borderRadius:8,fontSize:12}}/>
            <Bar dataKey="unitsSold" fill={T.teal} radius={[0,4,4,0]}/>
          </BarChart>
        </ResponsiveContainer>}
      </div>
      <div className="p-5" style={{backgroundColor:T.card,borderRadius:RADIUS.md,boxShadow:ELEVATION.raised}}>
        <p className="text-[13px] uppercase tracking-[0.06em] mb-4 font-medium" style={{color:T.teal}}>Category mix</p>
        {(data.categoryMix||[]).length===0?<p className="text-[13.5px]" style={{color:"rgba(36,62,65,0.7)"}}>No category revenue in this range.</p>:
        <div className="flex flex-col gap-3">
          {data.categoryMix.map(c=>(
            <div key={c.category}>
              <div className="flex justify-between text-[12.5px] mb-1">
                <span style={{color:T.teal}}>{c.category}</span>
                <span style={{color:"rgba(36,62,65,0.7)"}}>₹{c.revenue.toLocaleString("en-IN")} · {c.unitsSold} u</span>
              </div>
              <div className="h-1.5 rounded-full overflow-hidden" style={{backgroundColor:"rgba(36,62,65,0.08)"}}>
                <div className="h-full rounded-full" style={{width:`${Math.round((c.revenue/catMax)*100)}%`,backgroundColor:T.gold}}/>
              </div>
            </div>
          ))}
        </div>}
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-6">
      <div className="p-5" style={{backgroundColor:T.card,borderRadius:RADIUS.md,boxShadow:ELEVATION.raised}}>
        <p className="text-[13px] uppercase tracking-[0.06em] mb-3 font-medium" style={{color:T.teal}}>Returns pending</p>
        {(data.pendingReturns||[]).length===0?<p className="text-[13px]" style={{color:"rgba(36,62,65,0.65)"}}>None pending.</p>:
        <div className="flex flex-col gap-2">{data.pendingReturns.map(r=><button key={r.id} type="button" onClick={()=>setView?.("returns")} className="flex justify-between text-[13px] py-1.5 text-left" style={{borderBottom:"1px solid rgba(36,62,65,0.06)"}}>
          <span style={{color:T.teal}}>#{r.orderNumber}</span><span className="truncate max-w-[50%]" style={{color:"rgba(36,62,65,0.7)"}}>{r.itemName}</span>
        </button>)}</div>}
      </div>
      <div className="p-5" style={{backgroundColor:T.card,borderRadius:RADIUS.md,boxShadow:ELEVATION.raised}}>
        <p className="text-[13px] uppercase tracking-[0.06em] mb-3 font-medium" style={{color:T.teal}}>Stock alerts</p>
        {(data.lowStock||[]).length===0?<p className="text-[13px]" style={{color:"rgba(36,62,65,0.65)"}}>Nothing low or sold out.</p>:
        <div className="flex flex-col gap-2">{data.lowStock.map(p=><button key={p.id} type="button" onClick={()=>setView?.("products")} className="flex justify-between text-[13px] py-1.5 text-left" style={{borderBottom:"1px solid rgba(36,62,65,0.06)"}}>
          <span style={{color:T.teal}}>{p.name}</span><Badge variant={p.status==="sold-out"?"error":"warning"}>{p.status}</Badge>
        </button>)}</div>}
      </div>
      <div className="p-5" style={{backgroundColor:T.card,borderRadius:RADIUS.md,boxShadow:ELEVATION.raised}}>
        <p className="text-[13px] uppercase tracking-[0.06em] mb-3 font-medium" style={{color:T.teal}}>Payment issues (14d)</p>
        {(data.failedPayments||[]).length===0?<p className="text-[13px]" style={{color:"rgba(36,62,65,0.65)"}}>No failed/pending recently.</p>:
        <div className="flex flex-col gap-2">{data.failedPayments.map(o=><button key={o.orderNumber} type="button" onClick={()=>setView?.("orders")} className="flex justify-between text-[13px] py-1.5 text-left" style={{borderBottom:"1px solid rgba(36,62,65,0.06)"}}>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
      <div className="p-5" style={{backgroundColor:T.card,borderRadius:RADIUS.md,boxShadow:ELEVATION.raised}}>
        <p className="text-[13px] uppercase tracking-[0.06em] mb-3 font-medium" style={{color:T.teal}}>Quiet forms</p>
        <p className="text-[12px] mb-3" style={{color:"rgba(36,62,65,0.65)"}}>In catalog, zero paid sales in range</p>
        {(data.quietForms||[]).length===0?<p className="text-[13px]" style={{color:"rgba(36,62,65,0.65)"}}>Every listed form sold at least once — or catalog empty.</p>:
        <div className="flex flex-col gap-2 max-h-[220px] overflow-y-auto">{data.quietForms.map(p=><div key={p.id} className="flex justify-between text-[13px] py-1.5" style={{borderBottom:"1px solid rgba(36,62,65,0.06)"}}>
          <span style={{color:T.teal}}>{p.name}</span><span style={{color:"rgba(36,62,65,0.55)"}}>{p.category}</span>
        </div>)}</div>}
      </div>
      <div className="p-5" style={{backgroundColor:T.card,borderRadius:RADIUS.md,boxShadow:ELEVATION.raised}}>
        <p className="text-[13px] uppercase tracking-[0.06em] mb-3 font-medium" style={{color:T.teal}}>Recent orders</p>
        {(data.recentOrders||[]).length===0?<p className="text-[13px]" style={{color:"rgba(36,62,65,0.65)"}}>No orders yet.</p>:
        <div className="flex flex-col gap-2">{data.recentOrders.map(o=><button key={o.orderNumber} type="button" onClick={()=>setView?.("orders")} className="flex justify-between items-center text-[13px] py-1.5 text-left" style={{borderBottom:"1px solid rgba(36,62,65,0.06)"}}>
          <span style={{color:T.teal}}>#{o.orderNumber}</span>
          <span className="flex items-center gap-2" style={{color:"rgba(36,62,65,0.75)"}}>₹{Number(o.total).toLocaleString("en-IN")}
            <Badge variant={o.paymentStatus==="paid"||o.paymentStatus==="cod"?"success":o.paymentStatus==="failed"?"error":"warning"}>{o.paymentStatus}</Badge>
          </span>
        </button>)}</div>}
      </div>
    </div>

    {funnelStatus&&<div className="fixed inset-0 z-50 flex justify-end" style={{backgroundColor:"rgba(36,62,65,0.45)"}} onClick={()=>setFunnelStatus(null)}>
      <div className="w-full max-w-md h-full overflow-y-auto p-6" style={{backgroundColor:T.cream}} onClick={e=>e.stopPropagation()}>
        <div className="flex items-center justify-between mb-5">
          <div>
            <p className="text-[11px] tracking-[0.14em] uppercase" style={{color:"rgba(36,62,65,0.55)"}}>Studio flow</p>
            <h2 className="italic text-[22px] capitalize" style={{fontFamily:"'Fraunces',serif",color:T.teal}}>{funnelStatus}</h2>
          </div>
          <button onClick={()=>setFunnelStatus(null)} aria-label="Close"><X size={20} style={{color:T.teal}}/></button>
        </div>
        {funnelLoading?<p className="text-[13px]" style={{color:"rgba(36,62,65,0.7)"}}>Loading…</p>:
        funnelOrders.length===0?<p className="text-[13px]" style={{color:"rgba(36,62,65,0.7)"}}>No orders in this stage.</p>:
        <div className="flex flex-col gap-2">{funnelOrders.map(o=><div key={o.orderNumber} className="p-3" style={{backgroundColor:T.card,boxShadow:ELEVATION.raised,borderRadius:RADIUS.sm}}>
          <div className="flex justify-between text-[13px] mb-1"><span style={{color:T.teal}}>#{o.orderNumber}</span><span>₹{Number(o.total).toLocaleString("en-IN")}</span></div>
          <p className="text-[12px] truncate" style={{color:"rgba(36,62,65,0.65)"}}>{o.email}</p>
        </div>)}</div>}
        <button type="button" onClick={()=>{setFunnelStatus(null);setView?.("orders");}} className="mt-6 w-full py-3 text-[12px] uppercase tracking-[0.1em]" style={{backgroundColor:T.teal,color:"white",borderRadius:RADIUS.sm}}>Open orders</button>
      </div>
    </div>}
  </div>;
}
