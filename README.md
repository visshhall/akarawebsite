...
**Still needed before the app is genuinely usable** (real, remaining work, not yet started): the actual `/android-app` marketing/download page the integration guide describes, and a real APK build via `eas build` to actually upload through this new pipeline.

**Files:** `server.js`, `server/auth.js`, `server/routes/auth.js`, `server/routes/admin/settings.js`, `db/schema.sql`, `src/AdminApp.jsx`
