...
│   ├── adminAuth.js           admin sessions — structurally separate from customer auth
...
│   ├── seed-admin.js                      creates/updates the one admin account (run directly, no HTTP endpoint)
│   └── routes/
│       ├── auth.js                 signup/login/logout/me (customer)
│       ├── products.js              public product list/detail
│       ├── orders.js                 checkout, payment verification, cancellation, order history
│       ├── addresses.js               saved address book
│       ├── returns.js                  return request submission
│       ├── reviews.js                   product review submission + summary
│       ├── wishlist.js                   saved-items list
│       ├── coupons.js                     public shipping-cost/coupon/maintenance-status lookup
...
├── index.html                 page shell — Google tag, fonts, meta tags, favicon
...
Both commands are idempotent — safe to re-run if something goes wrong
partway through.

## Setting up (or changing) the admin account
...
again with the same email updates the password rather than creating a
duplicate account — this is also how you change it later.
...
There is currently no drag-and-drop admin screen for this (see "Not yet
built" below) — the practical way to get real product photos live is to
send them directly to Claude in chat, named starting with the exact
product ID (e.g. `aether-pendant-lamp-1.jpg`). They get validated,
uploaded to permanent storage (Cloudflare R2), and attached to the
correct product from there.
...
customer reviews (not placeholder text), a wishlist, and a cart that
...
configurable handling fee, correctly taxed. A payment webhook exists
independently of the customer's own browser completing the flow, so a
payment that succeeds even if someone's connection drops still results
in a confirmed order and a real confirmation email/WhatsApp message.

**Customer account:** signup/login, saved addresses with PIN-code
auto-fill, order history with real-time status tracking, self-service
order cancellation (server-enforced 30-minute window), return requests
with photo upload, downloadable PDF invoices, and a real password-reset
flow (secure tokens, single-use, expiring).

**Admin panel** (`/admin`, structurally isolated from customer auth —
separate database table, separate signing secret, separate 12-hour
session): a real dashboard with live revenue/order charts, full product
management, order management with an enforced status lifecycle
(confirmed → production → qc → dispatched → delivered, with cancellation
allowed at any pre-delivery point, and one legitimate backward step —
qc back to production — for genuine quality-check failures), a
maintenance-mode toggle, shipping cost and COD settings, an admin action
audit log, and a customer list.
...
- **A real admin media gallery.** Product photo/video upload currently
  happens by sending files directly in chat (see above), not through an
  in-app screen. There's also no way yet to reorder photos, pick a "main"
  image, or tag a photo to a specific size/color variant — the first
  photo sent for a product becomes its main image by default.
- **Size/color variants with independent pricing and stock.** A single
  product can't yet have, say, a Small and a Large with different prices
  and separately tracked stock.
- **No 2FA on the admin account.**
- **Homepage featured-product selection** is still the first 3 products
  in catalog order, not an admin-curated pick.
- **FAQ content** is still hardcoded in the frontend, not admin-editable.
