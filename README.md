...
## 2026-09-24-lookbook-anniv-v106 — Anti-sale · Anniversary · Designer lookbook
- **Anti-sale posture:** Footer line — “We do not run flash sales. Price is the price.”
- **Piece anniversary:** Scheduler emails ~365 days after delivery (+ logged-in soft banner). Column `orders.anniversary_email_sent_at`. Run `npm run db:sync`.
- **Designer lookbook:** `/lookbook` password gate. Admin → Settings → 08 · Designer lookbook (password + optional PDF URL). Live product specs after unlock.

# ĀKĀRA — website project

**Last updated: 24 September 2026** (SEO noindex fix, header English-only, perf fonts)  
**Latest SW version:** `2026-09-24-seo-header-perf-v102`  
**Production:** [www.akaraonline.co.in](https://www.akaraonline.co.in)

Stack: Node/Express + React (Vite) + Postgres (Railway) + Cloudflare + R2 + Razorpay + Shiprocket + Resend + Gupshup (optional)

| Document | Purpose |
|----------|---------|
| `AKARA_PROJECT_DOCUMENTATION.md` | Architecture, decisions, full narrative |
| `AKARA_Master_Pending_List.md` | Remaining product / ops work |
| `AKARA_MARKETING_ADS.md` | Google Ads / GA / Merchant notes |
| `DEPLOY_VERIFY.md` | Deploy smoke checks |

---

## Clean deploy tree (15 Sep 2026)

Website code lives only in this folder. Non-deploy material (old zips, mobile experiments, ops QA notes, split SQL snippets) was moved to sibling folder:

`../akara-not-website/`

Use `npm run db:sync` against `db/schema.sql` — do not rely on root-level `*.sql` patches (archived).
...
### Partial refund UX polish (`v101`)

**Why:** Finance needs clearer controls than amount + free-text reason alone.

**What:**
- Order / already refunded / still refundable summary cards.
- Payment method + status line; COD called out as offline.
- Amount validation vs remaining; live “after this refund” balance.
- Quick **25% / 50% / 75% / 100%** of remaining.
- Reason presets (damage, partial return, goodwill, pricing) + optional note.
- Confirm button shows the exact ₹ amount.
- Client blocks over-refund before API call (server still authoritative via `refundOrderPartial`).

**Files:** `src/AdminApp.jsx` · API unchanged: `POST /api/admin/orders/:orderNumber/refund`

### Database sync architecture documented

See **`docs/DATABASE_SYNC.md`**: `npm run db:sync` → `schema.sql` + coupon column ensures; never rewrites CMS; must use `pool.query`.

### Weekly documentation process

See **`docs/WEEKLY_DOC_UPDATE.md`** — bump README SW, changelog, pending list, architecture only when structure changes.

## Changelog — 19–22 September 2026

### 22 Sep 2026 — `db:sync` fix (`v100`)

**Bug:** `npm run db:sync` failed with `query is not defined` after schema ran.

**Cause:** Extra `ALTER TABLE coupons …` used bare `query()` while only `pool` is imported from `server/db.js`.

**Fix:** `server/db-sync.js` uses `pool.query` only. Still ensures coupon columns: `first_order_only`, `max_discount_amount`, `show_on_banner`, `show_on_checkout`. CMS pages are **not** overwritten.

**Deploy:** Railway shell → `npm run db:sync`

---

### 22 Sep 2026 — Header Hindi size (`v99`)

**What:** `(आकार)` next to the wordmark increased to `clamp(1.7rem, 5.2vw, 2.2rem)`, weight 600, slight optical align. Devanagari reads smaller than Latin at the same px.

**File:** `src/AkaraApp.jsx` (Header)

---

### 22 Sep 2026 — Sold-out status + auto slug (`v98`)

**Bug 1 — Sold out still “in stock”:** Choosing Status → sold-out while **Studio capacity** still held a positive number forced save back to `in-stock`. Shop badges also preferred positive `stock_qty` over status.

**Fix:**
- Explicit status `sold-out` / `draft` / `hidden` / `pre-order` never overridden by capacity on save.
- Selecting sold-out sets capacity to **0** (admin UI + server PUT).
- Customer `cardStatus`: sold-out / draft / hidden / pre-order win over stock qty.

**Bug 2 — Product ID not following name:** On **new** products, name still auto-fills URL slug until the admin edits the ID field (`idManual`). Existing product IDs stay fixed (links).

**Files:** `src/AdminApp.jsx`, `src/AkaraApp.jsx`, `server/routes/admin/products.js`

---

### 22 Sep 2026 — Coupons placement + single use + horizontal admin (`v97` area)

**What:**
- Placement flags: `show_on_banner`, `show_on_checkout` — banner off does not hide checkout offers.
- Checkout **Available offers** card when coupons are active for checkout.
- One coupon per order (apply replaces previous).
- **Upto** discounts via `max_discount_amount` with percent type.
- Admin Settings coupon UI horizontal / `items-start` so Dispatch & Password cards no longer stretch with coupon form.
- Misuse guards: identity (email + phone + `deleted_customer_archives`), `one_per_customer`, `first_order_only`, max total uses, expiry.

**Files:** `server/settings.js`, `server/routes/coupons.js`, `server/routes/admin/settings.js`, `src/AdminApp.jsx`, `src/AkaraApp.jsx`, `db/schema.sql`

---

### 22 Sep 2026 — Tax invoice PDF redesign

**What:** Customer invoice PDF aligned to professional tax-invoice layout (header band, bill/ship to, HSN table, CGST/SGST, paid stamp, bank/declaration). Noto font embedding for brand glyphs.

**File:** `src/AkaraApp.jsx` (`downloadInvoicePDF`), related PDF helpers

---

### 21–22 Sep 2026 — Category filter + related products + audits

**What:**
- `normalizeCategoryName` / `categoriesMatch` so lamps do not appear under Planters from alias mismatches.
- Related products: prefer same category, fill at least one from other categories (flexible, not hard-coded 3).
- Automated category audit heuristics in admin products route.
- Admin product **category** is a dropdown from Catalog → Categories (not free-text only).

**Files:** `src/AkaraApp.jsx`, `server/routes/admin/products.js`, `src/AdminApp.jsx`

---

### 20–21 Sep 2026 — Shop UX, header, lead-time copy

**What:**
- Luxury filter sidebar (desktop sticky); mobile stacked; teal high-contrast labels.
- Sticky header search inside bar; wordmark image + Devanagari; install icon removed from header density.
- Site-wide lead time: **2–3 days studio / ~2 weeks to door** (replaced old “2–3 weeks” copy).
- Homepage storytelling band rewritten; PDP duplicate description removed (keep under media tabs).
- Magazine hero sizing; shop grid Art placeholders; spacing rhythm; sort + piece count.
- Welcome coupon one-time (identity), not “first order only if used on first try”.

---

### 19–20 Sep 2026 — Commerce integrity (recap)

- Server validation (`server/validate.js`) on all name/address/checkout fields.
- Low stock “N remaining”; max **min(5, stock_qty)** per product per order.
- Account delete → `deleted_customer_archives` (compliance retention).
- Cart `ON CONFLICT` / transaction to stop duplicate cart_items key errors.
- Admin product list stock-capacity warning banner (JSX ternary fixed for build).
- Journey rail, cancel window **10 min**, batch sync, Shiprocket AWB path (ops).

---

### Deploy checklist (current)

1. Deploy this tree (Git / Railway). Build must pass `scripts/verify-sw-version.mjs`.
2. Railway: `npm run db:sync` (schema + coupon columns; **does not** rewrite CMS).
3. Confirm live `/sw.js` shows `2026-09-22-dbsync-query-v100` (or later).
4. Smoke: set product **sold out** + save → shop shows sold out; new product name auto-slugs; apply one coupon at checkout; invoice PDF on paid order.

### Intentionally separate

- Mobile APK / Expo — other chat / package
- Google Ads / Merchant full go-live — `AKARA_MARKETING_ADS.md`
- Device fingerprinting — only if abuse appears

## Changelog — 18–19 September 2026

### 19 Sep 2026 — server-side field validation + Google button (`v77`)

**Why:** Client-only checks on profile/name fields could be bypassed (e.g. Burp Suite). Special characters were accepted server-side.

**What / how:**
- Expanded `server/validate.js`: `validPersonName`, `cleanPersonName`, `validAddressLine`, `validCityOrState`, `validMessage`, `validPin` (Unicode letters allowed; blocks `@ $ <` etc.).
- Wired into: profile `PUT`, signup name, addresses, checkout shipping address, contact, bulk enquiry, reviews comment.
- **Google Sign-In / Link:** removed brand cream/teal frame behind the GIS button — native Google button only (`src/AkaraApp.jsx`, `src/index.css`).

**Connected to:** `server/routes/auth.js`, `addresses.js`, `orders.js`, `contact.js`, `bulk-orders.js`, `reviews.js`.

**Auth SQL:** All customer auth/profile queries remain **parameterised** (`$1`, `$2`, …). Validation is content rules; parameterisation already blocked SQL injection.

---

### 19 Sep 2026 — low-stock “N remaining” + order qty cap (`v76`)

**What:**
- When admin sets `stock_qty`: **1–10** → customer sees **“N remaining”** on cards + PDP; **11–20** → “Low stock”; **0** → sold out.
- **Max 5 units per product per order** (atelier limit), or lower if `stock_qty` is smaller.
- Enforced client (cart steppers, rehydrate) **and** server (`priceCartServerSide` / checkout).

**Connected to:** `stock_qty` on `products`, `StockBadge`, `MAX_UNITS_PER_PRODUCT` in `AkaraApp.jsx` + `orders.js`.

---

### 18–19 Sep 2026 — ops polish (summary)

| Area | Change |
|------|--------|
| Coupons | Identity checks (email/phone + `deleted_customer_archives`); first-order / one-per-customer cannot be farmed by delete+re-register |
| Account delete | Snapshot archived with `retain_until` (compliance) before customer row removal |
| Shop filters | URL sync (`replaceState`); price range min ₹100, max = live catalog ceiling rounded to next ₹1000 |
| Wishlist | Guest merge on login; clear on logout |
| Auth header | `authChecked` guard — no sign-in flash on refresh |
| Order journey | Single tracking card; cancel window **10 min**; vertical rail on mobile |
| Dashboard | Live Studio risk / Need attention counts; break-even; profit uses variant-aware pricing |
| Email / PDF | Noto font for Care PDF; fewer status emails; tracking link fixes |
| PWA / SW | Version lock enforced on every build |

---

## Code comment standard (required for every change)

Every non-trivial block must answer **what / why / how / connected to**:

1. **What** — what this function, route, or UI block does  
2. **Why** — product or bug reason (not “cleanup”)  
3. **How** — key mechanism (e.g. delete-then-reinsert, SoftBoundary, SW version lock)  
4. **Connected to** — related routes, tables, admin screens, or customer pages  

Prefer short block comments above functions and section banners (`// ====`).  
Do **not** comment every line. Do **not** leave dead code without a why.  
New features in `AkaraApp.jsx` / `AdminApp.jsx` / `server/routes/**` must include this before merge.

---

## Changelog — 16–17 September 2026

### 17 Sep 2026 — colour vs finish + per-colour media (`v43`)

- **PDP:** Removed duplicate Finish swatch chips; one **Colour** control (swatch + name pills). Finish is optional text note only.
- **Admin:** Finish swatches → **Finish note**; **Details → Colors** can attach optional photos per colour.
- **DB:** `product_colors.media` JSONB (empty = use product Media gallery).
- **Connected to:** VariantEditor, ProductDetailView gallery, `PUT …/variants`.



### 17 Sep 2026 — home spacing, variants clear, PDP hooks (`v37` → `v41`)

**Homepage**
- Tightened section padding (`py-20/28` → `py-12/16`) and title margins so mid-page cream gaps are less sparse
- Section rules (STORIES / STUDIO) less vertical air
- **Connected to:** `HomeView` in `src/AkaraApp.jsx` only (not CMS page shells)

**Product variants (admin)**
- **Root cause:** `PUT /api/admin/products/:id/variants` only deleted `product_colors`; size-only rows (`color_id NULL`) survived CASCADE and reappeared after “clear”
- **Fix:** `DELETE FROM product_variants` then colours, then re-insert (including empty list)
- UI: **Clear all in database** button; save response returns live counts
- **Connected to:** admin Details tab; customer PDP size/colour pickers; checkout validation

**PDP SoftBoundary / React #310**
- Hooks (`useState` waitlist, colour/size effects, `view_item`) ran **after** early returns when catalogue finished loading → “This section failed to load”
- **Fix:** all hooks before any `return` in `ProductDetailView`
- Gallery/media/dims hardened against empty media and null dims
- **Connected to:** `/product/:id`, SoftBoundary, analytics `view_item`

**Room stories admin**
- Empty board list no longer re-seeds four defaults on next GET (same “came back” class of bug)
- **Connected to:** `GET/PUT` room-stories settings; homepage Room Stories section

**Deploy note**
- Confirm live SW: `curl -s https://www.akaraonline.co.in/sw.js | head -8` must show `2026-09-17-home-tight-v41` (or newer). Older SW means Railway is not on this tree — variant clear will look “broken” until server routes match.

**robots.txt**
- Disallow `/api/`, `/webhooks/`, `/apk` in addition to private app paths
- Sitemap unchanged: `https://www.akaraonline.co.in/sitemap.xml` (dynamic in `server.js`)

**Primary files:** `src/AkaraApp.jsx`, `src/AdminApp.jsx`, `server/routes/admin/products.js`, `server/routes/admin/settings.js`, `server/routes/products.js`, `public/robots.txt`, `public/sw.js`, `server.js`

### 16 Sep 2026 — margin panel, deps, analytics, search

- Admin product Basics: target **30%** / min **25%** margin helper from cost fields
- Search ranking tightened (name/category priority); storefront link from product editor
- Analytics rate limit isolated; `analytics_events` idempotent schema
- Scroll-to-top FAB threshold/z-index; contact card layout
- SW version lock + `verify-sw-version.mjs` discipline
...
```bash
npm run db:sync    # schema + CMS FAQ/refund blocks (idempotent)
npm run build      # SW lock + vite build (Railway usually runs this)
```

1. Confirm SW: `curl -s https://www.akaraonline.co.in/sw.js | head -8` → expect `2026-09-17-home-tight-v41` (or newer)
2. Hard refresh once if UI looks stale
3. Optional: set `ERROR_WEBHOOK_URL` (HTTPS Slack/Discord) for client error alerts

---

## Changelog — 14–15 September 2026

### 15 Sep 2026 — customer gaps + monitoring (`v18` → `v20`)

**Error monitoring**
- `POST /api/client-errors` (rate-limited); logs `[client-error]` on Railway
- Optional `ERROR_WEBHOOK_URL` forward
- Browser: `window.onerror`, `unhandledrejection`
- React: `ErrorBoundary` + `SoftBoundary` call `reportClientError` (`src/shared.jsx`)

**Image pipeline**
- Product uploads auto-framed to **4∶5 (1600×2000)** on cream (`server/upload.js`)
- Admin upload copy updated; re-upload old R2 assets to reframe

**Customer structural gaps**
- Product cards / featured: `object-contain` + 4∶5 frames; `sizes` hints
- Search: category + availability filters; empty state with category chips
- PDP: trust panel (lead time, dispatch, Razorpay, returns, Mumbai studio)
- Shop category intro: made-to-order honesty line
- Account → Payment methods: Cards / UPI / Net banking / Wallets explainer
- Track order: guest form + email deep-link copy
- Cart drawer empty state; mobile drawer **Studio / Help** groups
- Multi-tab cart & wishlist: `storage` + `BroadcastChannel` (tab id, no echo loop)
- LegalShell: clearer teal labels / hierarchy

**Files (primary):** `src/AkaraApp.jsx`, `src/shared.jsx`, `server/upload.js`, `server/routes/client-errors.js`, `server.js`, `public/sw.js`

### 14 Sep 2026 — admin UX, addresses, auth (`v14` → `v17`)

- Admin: atelier panels (radius restored), teal labels (readable on cream), order **Show details** collapse, Behaviour empty-state copy, cost/profit note on dashboard
- **Auto batch** `AK-YYYYMM-NNN` on new COD/paid orders (`server/routes/orders.js`)
- Addresses: optional **GSTIN**; bulk enquiry note under form/list
- Phone-change OTP: always **email fallback** when WhatsApp fails (Google accounts)
- Fixed **Add address** crash (`editingAddrId` missing `useState`)
- Floating **scroll-to-top** control
- Product list admin crash (`pi` undefined in map)

**Files (primary):** `src/AdminApp.jsx`, `src/AkaraApp.jsx`, `server/routes/addresses.js`, `server/routes/auth.js`, `server/routes/orders.js`, `db/schema.sql` (gstin)

### 14 Sep 2026 — CMS / waitlist / batches (`v14`)

- Homepage sections editable via CMS JSON keys
- Waitlist restock emails; production batches; stock decrement paths
- See earlier entries below for detail

---

## Historical notes (kept)

## 2026-09-14 CMS depth + waitlist restock + admin ledger (v14)
- Homepage trust / category / room / why / featured / CTA copy → Site content → **Homepage Sections** (JSON paragraph or heading+paragraph keys)
- Hero still: Homepage Hero + Catalog → Homepage hero image
- Restock: product leaves sold-out → emails waitlist (once per email), sets notified_at
- Admin Products / Orders: atelier chips, numbered ledger rows
- After deploy: `npm run db:sync` if waitlist table missing

## 2026-09-13 ops sync (v11)
- Production batches auto-created when order → Production
- Batch on invoice + packing slip
- Waitlist (sold-out notify) + admin list
- Stock qty decrements on paid/COD order
- Post-delivery care email includes review CTA
- CMS: home_sections, contact, bulk-orders pages
- After deploy: `npm run db:sync`

## Production batches (AK-YYYYMM-NNN)

- Admin → **Batches**: create print runs, assign to order numbers
- DB: `production_batches`; order `items[]` may include `batchNumber`
- After deploy: `npm run db:sync` (adds table)
...
## Changelog since clean zip upload (through 11 Sep 2026)

Full narrative: **`AKARA_PROJECT_DOCUMENTATION.md`**.

### 11 Sep 2026 (latest)

- **White-screen fix:** removed broken Vite `manualChunks` (React `useState` undefined)
- **Login fix:** import `TurnstileWidget` in `AkaraApp.jsx`
- **WhatsApp signup OTP:** stronger Gupshup payload + **always email OTP if WhatsApp fails**
- **Stock accuracy:** cart re-sync from catalogue; block checkout on sold-out lines
- **Invoice / packing:** collector detail + studio-made footer on tax invoice PDF
- **Care PDF** on order confirmation email (`server/careCardPdf.js`)
- **Trade page** repositioned as Interior projects & studios (MOQ / 48h)
- **Error boundaries:** SoftBoundary per route + root recovery; CSRF retry on 403
- **Gallery:** 4:5 + `object-contain`; hero `fetchPriority="high"`
- **Build:** safe Vite config (no aggressive React split); hashed assets long-cache

### 9–10 Sep 2026

- `db:sync`, SW versioning, Turnstile (customer + admin), CSP for GA4/Ads
- Shiprocket webhook auth, AWB on tracking, returns delivered-only
- Google link, PWA push foundation, admin dashboard / room stories
- Studio honesty CMS (About/Craft), account UI polish
- Security audit notes in earlier README sections below

### Do not reintroduce

- Custom `manualChunks` that isolate `react` / `react-dom` into separate vendor bags
- Using `TurnstileWidget` without importing it from `shared.jsx`
- Serving SW with long Cloudflare cache without version bump
...
## Security fixes (12 Sep 2026)
- F-1: `POST /api/auth/reset-password` rate-limited with `otpVerifyRateLimit`.
- F-2: Android APK upload validates ZIP/APK magic bytes via `file-type`, not only `.apk` filename.
- Room stories cards: teal panel background (not cream).
