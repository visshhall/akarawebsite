...
## Files changed in this pass (Security sweep + Admin Panel)
...
**Modified files:**
- `db/schema.sql` — admins/change_log tables, phone UNIQUE constraint,
  orders.discount/coupon_code columns (all with safe migrations for an
  already-live database)
- `server.js` — `trust proxy`, admin routes wired in
- `server/auth.js` — uses shared `IS_PRODUCTION`, phone dedup on signup
- `server/adminAuth.js` — uses shared `IS_PRODUCTION`
- `server/csrf.js` — uses shared `IS_PRODUCTION`
- `server/validate.js` — added `normalizePhone`/`validIndianPhone`
- `server/routes/orders.js` — coupon support in real checkout pricing
- `src/AkaraApp.jsx` — phone normalization fix, coupon wired through
  Cart → Checkout → real API, Invoice shows real discount
...
- `DATABASE_URL`, `JWT_SECRET`, `CSRF_SECRET`, `RAZORPAY_KEY_ID`,
  ...
  all from earlier passes, unchanged this pass.
