## Security audit fixes (9 Sep 2026)

**Critical**
- Shiprocket webhook: **token required** (no empty-header bypass); timing-safe compare
- Webhook rate limit: 120 req/min on Razorpay + courier endpoints

**Hardening**
- Guest order track endpoint rate-limited
- HTTP TRACE/TRACK methods → 405
- Error handler already omits stacks in production responses

**Already solid (verified, no change)**
- Razorpay payment + webhook HMAC verification (timing-safe)
- CSRF on cookie-auth state-changing `/api/*`
- Admin vs customer JWT secrets separated
- Login rate limits; bcrypt passwords
- Push endpoint allowlist; CSP headers; `x-powered-by` off
- Parameterized SQL; SEO HTML escaped

**Ops:** Keep `SHIPROCKET_WEBHOOK_TOKEN` set in Railway and matching Shiprocket dashboard. After deploy, Shiprocket must send `x-api-key`.

---

## Live structural gap fixes (audit 9 Sep 2026)

**Date:** 9 September 2026

- Analytics route confirmed mounted **before** CSRF (`/api/analytics/event`) — redeploy required if live still 404s
- Product cards: stronger sold-out detection (all size variants); no bag CTA when sold out; lead time line
- Checkout: empty bag shows explore CTA (hooks-safe)
- Maintenance: non-blocking bottom banner + localStorage dismiss
- Shop category chips: horizontal scroll on small screens
- Explicit `/robots.txt` route + cleaned robots file

---

## AWB assign hardening + admin layout fixes (Room Stories, shell, orders)

**Date:** 9 September 2026

- Shiprocket: robust `assignAwbForShipment` (multiple body shapes + shipment details fallback); broader AWB extraction; logging after create.
- Admin Room Stories: clear board cards, labeled fields, Remove + Active, Save/Add in header and footer.
- AdminPageHeader + main shell: no odd button placement / horizontal overflow; order cards avoid `md:contents` overlap.
