...
async function runMigrate() {
  ...
  await query(`ALTER TABLE coupons ADD COLUMN IF NOT EXISTS first_order_only BOOLEAN DEFAULT false`);
  await query(`ALTER TABLE coupons ADD COLUMN IF NOT EXISTS max_discount_amount INTEGER`);
  await query(`ALTER TABLE coupons ADD COLUMN IF NOT EXISTS show_on_checkout BOOLEAN DEFAULT true`);
  console.log("[db:sync] coupons.first_order_only ensured");
}
