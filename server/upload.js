...
// Product frames on the storefront are 4∶5. Every product image is
// normalised to that ratio on upload so grids never show half-cropped
// sides or empty bands from mismatched source photos. Non-product uploads
// (returns, etc.) still pass through the same helper when processUpload
// is used — consistent framing is fine for those too.
const PRODUCT_FRAME_W = 1600;
const PRODUCT_FRAME_H = 2000; // 4:5
const FRAME_BG = { r: 247, g: 243, b: 235 }; // T.cream

async function stripImageMetadata(buffer, mime) {
  const image = sharp(buffer, { failOn: "error" }).rotate(); // EXIF auto-orient before strip
  const meta = await image.metadata();
  const srcW = meta.width || PRODUCT_FRAME_W;
  const srcH = meta.height || PRODUCT_FRAME_H;

  // Fit inside 4:5 canvas, then extend with cream so the stored file is
  // always exactly 4:5 (or smaller if source is tiny — withoutEnlargement
  // on the fit step, then pad to full frame only when we target fixed size).
  const fitted = await sharp(buffer, { failOn: "error" })
    .rotate()
    .resize({
      width: PRODUCT_FRAME_W,
      height: PRODUCT_FRAME_H,
      fit: "contain",
      background: FRAME_BG,
      withoutEnlargement: false, // upscale slightly if needed so small phone shots still fill frame cleanly after pad
    })
    .toBuffer();

  // Second pass guarantees exact canvas size (sharp contain can leave 1px oddities on some inputs)
  const framed = sharp(fitted).resize({
    width: PRODUCT_FRAME_W,
    height: PRODUCT_FRAME_H,
    fit: "contain",
    background: FRAME_BG,
  ...
  if (mime === "image/png") return { buffer: await framed.png({ compressionLevel: 8 }).toBuffer(), mime: "image/png", ext: "png", width: PRODUCT_FRAME_W, height: PRODUCT_FRAME_H, sourceWidth: srcW, sourceHeight: srcH };
  if (mime === "image/webp") return { buffer: await framed.webp({ quality: 90 }).toBuffer(), mime: "image/webp", ext: "webp", width: PRODUCT_FRAME_W, height: PRODUCT_FRAME_H, sourceWidth: srcW, sourceHeight: srcH };
  return { buffer: await framed.jpeg({ quality: 90, mozjpeg: true }).toBuffer(), mime: "image/jpeg", ext: "jpg", width: PRODUCT_FRAME_W, height: PRODUCT_FRAME_H, sourceWidth: srcW, sourceHeight: srcH };
}
