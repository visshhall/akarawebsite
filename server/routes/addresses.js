...
import { sanitize, normalizePhone, validIndianPhone, validEmail, validPersonName, cleanPersonName, validAddressLine, cleanAddressLine, validCityOrState, cleanCityOrState, validPin } from "../validate.js";
...
router.post("/", requireAuth, asyncHandler(async (req, res) => {
  ...
  const { rows } = await query(
    ...
    [
      ...
      cleanAddressLine(line),
      cleanCityOrState(city),
      state && String(state).trim() ? cleanCityOrState(state) : "",
      String(pin).trim().slice(0, 6),
      normalizePhone(phone),
      sanitize(landmark || "", 150),
      gst,
      em,
    ]
  );
...
function validateAddressFields({ name, line, city, state, pin, phone, gstin, email }) {
  if (!validPersonName(name)) return "Enter a valid name (letters only — no special characters).";
  if (!validAddressLine(line)) return "Enter a valid address (at least 5 characters, no < >).";
  if (!validCityOrState(city)) return "Enter a valid city name.";
  if (state && String(state).trim() && !validCityOrState(state)) return "Enter a valid state name.";
  if (!validPin(pin)) return "A valid 6-digit PIN code is required.";
