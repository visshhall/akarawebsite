...
router.post("/", requireAuth, asyncHandler(async (req, res) => {
  const { name, line, city, state, pin, phone, landmark } = req.body || {};
  const error = validateAddressFields({ name, line, city, pin, phone });
  if (error) return res.status(400).json({ error });

  const { rows } = await query(
    `INSERT INTO addresses (customer_id, name, line, city, state, pin, phone, landmark)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`,
    [req.customer.id, sanitize(name).slice(0,200), sanitize(line).slice(0,300), sanitize(city).slice(0,100), sanitize(state||"").slice(0,100), sanitize(pin).slice(0,6), normalizePhone(phone), sanitize(landmark||"").slice(0,150)]
  );
...
router.put("/:id", requireAuth, asyncHandler(async (req, res) => {
  ...
  const { rows } = await query(
    `UPDATE addresses SET name=$1, line=$2, city=$3, state=$4, pin=$5, phone=$6, landmark=$7
     WHERE id=$8 AND customer_id=$9 RETURNING *`,
    [sanitize(name).slice(0,200), sanitize(line).slice(0,300), sanitize(city).slice(0,100), sanitize(state||"").slice(0,100), sanitize(pin).slice(0,6), normalizePhone(phone), sanitize(landmark||"").slice(0,150), req.params.id, req.customer.id]
  );
...
function toFrontendAddress(row) {
  return { id: row.id, name: row.name, line: row.line, city: row.city, state: row.state, pin: row.pin, phone: row.phone, landmark: row.landmark };
}
