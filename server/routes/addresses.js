...
router.post("/", requireAuth, asyncHandler(async (req, res) => {
  const { name, line, city, state, pin, phone, landmark, gstin } = req.body || {};
  const error = validateAddressFields({ name, line, city, pin, phone, gstin });
  if (error) return res.status(400).json({ error });

  const gst = cleanGstin(gstin) || null;
  const { rows } = await query(
    `INSERT INTO addresses (customer_id, name, line, city, state, pin, phone, landmark, gstin)
...
router.put("/:id", requireAuth, asyncHandler(async (req, res) => {
  ...
  const gst = cleanGstin(gstin) || null;
  const { rows } = await query(
    `UPDATE addresses SET name=$1, line=$2, city=$3, state=$4, pin=$5, phone=$6, landmark=$7, gstin=$8
...
function validateAddressFields({ name, line, city, pin, phone, gstin }) {
