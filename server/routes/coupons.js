import { Router } from "express";
import { attachCustomer } from "../auth.js";
import { asyncHandler } from "../asyncHandler.js";
import { lookupCoupon, getShippingSettings, getMaintenanceSettings, getFeaturedCoupons, getCheckoutCoupons } from "../settings.js";
import rateLimit from "express-rate-limit";

const couponValidateRateLimit = rateLimit({
  windowMs: 15 * 60 * 1000,
  max: 40,
  standardHeaders: true,
  legacyHeaders: false,
  message: { error: "Too many coupon checks. Please try again later." },
});

const router = Router();

// GET /api/coupons/shipping — the live shipping cost + free-shipping
// threshold, read-only and public. Lets Cart/Checkout show an accurate
// preview total instead of hardcoded numbers that could silently drift
// from what an admin has actually set — the same reasoning as the coupon
// validation route below, just for the other two values that used to be
// hardcoded constants.
router.get("/shipping", asyncHandler(async (req, res) => {
  const settings = await getShippingSettings();
  res.json(settings);
}));

// GET /api/coupons/maintenance — whether the "still under construction"
// notice should show, and what it says. Public and read-only for the
// same reason /shipping is — a visitor sees this before ever logging in.
router.get("/maintenance", asyncHandler(async (req, res) => {
  const settings = await getMaintenanceSettings();
  res.json(settings);
}));

// GET /api/coupons/featured — real, deliberately narrow public listing
// (see server/settings.js's own comment on getFeaturedCoupons for the
// full reasoning). Genuinely public/read-only, same as /shipping and
// /maintenance above — a visitor should see an active promotion before
// ever creating an account.
router.get("/featured", asyncHandler(async (req, res) => {
  const coupons = await getFeaturedCoupons();
  res.json({ coupons });
}));

// Cart + checkout offer cards (not the top banner)
router.get("/checkout-offers", asyncHandler(async (req, res) => {
  const coupons = await getCheckoutCoupons();
  res.json({ coupons });
}));


// GET /api/coupons/validate/:code — the ONLY way the frontend ever learns
// a coupon's real discount percentage now that coupons are admin-managed
// (previously the frontend just hardcoded "AKARA10 = 10%", which broke the
// moment that could change). This calls the exact same lookupCoupon()
// function the real checkout pricing uses (server/settings.js) — the
// preview shown on the Cart page and what actually gets charged at
// checkout can never disagree, since they're the same lookup.
//
// Deliberately public (no login required) and read-only — this only
// reveals whether ONE specific, guessed code is currently active and its
// discount percentage, the same information a legitimate promotional code
// is meant to be used with anyway. It does not list other coupons or
// reveal anything about codes not specifically asked about.
router.get("/validate/:code", couponValidateRateLimit, attachCustomer, asyncHandler(async (req, res) => {
  const raw = String(req.params.code || "").trim().slice(0, 40);
  if (!raw) return res.json({ valid: false });
  // Coupons only for signed-in customers (guest checkout cannot redeem)
  if (!req.customer?.id) {
    return res.json({ valid: false, reason: "Sign in to apply a coupon." });
  }
  const coupon = await lookupCoupon(raw, req.customer.id, {
    email: req.customer.email,
    phone: req.customer.phone,
  });
  if (!coupon) {
    return res.json({
      valid: false,
      reason: "This code is not available for your account — it may be for first orders only, already used, or expired.",
    });
  }
  const subtotalQ = Number(req.query.subtotal);
  if (coupon.minOrderAmount != null && Number(coupon.minOrderAmount) > 0 && Number.isFinite(subtotalQ) && subtotalQ > 0) {
    if (subtotalQ < Number(coupon.minOrderAmount)) {
      return res.json({
        valid: false,
        reason: `This code needs a minimum order of ₹${Number(coupon.minOrderAmount).toLocaleString("en-IN")} (products only, before shipping).`,
        minOrderAmount: Number(coupon.minOrderAmount),
      });
    }
  }
  res.json({
    valid: true,
    code: coupon.code,
    discountPercent: coupon.discountPercent,
    maxDiscountAmount: coupon.maxDiscountAmount ?? null,
    minOrderAmount: coupon.minOrderAmount ?? null,
  });
}));

export default router;
