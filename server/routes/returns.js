...
router.post("/", requireAuth, asyncHandler(async (req, res) => {
  ...
  // Returns only after delivery — ordering 5 minutes ago must not be eligible.
  if (returnOrder.status !== "delivered") {
    return res.status(403).json({
      error: "Returns are only available after your order has been delivered. Current status: " + returnOrder.status + ".",
    });
  }
