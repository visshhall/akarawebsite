import { Router } from "express";
import { query } from "../db.js";
import { asyncHandler } from "../asyncHandler.js";

const router = Router();

async function isSalesPaused() {
  try {
    const { rows } = await query("SELECT value FROM settings WHERE key='sales_paused'");
    return rows[0]?.value === "1";
  } catch {
    return false;
  }
}

function parseJsonArray(val) {
  if (Array.isArray(val)) return val;
  if (val == null || val === "") return [];
  if (typeof val === "string") {
    try {
      const parsed = JSON.parse(val);
      return Array.isArray(parsed) ? parsed : [];
    } catch {
      return [];
    }
  }
  return [];
}

function toFrontendShape(row) {
  return {
    id: row.id,
    name: row.name,
    cat: row.category,
    price: row.price,
    dims: row.dims == null ? "" : String(row.dims),
    hsn: row.hsn,
    status: row.status,
    description: row.description,
    metaTitle: row.meta_title,
    metaDesc: row.meta_desc,
    media: parseJsonArray(row.media),
    keyFeatures: parseJsonArray(row.key_features),
    featuredOrder: row.featured_order,
    accessoriesNote: row.accessories_note,
    finishes: parseJsonArray(row.finishes),
    createdAt: row.created_at,
    stockQty: row.stock_qty != null ? Number(row.stock_qty) : null,
  };
}

function applySalesPause(shape, variants, paused) {
  if (!paused) return { shape, variants };
  return {
    shape: { ...shape, status: "sold-out" },
    variants: (variants || []).map((v) => ({ ...v, status: "sold-out" })),
  };
}

router.get("/", asyncHandler(async (req, res) => {
  const paused = await isSalesPaused();
  const { rows } = await query("SELECT * FROM products WHERE status NOT IN ('draft','hidden') ORDER BY category, name");

  const { rows: allColors } = await query("SELECT id, product_id, variant_key, label, swatch_hex, sort_order, media FROM product_colors ORDER BY sort_order");
  const { rows: allVariants } = await query("SELECT id, product_id, color_id, size, price, status, dims FROM product_variants");
  const colorsByProduct = {};
  for (const c of allColors) (colorsByProduct[c.product_id] ||= []).push({
      id: c.id, variantKey: c.variant_key, label: c.label, swatchHex: c.swatch_hex,
      media: parseJsonArray(c.media),
    });
  const variantsByProduct = {};
  for (const v of allVariants) (variantsByProduct[v.product_id] ||= []).push({ id: v.id, colorId: v.color_id, size: v.size, price: v.price, status: v.status, dims: v.dims });

  res.json({
    salesPaused: paused,
    products: rows.map((row) => {
      const base = toFrontendShape(row);
      const variants = variantsByProduct[row.id] || [];
      const { shape, variants: vOut } = applySalesPause(base, variants, paused);
      return {
        ...shape,
        colors: colorsByProduct[row.id] || [],
        variants: vOut,
      };
    }),
  });
}));

router.get("/:id", asyncHandler(async (req, res) => {
  // WHAT: Single product for deep links / any client that does not use the catalogue list.
  // WHY:  List endpoint already returned colors[].media; this route omitted colours entirely → out of sync.
  // HOW:  Same color + variant joins as GET /, including product_colors.media.
  // CONNECTED TO: ProductDetailView (usually via catalogue); admin storefront link; sitemap.
  const paused = await isSalesPaused();
  const { rows } = await query("SELECT * FROM products WHERE id = $1 AND status NOT IN ('draft','hidden')", [req.params.id]);
  if (rows.length === 0) return res.status(404).json({ error: "Product not found." });
  const base = toFrontendShape(rows[0]);
  const { rows: colorRows } = await query(
    "SELECT id, variant_key, label, swatch_hex, sort_order, media FROM product_colors WHERE product_id=$1 ORDER BY sort_order",
    [req.params.id]
  );
  const colors = colorRows.map((c) => ({
    id: c.id,
    variantKey: c.variant_key,
    label: c.label,
    swatchHex: c.swatch_hex,
    media: parseJsonArray(c.media),
  }));
  const { rows: variants } = await query("SELECT id, product_id, color_id, size, price, status, dims FROM product_variants WHERE product_id=$1", [req.params.id]);
  const mapped = variants.map((v) => ({ id: v.id, colorId: v.color_id, size: v.size, price: v.price, status: v.status, dims: v.dims }));
  const { shape, variants: vOut } = applySalesPause(base, mapped, paused);
  res.json({ product: { ...shape, colors, variants: vOut }, salesPaused: paused });
}));

export default router;
