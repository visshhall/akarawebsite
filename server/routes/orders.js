...
async function priceCartServerSide(cartItems, couponCode, customerId) {
  ...
  const [{ rows: products }, { shippingCost: SHIPPING_COST, freeShippingThreshold: FREE_SHIPPING_THRESHOLD }, coupon] = await Promise.all([
    query("SELECT id, name, price, hsn, stock FROM products WHERE id = ANY($1)", [ids]),
    getShippingSettings(),
...
router.post("/checkout", requireAuth, asyncHandler(async (req, res) => {
  ...
  const priced = await priceCartServerSide(items, couponCode, req.customer.id);
...
router.patch("/:orderNumber/cancel", requireAuth, asyncHandler(async (req, res) => {
  ...
  const { rows: updated } = await query(
    "UPDATE orders SET status='cancelled', updated_at=now() WHERE order_number=$1 RETURNING *",
