...
export function toFrontendOrder(row) {
  return {
    ...
    email: row.email,
    phone: row.phone,
    // Defensive, real hardening — not a confirmed live bug the way
    // products.media was, but the exact same real bug CLASS: this
    // column is JSONB NOT NULL, which guarantees non-null, never
    // guarantees it's genuinely array-shaped. Every current order is
    // built through the real checkout code path, which always
    // constructs items as a real array, so this specific column has
    // never actually shown the {} corruption media did — but the
    // frontend calls .map() on order.items directly, with no fallback
    // of its own, in multiple real places (order confirmation, the
    // invoice PDF, order tracking). Guarding it once, here, at the one
    // real shared serialization point every order response passes
    // through, closes the whole class off at its real source rather
    // than needing a defensive check repeated at every one of those
