...
router.post("/", requireAuth, asyncHandler(async (req, res) => {
  ...
  const paidOk = order.payment_status === "paid" || (order.payment_status === "cod" && order.status === "delivered");
  ...
  // call .some() on).
  const containsProduct = (Array.isArray(order.items) ? order.items : []).some(i => i.id === productId);
