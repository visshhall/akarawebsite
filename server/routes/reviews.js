...
import { sanitize, validMessage, cleanMessage } from "../validate.js";
...
router.post("/", requireAuth, asyncHandler(async (req, res) => {
  ...
  const { rows: orderRows } = await query(
    "SELECT id, items, payment_status, status FROM orders WHERE order_number=$1 AND customer_id=$2",
  ...
  // Verified purchase: paid online, or COD placed (customer confirmed the order).
  // Delivered-only was too late for the post-purchase thank-you review prompt.
  const paidOk = order.payment_status === "paid" || order.payment_status === "cod";
  if (!paidOk) return res.status(403).json({ error: "This order isn't eligible for a review yet." });
  // Found during a wider audit of the same bug class as the Orders-list
  // status badge: payment_status alone doesn't tell you an order is
  // still active — a cancelled order that failed to refund stays
  // "paid" forever, and would otherwise still be reviewable.
  if (order.status === "cancelled") return res.status(403).json({ error: "This order was cancelled, so it can't be reviewed." });

  // Real, same defensive guard as everywhere else in this sweep —
  // order.items is NOT NULL in the schema, so `|| []` already handled
  // null/undefined, but not the specific {} shape that broke product
  // media (an object, not an array, still truthy, still not safe to
  // call .some() on).
  ...
  const containsProduct = orderItems.some(i => i && (i.id === productId || i.product_id === productId));
  ...
  try {
    const { rows } = await query(
      ...
      [productId, req.customer.id, order.id, ratingNum, comment ? sanitize(comment).slice(0, 1000) : null]
    );
