...
router.put("/", requireAuth, asyncHandler(async (req, res) => {
  ...
  for (const raw of incoming.slice(0, 100)) {
    ...
    if (seen.has(key)) {
      ...
      // MAX not SUM — summing duplicates inflated qty on racey clients.
      if (existing) existing.qty = Math.min(99, Math.max(existing.qty, n.qty));
      continue;
    }
    seen.add(key);
    lines.push(n);
...
router.post("/merge", requireAuth, asyncHandler(async (req, res) => {
  ...
  for (const raw of incoming.slice(0, 100)) {
    const n = normalizeLine(raw);
    if (!n) continue;
    // MAX not SUM — summing on every page load doubled 1→2→4→… (merge ran after local hydrate).
    await query(
      `INSERT INTO cart_items (customer_id, product_id, size, color, qty)
       VALUES ($1,$2,$3,$4,$5)
