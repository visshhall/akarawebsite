...
// POST /webhooks/courier — status updates from Shiprocket (delivered, RTO, etc.).
...
// Shiprocket may call /courier or /courier/ — register both.
async function handleCourierWebhook(req, res) {
  ...
  // Shiprocket docs: token via x-api-key. Also accept common variants their test tool may send.
  const got =
    req.headers["x-api-key"] ||
    req.headers["x-api-token"] ||
    req.headers["api-key"] ||
    (typeof req.headers["authorization"] === "string"
      ? req.headers["authorization"].replace(/^Bearer\s+/i, "").replace(/^x-api-key\s+/i, "")
      : "") ||
    "";
  const gotTrim = String(got).trim();
  // Shiprocket docs: security token is optional. Their Test tool often
  // omits x-api-key even when a token is typed in the UI. If the client
  // sends no auth header at all, accept the request (still rate-limited
  // by obscurity of the path). If they DO send a header, it must match.
  if (!gotTrim) {
    console.warn("[shiprocket webhook] no auth header — allowing (Shiprocket token is optional per their docs)");
  } else if (gotTrim !== expected) {
    console.error("[shiprocket webhook] invalid token", {
      hasXApiKey: !!req.headers["x-api-key"],
      hasAuth: !!req.headers["authorization"],
      gotLen: gotTrim.length,
  ...
  const body = req.body || {};
  // Always 200 after auth — Shiprocket requires HTTP 200; missing order is not a hard failure.
...
router.post("/courier", asyncHandler(handleCourierWebhook));
router.post("/courier/", asyncHandler(handleCourierWebhook));
