...
router.post("/razorpay", webhookRateLimit, asyncHandler(async (req, res) => {
...
async function handleCourierWebhook(req, res) {
  ...
  // SECURITY: token is required when configured. Empty header must not
  // update order status (forged "delivered" would be critical).
  if (!gotTrim) {
    console.error("[shiprocket webhook] missing auth header — rejected");
    return res.status(401).json({ error: "Unauthorized." });
  }
  const a = Buffer.from(gotTrim);
...
router.post("/courier", webhookRateLimit, asyncHandler(handleCourierWebhook));
router.post("/courier/", webhookRateLimit, asyncHandler(handleCourierWebhook));
