...
router.put("/:id", requireRole("admin","super_admin"), asyncHandler(async (req, res) => {
  ...
  const { name, category, price, dims, hsn, status, description, metaTitle, metaDesc, media, keyFeatures, accessoriesNote, finishes, costBreakdown } = req.body || {};
  ...
  const { rows } = await query(
    `UPDATE products SET name=$1, category=$2, price=$3, dims=$4, hsn=$5, status=$6, description=$7, meta_title=$8, meta_desc=$9, media=$10, key_features=$11, accessories_note=$12, finishes=$13, cost_breakdown=$14, updated_at=now()
     WHERE id=$15 RETURNING *`,
    [updated.name, updated.category, updated.price, updated.dims, updated.hsn, updated.status, updated.description, updated.meta_title, updated.meta_desc, updated.media, updated.key_features, updated.accessories_note, updated.finishes, updated.cost_breakdown, req.params.id]
  );
