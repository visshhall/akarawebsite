...
import { auditProductCategories } from "../../categoryAudit.js";
import { sanitize } from "../../validate.js";
import { sendLowStockAlertEmail, sendWaitlistRestockEmail } from "../../email.js";
...
// GET /api/admin/products/category-audit — automated category mismatch scan
router.get("/category-audit", requireRole("admin","super_admin"), asyncHandler(async (_req, res) => {
  const { rows } = await query(
    `SELECT id, name, category, status FROM products ORDER BY name`
  );
  const report = auditProductCategories(rows);
  res.json(report);
}));
...
function validateFinishes(finishes) {
  ...
  if (finishes.length > 12) return "Maximum 12 finishes.";
  for (const f of finishes) {
    if (!f || typeof f.name !== "string" || !f.name.trim()) return "Each finish needs a name.";
    if (f.name.length > 40) return "Finish name too long.";
    if (f.hex && !/^#[0-9A-Fa-f]{3,8}$/.test(String(f.hex).trim())) return "Finish colour must be a hex code like #C4B49A.";
  }
  return null;
}
// Real, fixed set of cost categories — directly requested: material,
// labor, electricity, packaging, transport, design, and a real
// catch-all "other" for anything that doesn't fit the named ones.
// Deliberately a closed, real set (not free-form keys) so the
// dashboard chart being built alongside this can always assume the
// exact same real categories exist across every product, rather than
// handling an unpredictable, admin-typed set of category names.
function validateCostBreakdown(costBreakdown) {
  if (!costBreakdown || typeof costBreakdown !== "object" || Array.isArray(costBreakdown)) {
    return "Cost breakdown must be an object.";
  }
  for (const key of Object.keys(costBreakdown)) {
    if (!COST_CATEGORIES.includes(key)) return `Unknown cost category: ${key}.`;
  }
  for (const key of COST_CATEGORIES) {
    if (costBreakdown[key] === undefined) continue;
    const v = costBreakdown[key];
    // Real, deliberate choice to allow 0 (a genuinely real, valid cost
    // for a category that doesn't apply to a given product — e.g. no
    // real transport cost for a locally-collected order) but reject
    // anything negative or non-numeric, since a real, negative cost is
    // never a genuine, real input, only ever a real mistake.
    if (typeof v !== "number" || !Number.isFinite(v) || v < 0) {
      return `Cost for "${key}" must be a real, non-negative number.`;
    }
  }
  return null;
}
...
router.get("/:id", requireRole("admin","super_admin"), asyncHandler(async (req, res) => {
  ...
  const { rows: colors } = await query(
    "SELECT id, variant_key, label, swatch_hex, sort_order, media FROM product_colors WHERE product_id=$1 ORDER BY sort_order",
    [req.params.id]
  );
  ...
  res.json({
    product: rows[0],
    colors: colors.map(c => ({
      id: c.id, variantKey: c.variant_key, label: c.label, swatchHex: c.swatch_hex, sortOrder: c.sort_order,
      media: Array.isArray(c.media) ? c.media : (typeof c.media === "string" ? (() => { try { const p = JSON.parse(c.media); return Array.isArray(p) ? p : []; } catch { return []; } })() : []),
    })),
...
router.put("/:id/variants", requireRole("admin","super_admin"), asyncHandler(async (req, res) => {
  ...
  for (const c of colors) {
    ...
    // Optional per-colour media (same rules as product media). Empty = fall back to product gallery.
    if (c.media !== undefined && c.media !== null) {
      const mediaErr = validateMedia(Array.isArray(c.media) ? c.media : []);
      if (mediaErr) return res.status(400).json({ error: `Colour "${c.label}": ${mediaErr}` });
    }
  }
  ...
  // WHAT: Replace this product's entire colour + variant set in one request.
  // WHY:  Partial row deletes left size-only variants (color_id NULL) in the DB;
  //       admins saw “deleted” combinations return on reopen.
  // HOW:  DELETE all product_variants for product_id, then all product_colors,
  //       then INSERT the payload (may be empty arrays = base price only).
  // CONNECTED TO: AdminApp VariantEditor (Details tab) → customer ProductDetailView
  //               size/colour pickers + checkout stock checks; products public API.
  // Real delete-then-reinsert. MUST delete product_variants first —
  // size-only rows have color_id NULL and are NOT removed by cascading
  // DELETE on product_colors (only rows that reference a color_id are).
  // That bug left "ghost" S/M/L (or merge) variants after an admin
  // cleared the list and saved — they reappeared on every reopen.
  ...
  for (let i = 0; i < colors.length; i++) {
    const c = colors[i];
    const colorMedia = Array.isArray(c.media) ? c.media : [];
    const { rows: inserted } = await query(
      "INSERT INTO product_colors (product_id, variant_key, label, swatch_hex, sort_order, media) VALUES ($1,$2,$3,$4,$5,$6::jsonb) RETURNING id",
      [productId, c.variantKey, c.label, c.swatchHex || null, i, JSON.stringify(colorMedia)]
    );
  ...
  const { rows: checkV } = await query("SELECT COUNT(*)::int AS n FROM product_variants WHERE product_id=$1", [productId]);
  const { rows: checkC } = await query("SELECT COUNT(*)::int AS n FROM product_colors WHERE product_id=$1", [productId]);
  res.json({ ok: true, variantCount: checkV[0]?.n ?? 0, colorCount: checkC[0]?.n ?? 0 });
}));
...
router.put("/:id", requireRole("admin","super_admin"), asyncHandler(async (req, res) => {
  ...
  const { name, category, price, dims, hsn, status, stockQty, description, metaTitle, metaDesc, media, keyFeatures, accessoriesNote, finishes, costBreakdown } = req.body || {};
  ...
  if (finishes !== undefined) {
    const finErr = validateFinishes(finishes);
    if (finErr) return res.status(400).json({ error: finErr });
  }
  if (costBreakdown !== undefined) {
    const cbErr = validateCostBreakdown(costBreakdown);
    if (cbErr) return res.status(400).json({ error: cbErr });
  }
  ...
  const updated = {
    ...
    stock_qty: stockQty !== undefined
      ? (stockQty === null || stockQty === "" ? null : (Number.isInteger(Number(stockQty)) && Number(stockQty) >= 0 ? Number(stockQty) : existing.stock_qty))
      : existing.stock_qty,
    ...
    finishes: finishes !== undefined
      ? JSON.stringify(finishes.map(f => ({ name: sanitize(f.name).trim().slice(0, 40), hex: f.hex ? String(f.hex).trim().slice(0, 9) : null })))
      : JSON.stringify(existing.finishes || []),
    // Real, deliberate normalization: only ever stores the fixed,
    // known COST_CATEGORIES keys, in the same order, even if the admin
    // UI only ever sent a subset (e.g. left "transport" untouched at
    // its default) — keeps every real product's cost_breakdown value
    // structurally identical, which the real profit dashboard being
    // built alongside this can then rely on without defensive checks
    // for missing keys on every single read.
    cost_breakdown: costBreakdown !== undefined
      ? JSON.stringify(Object.fromEntries(COST_CATEGORIES.map(k => [k, Number(costBreakdown[k]) || 0])))
      : JSON.stringify(existing.cost_breakdown && Object.keys(existing.cost_breakdown).length > 0
          ? Object.fromEntries(COST_CATEGORIES.map(k => [k, Number(existing.cost_breakdown[k]) || 0]))
          : Object.fromEntries(COST_CATEGORIES.map(k => [k, 0]))),
  };

  // Sold-out always stores capacity 0 so customer badges cannot show “in stock” from leftover qty
  if (updated.status === "sold-out") updated.stock_qty = 0;

  const { rows } = await query(
    `UPDATE products SET name=$1, category=$2, price=$3, dims=$4, hsn=$5, status=$6, description=$7, meta_title=$8, meta_desc=$9, media=$10, key_features=$11, accessories_note=$12, finishes=$13, cost_breakdown=$14, stock_qty=$15, updated_at=now()
     WHERE id=$16 RETURNING *`,
    [updated.name, updated.category, updated.price, updated.dims, updated.hsn, updated.status, updated.description, updated.meta_title, updated.meta_desc, updated.media, updated.key_features, updated.accessories_note, updated.finishes, updated.cost_breakdown, updated.stock_qty, req.params.id]
  );
  ...
  // Restock: was sold-out (or stock 0) and is now available → email waitlist once.
  const wasUnavailable = existing.status === "sold-out" || (existing.stock_qty != null && Number(existing.stock_qty) === 0);
  const nowAvailable = updated.status !== "sold-out" && updated.status !== "draft" && updated.status !== "hidden"
    && (updated.stock_qty == null || Number(updated.stock_qty) > 0);
  if (wasUnavailable && nowAvailable) {
    const { rows: waiters } = await query(
      `SELECT id, email FROM product_waitlist
    ...
    for (const w of waiters) {
      try {
        await sendWaitlistRestockEmail({
          email: w.email,
          productName: updated.name || rows[0]?.name,
          productId: req.params.id,
        });
        await query("UPDATE product_waitlist SET notified_at = now() WHERE id = $1", [w.id]);
      } catch (err) {
        console.error("[waitlist] notify failed", w.email, err?.message || err);
      }
    }
  }
...
router.post("/", requireRole("admin","super_admin"), asyncHandler(async (req, res) => {
  let { id, name, category, price, dims, hsn, status, description, metaTitle, metaDesc, media, keyFeatures, accessoriesNote } = req.body || {};
  // Auto URL slug from name when admin leaves id empty (same rules as admin UI).
  ...
  if (!id || !/^[a-z0-9-]+$/.test(id)) {
    return res.status(400).json({ error: "Product ID must be lowercase letters, numbers, and hyphens only (used as the URL slug). Type a name to auto-fill, or enter a custom slug." });
  }
