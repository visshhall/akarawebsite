...
// requireRole("admin","super_admin") specifically — not even a regular admin can
...
const PAGES = {
  ...
  contact: "Contact",
  home: "Homepage Hero",
  home_sections: "Homepage Sections (titles & strips)",
  footer: "Footer",
...
// for editing.
router.get("/:pageKey", requireRole("admin","super_admin"), asyncHandler(async (req, res) => {
...
router.put("/:pageKey", requireRole("admin","super_admin"), asyncHandler(async (req, res) => {
...
router.get("/:pageKey/history", requireRole("admin","super_admin"), asyncHandler(async (req, res) => {
  if (!PAGES[req.params.pageKey]) return res.status(404).json({ error: "Unknown page." });
  const { rows } = await query(
...
router.post("/:pageKey/revert/:versionId", requireRole("admin","super_admin"), asyncHandler(async (req, res) => {
  const pageKey = req.params.pageKey;
  if (!PAGES[pageKey]) return res.status(404).json({ error: "Unknown page." });
