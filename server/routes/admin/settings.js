...
router.get("/coupons", requireRole("admin","super_admin"), asyncHandler(async (req, res) => {
  const { rows } = await query("SELECT code, discount_percent, COALESCE(first_order_only, false) AS first_order_only, active, expires_at, max_redemptions, one_per_customer, featured, label, created_at FROM coupons ORDER BY created_at DESC");
...
router.post("/coupons", requireRole("admin","super_admin"), asyncHandler(async (req, res) => {
  const { code, discountPercent, expiresAt, maxRedemptions, onePerCustomer, firstOrderOnly, featured, label } = req.body || {};
  ...
  await query(
    "INSERT INTO coupons (code, discount_percent, expires_at, max_redemptions, one_per_customer, first_order_only, featured, label) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)",
    [normalizedCode, discountPct, expiresAt || null, maxRedemptions || null, Boolean(onePerCustomer), Boolean(firstOrderOnly), Boolean(featured), label || null]
  );
