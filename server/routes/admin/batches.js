...
router.get("/", requireAdmin, asyncHandler(async (req, res) => {
  ...
  for (const r of rows) {
    ...
    // Highest linked non-cancelled order status drives display when ahead of batch row
    ...
    const rank = { confirmed: 0, production: 1, qc: 2, dispatched: 3, delivered: 4 };
    let maxOrder = null;
    let maxR = -1;
    for (const s of stRows) {
      const rr = rank[s.status] ?? -1;
      if (rr > maxR) { maxR = rr; maxOrder = s.status; }
    }
    let displayStatus = r.status;
    if (maxOrder === "qc" && r.status !== "closed") displayStatus = "qc";
    else if (maxOrder === "production" && (r.status === "open" || r.status === "in_production")) displayStatus = "in_production";
    else if ((maxOrder === "dispatched" || maxOrder === "delivered") && r.status !== "closed") displayStatus = "qc"; // still closable
    // Auto-heal DB row so UI buttons match reality
    if (displayStatus !== r.status && displayStatus !== "closed") {
      try {
        await query(`UPDATE production_batches SET status=$1, updated_at=now() WHERE id=$2`, [displayStatus, r.id]);
      } catch (_) {}
    }
    enriched.push({
      id: r.id,
