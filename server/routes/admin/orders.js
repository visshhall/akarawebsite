...
import { createShipment, fetchTrackingStatus, cancelShipment } from "../../shiprocket.js";
...
router.patch("/:orderNumber/status", requireAdmin, asyncHandler(async (req, res) => {
  ...
  if (status === "dispatched") {
    ...
    const addr = existing.shipping_address || {};
    const itemsRaw = Array.isArray(existing.items) ? existing.items : [];
    const shipPayload = {
    ...
    const shipment = await createShipment(shipPayload, pickupLocation);
    if (!shipment?.ok) {
      await logAdminAction(req.admin.id, "order.shiprocket_failed", {
        orderNumber: req.params.orderNumber,
        from: currentStatus,
        error: shipment?.error || "unknown",
        skipped: !!shipment?.skipped,
      });
      return res.status(502).json({
        error: shipment?.error || "Shiprocket could not create the shipment. Order was NOT marked dispatched.",
        shipment: {
          ok: false,
          skipped: !!shipment?.skipped,
