...
/** Keep production_batches.status in step with the order that carries that batch number. */
async function syncBatchStatusFromOrder(orderRow, orderStatus) {
  const map = {
  ...
  const batchNumbers = [...new Set(items.map((i) => i && i.batchNumber).filter(Boolean))];
  for (const bn of batchNumbers) {
    try {
      // Never reopen a closed batch when moving backward; only advance or set to matching stage.
      if (target === "closed") {
        await query(
          `UPDATE production_batches SET status = 'closed', closed_at = COALESCE(closed_at, now()), updated_at = now()
           WHERE batch_number = $1 AND status IN ('open','in_production','qc')`,
          [bn]
        );
      } else if (target === "qc") {
        await query(
          `UPDATE production_batches SET status = 'qc', updated_at = now()
           WHERE batch_number = $1 AND status IN ('open','in_production','qc')`,
          [bn]
        );
      } else if (target === "in_production") {
        await query(
          `UPDATE production_batches SET status = 'in_production', updated_at = now()
           WHERE batch_number = $1 AND status IN ('open','in_production')`,
          [bn]
        );
      }
    } catch (e) {
      console.error("[batch-sync]", bn, e?.message || e);
    }
  }
}

async function ensureBatchOnProduction(orderRow) {
  // When moving to production: if no line has a batch yet, create AK-YYYYMM-NNN and stamp all lines.
  let items = orderRow.items;
  if (typeof items === "string") {
    try { items = JSON.parse(items); } catch { items = []; }
  }
  if (!Array.isArray(items) || items.length === 0) return orderRow;
  if (items.some((i) => i && i.batchNumber)) return orderRow;
  ...
  try {
    await query(
      `INSERT INTO production_batches (batch_number, year_month, product_id, notes, status)
       VALUES ($1,$2,$3,$4,'in_production')`,
      [batchNumber, ym, productId, `Auto · order ${orderRow.order_number}`]
    );
  } catch (e) {
    if (e?.code !== "23505") console.error("[auto-batch]", e?.message || e);
  }
...
router.patch("/:orderNumber/status", requireAdmin, asyncHandler(async (req, res) => {
  ...
  if (status === "dispatched") {
    ...
    await syncBatchStatusFromOrder(orderRow, "dispatched");
    if (orderRow.customer_id) {
      notifyCustomer(orderRow.customer_id, {
        title: "ĀKĀRA · On its way",
        body: `Order #${req.params.orderNumber} has been dispatched.`,
        url: `/order-status?order=${encodeURIComponent(req.params.orderNumber)}`,
        tag: `order-${req.params.orderNumber}`,
      }).catch(() => {});
    }
  ...
  if (status === "production" && rows[0]) {
    rows = [await ensureBatchOnProduction(rows[0])];
  }
  if (rows[0]) {
    await syncBatchStatusFromOrder(rows[0], status);
  }
  await logAdminAction(req.admin.id, "order.status_change", {
  ...
  if (rows[0]?.customer_id && ["production","qc","dispatched","delivered","cancelled"].includes(status)) {
    notifyCustomer(rows[0].customer_id, {
      ...
      body: `Order #${req.params.orderNumber} · ${statusLabels[status]||status}`,
      url: `/order-status?order=${encodeURIComponent(req.params.orderNumber)}`,
      tag: `order-${req.params.orderNumber}`,
    }).catch(()=>{});
  }
  ...
  // Close open auto-batches whose only linked orders are cancelled
  if (status === "cancelled" && order?.items) {
    try {
      const batchNos = [...new Set((order.items || []).map((i) => i.batchNumber).filter(Boolean))];
      for (const bn of batchNos) {
        const { rows: active } = await query(
          `SELECT 1 FROM orders o
           WHERE o.status != 'cancelled'
             AND EXISTS (
               SELECT 1 FROM jsonb_array_elements(CASE WHEN jsonb_typeof(o.items)='array' THEN o.items ELSE '[]'::jsonb END) e
               WHERE e->>'batchNumber' = $1
             )
           LIMIT 1`,
          [bn]
        );
        if (active.length === 0) {
          await query(
            `UPDATE production_batches SET status = 'closed', closed_at = COALESCE(closed_at, now()), notes = COALESCE(notes,'') || ' · closed (order cancelled)', updated_at = now()
             WHERE batch_number = $1 AND status IN ('open','in_production')`,
            [bn]
          );
        }
      ...
      console.error("[batch] close on cancel", e?.message || e);
    }
  }
