...
router.patch("/:orderNumber/status", requireAdmin, asyncHandler(async (req, res) => {
  ...
  // Dispatching without a pickup location used to still mark the order
  // dispatched — notifying the customer their order had shipped — even
  // though createShipment() below actually needs that pickup location
  // to create a real, trackable shipment. Checked before the update
  // itself runs, so a mistaken dispatch attempt never reaches the
  // database or sends a false "shipped" notification at all.
