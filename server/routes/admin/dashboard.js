...
router.get("/", requireRole("staff", "admin", "super_admin"), asyncHandler(async (req, res) => {
  ...
  // --- Live operational scores (all from SQL above; never hardcoded) ---
  const stuck = Number(stuckProduction.rows[0].count);
  const missingImg = Number(missingImages.rows[0].count);
  const pipeline = Number(inProduction.rows[0].count);
  // Attention uses full counts (not LIMIT 10 list lengths) so the badge matches reality.
  const lowStockOnly = lowStock.rows.filter((p) => p.status === "low-stock");
  const soldOutOnly = lowStock.rows.filter((p) => p.status === "sold-out");
  // Separate COUNT-style: list length is capped at 10 for returns/payments — use row length of limited lists
  // is wrong when >10. Re-query counts inline via already-fetched todayStrip later; for attention use:
  // pending returns list max 10 + we need true counts. Fetch was list-only — add dedicated counts below via
  // pendingReturns.rows.length is undercount. Fix: run count from list is OK if we change query to not limit
  // for counts — use lowStockOnly.length for low-stock only.
  const pendingReturnsN = pendingReturns.rows.length; // list capped at 10 — still used for display; see attentionCounts
  const failedPaymentsN = failedPayments.rows.length; // capped at 10
  // Prefer: attention from components we can trust without new queries in this replace —
  // We'll add exact counts via additional fields from todayStrip after its queries.
  // Temporary: use filtered low-stock only (not sold-out) + pending + failed lists.
  const attention =
    pendingReturnsN + lowStockOnly.length + failedPaymentsN;

  // Studio risk: weighted live pressure score 0–100 (documented for admin UI)
  //   stuck (>14d in production/qc): 15 pts each, max 45
  //   attention items: 6 pts each, max 30
  //   catalogue missing images: 4 pts each, max 20
  //   active pipeline (confirmed/production/qc): 1 pt each, max 15
  const riskStuck = Math.min(45, stuck * 15);
  const riskAtt = Math.min(30, attention * 6);
  const riskImg = Math.min(20, missingImg * 4);
  const riskPipe = Math.min(15, pipeline * 1);
  const studioRisk = Math.min(100, riskStuck + riskAtt + riskImg + riskPipe);
  const studioRiskLabel = studioRisk >= 60 ? "High" : studioRisk >= 30 ? "Elevated" : "Calm";
  const studioRiskBreakdown = {
    stuckOrders: stuck,
    stuckPoints: riskStuck,
    attentionItems: attention,
    attentionPoints: riskAtt,
    missingImages: missingImg,
    missingImagePoints: riskImg,
    pipelineOrders: pipeline,
    pipelinePoints: riskPipe,
    formula: "min(100, stuck×15≤45 + attention×6≤30 + missingImages×4≤20 + pipeline×1≤15)",
  };
  const attentionBreakdown = {
    pendingReturns: pendingReturnsN,
    lowStock: lowStockOnly.length,
    soldOut: soldOutOnly.length,
    failedOrPendingPayments14d: failedPaymentsN,
    total: attention,
    note: "Needs attention = pending returns + low-stock products + failed/pending payments (14 days). Sold-out is listed separately and does not inflate this number.",
  };
  ...
  // Refine attention with exact pending-returns COUNT (list was LIMIT 10).
  const pendingReturnsExact = Number(pendingReturnsCountQ.rows[0]?.count || 0);
  const lowStockExact = lowStock.rows.filter((p) => p.status === "low-stock").length;
  const soldOutExact = lowStock.rows.filter((p) => p.status === "sold-out").length;
  const failedExact = failedPayments.rows.length; // up to 10 listed; see note
  attentionBreakdown.pendingReturns = pendingReturnsExact;
  attentionBreakdown.lowStock = lowStockExact;
  attentionBreakdown.soldOut = soldOutExact;
  attentionBreakdown.failedOrPendingPayments14d = failedExact;
  attentionBreakdown.total = pendingReturnsExact + lowStockExact + failedExact;
  attentionBreakdown.note =
    "Needs attention = pending returns (all) + products marked low-stock + failed/pending payments in the last 14 days. Sold-out is shown separately and is not included in the number.";
  const attentionExact = attentionBreakdown.total;
  // Recompute risk with refined attention
  const riskAtt2 = Math.min(30, attentionExact * 6);
  const studioRiskExact = Math.min(100, riskStuck + riskAtt2 + riskImg + riskPipe);
  const studioRiskLabelExact = studioRiskExact >= 60 ? "High" : studioRiskExact >= 30 ? "Elevated" : "Calm";
  studioRiskBreakdown.attentionItems = attentionExact;
  studioRiskBreakdown.attentionPoints = riskAtt2;


  // Automated category audit (lamps under Planters, missing category, etc.)
  let categoryAudit = { summary: { issueCount: 0, conflicts: 0, missingCategory: 0 }, issues: [] };
  try {
    const { rows: catRows } = await query(`SELECT id, name, category, status FROM products`);
    categoryAudit = auditProductCategories(catRows);
  } catch (e) {
    console.warn("[dashboard] category audit skipped:", e?.message || e);
  }
  ...
  res.json({
    todayStrip: {
      ...
      lowStock: lowStockExact,
      soldOut: soldOutExact,
      pendingReturns: pendingReturnsExact,
    ...
    attentionCount: attentionExact,
    attentionBreakdown,
    categoryAudit,
    studioRisk: studioRiskExact,
    studioRiskLabel: studioRiskLabelExact,
    studioRiskBreakdown,
...
router.get("/profit", requireRole("admin", "super_admin"), asyncHandler(async (req, res) => {
  ...
  // Exclude cancelled — test cancels must not drag realised profit negative.
  // Revenue = sum of order line (price × qty) as charged at checkout (GST-inclusive rupees).
  const { rows: salesRows } = await query(
    `SELECT item->>'id' AS product_id,
       SUM((item->>'qty')::int) AS units_sold,
       SUM((item->>'price')::numeric * (item->>'qty')::int) AS revenue,
       ROUND(AVG((item->>'price')::numeric)::numeric, 2) AS avg_unit_price
     FROM orders, jsonb_array_elements(items) AS item
     WHERE payment_status IN ('paid','cod')
       AND status IS DISTINCT FROM 'cancelled'
     GROUP BY item->>'id'`
  );
  const salesByProduct = Object.fromEntries(salesRows.map(r => [r.product_id, { unitsSold: Number(r.units_sold), revenue: Number(r.revenue), avgUnitPrice: r.avg_unit_price != null ? Number(r.avg_unit_price) : null }]));
  ...
  const items = products.map((p) => {
    ...
    const sales = salesByProduct[p.id] || { unitsSold: 0, revenue: 0, avgUnitPrice: null };
    const totalCost = totalCostPerUnit * sales.unitsSold;
    // Realised = what customers actually paid on lines − (unit cost × units). Not the chart bar.
    const profit = Math.round((sales.revenue - totalCost) * 100) / 100;
    ...
    // List margin = current catalogue price vs cost (pricing health). Independent of sales.
    const marginPercent =
    ...
    // When sold, realised margin uses average charged unit price if available.
    const realisedUnit = sales.unitsSold > 0 && sales.revenue > 0
      ? sales.revenue / sales.unitsSold
      : null;
    const realisedMarginPercent =
      hasCostData && realisedUnit > 0
        ? Math.round(((realisedUnit - totalCostPerUnit) / realisedUnit) * 1000) / 10
        : null;
    // Break-even: cost sheet is unit variable cost only → first unit sold at list price covers itself.
    // unitsToClearLoss: full-price units still needed if realised profit is negative (e.g. test/discount sales).
    const breakEvenUnits =
      expectedProfitPerUnit != null && expectedProfitPerUnit > 0 ? 1 : null;
    const unitsToClearLoss =
      profit < 0 && expectedProfitPerUnit != null && expectedProfitPerUnit > 0
        ? Math.ceil(Math.abs(profit) / expectedProfitPerUnit)
        : null;
    ...
    return {
      ...
      revenue: Math.round(Number(sales.revenue) * 100) / 100,
      avgSoldUnitPrice: realisedUnit != null ? Math.round(realisedUnit * 100) / 100 : null,
      realisedMarginPercent,
      totalCost: Math.round(totalCost * 100) / 100,
      breakEvenUnits,
      unitsToClearLoss,
      contributionPerUnit: expectedProfitPerUnit,
      profit,
      marginPercent,
