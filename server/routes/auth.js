...
router.post("/signup/request", publicFormRateLimit, requireTurnstile, asyncHandler(async (req, res) => {
  ...
  // Never send WhatsApp OTP to the business source number (same as GUPSHUP_SOURCE_NUMBER).
  // Meta/Gupshup routinely drop or reject template sends destination === source.
  ...
  // Always attempt WhatsApp (unless destination is the business line) AND email
  // with the same OTP so the customer always has a path to complete signup.
  let waResult = { skipped: true, error: "not_attempted" };
  if (isBusinessNumber) {
    console.warn("[auth/signup] Phone matches WhatsApp business source — skipping WA OTP, email only");
  ...
  if (waOk && emailOk) {
    return res.json({
      ok: true,
      channel: "both",
      notice: "Code sent on WhatsApp and email. Use either.",
    });
  }
  if (waOk) {
  ...
  if (emailOk) {
    return res.json({
      ok: true,
      channel: "email",
      notice: isBusinessNumber
        ? "Use a personal mobile for WhatsApp OTP — the business WhatsApp number can't receive its own templates. Code sent to your email."
        : "WhatsApp delivery failed — we sent the code to your email instead.",
    });
  }
  console.warn("[auth/signup] Both channels failed", { wa: waResult, email: emailResult });
  if (emailResult?.skipped && waResult?.skipped) {
    return res.status(502).json({
      error: "Couldn't send verification. Check GUPSHUP_* and RESEND_API_KEY on the server.",
    });
  }
  return res.status(502).json({
