...
router.post("/signup/request", publicFormRateLimit, requireTurnstile, asyncHandler(async (req, res) => {
  ...
  // WhatsApp first; if it fails for any reason, email the same OTP so signup is not blocked.
  const result = await sendSignupOTPWhatsApp(normalizedPhone, otp, name.trim());
  if (result?.ok === true) {
    return res.json({ ok: true, channel: "whatsapp" });
  }
  console.warn("[auth/signup] WhatsApp OTP not delivered:", result?.error || result);
  const emailResult = await sendSignupOTPEmail(cleanEmail, otp);
  if (emailResult?.ok === true) {
    return res.json({
      ok: true,
      channel: "email",
      notice: "WhatsApp delivery failed — we sent the code to your email instead.",
    });
  }
  if (emailResult?.skipped) {
    return res.status(502).json({
      error: "Couldn't send verification on WhatsApp or email. Check GUPSHUP_* and RESEND_API_KEY on the server.",
    });
  }
  return res.status(502).json({
