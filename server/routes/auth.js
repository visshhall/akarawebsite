...
router.post("/phone-change/request", requireAuth, publicFormRateLimit, asyncHandler(async (req, res) => {
  ...
  // OTP on WhatsApp to the NEW number — proves the customer controls that handset.
  // Falls back to email only if WhatsApp template is not configured.
  let result = await sendPhoneChangeOTPWhatsApp(cleanPhone, otp);
  let channel = "whatsapp";
  if (result?.skipped || result?.ok === false) {
    if (result?.skipped && !process.env.GUPSHUP_TEMPLATE_SIGNUP_OTP) {
      result = await sendPhoneChangeOTPEmail(req.customer.email, otp);
      channel = "email";
    }
  }
  if (result?.skipped || result?.ok === false) {
    return res.status(502).json({ error: "Couldn't send the verification code on WhatsApp. Please try again in a moment." });
  }
