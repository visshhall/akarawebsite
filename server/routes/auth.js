...
import { validEmail, validIndianPhone, normalizePhone, sanitize, validPersonName, cleanPersonName } from "../validate.js";
import {
...
import { sendSignupOTPWhatsApp, sendPhoneChangeOTPWhatsApp } from "../whatsapp.js";
...
import { requireTurnstile, getTurnstileSiteKey, turnstileConfigured } from "../turnstile.js";

const SITE_URL = process.env.SITE_URL || "https://www.akaraonline.co.in";

const router = Router();

// Public: whether Turnstile is on + site key (never the secret).
router.get("/turnstile-config", (_req, res) => {
  res.json({ enabled: turnstileConfigured(), siteKey: getTurnstileSiteKey() });
});
...
router.post("/signup/request", publicFormRateLimit, requireTurnstile, asyncHandler(async (req, res) => {
  const { name, email, phone, password } = req.body || {};
  if (!validPersonName(name)) {
    return res.status(400).json({ error: "Enter a valid name (letters only — no special characters)." });
  }
  ...
  await query(
    ...
    [cleanEmail, cleanName, normalizedPhone, passwordHash, otpHash]
  );

  // Signup verification is email-first while WhatsApp OTP is temporarily
  // unavailable for customers. Gupshup/WhatsApp code paths remain intact —
  // set SIGNUP_WHATSAPP_OTP=1 on Railway to re-enable background WhatsApp sends.
  // User-facing copy always points to email so the experience stays clear and premium.
  const waEnabled = ["1", "true", "yes"].includes(String(process.env.SIGNUP_WHATSAPP_OTP || "").toLowerCase());
  let waResult = { skipped: true, error: "whatsapp_otp_paused" };
  if (waEnabled) {
    const sourceDigits = String(process.env.GUPSHUP_SOURCE_NUMBER || "").replace(/\D/g, "");
    const phoneDigits = String(normalizedPhone || "").replace(/\D/g, "");
    const phoneAs91 = phoneDigits.length === 10 ? `91${phoneDigits}` : phoneDigits;
    const isBusinessNumber =
      sourceDigits &&
      (phoneAs91 === sourceDigits ||
        phoneDigits === sourceDigits ||
        phoneDigits === sourceDigits.replace(/^91/, "") ||
        phoneAs91 === sourceDigits.replace(/^91/, ""));
    if (isBusinessNumber) {
      console.warn("[auth/signup] Phone matches WhatsApp business source — skipping WA OTP");
      waResult = { skipped: true, error: "destination_is_business_number" };
    } else {
      waResult = await sendSignupOTPWhatsApp(normalizedPhone, otp, cleanName);
    }
  } else {
    console.log("[auth/signup] WhatsApp OTP paused (SIGNUP_WHATSAPP_OTP not enabled) — email only");
  }

  const emailResult = await sendSignupOTPEmail(cleanEmail, otp);
  if (emailResult?.ok === true) {
    return res.json({
      ok: true,
      channel: "email",
      notice:
        "WhatsApp verification is temporarily unavailable. A secure code has been sent to your email — please use that to complete registration.",
    });
  }
  if (emailResult?.skipped) {
    return res.status(502).json({
      error: "We couldn't send your verification email right now. Please try again in a moment.",
    });
  }
  console.warn("[auth/signup] Email OTP failed", emailResult, "wa=", waResult);
  return res.status(502).json({
    error: "We couldn't send your verification email right now. Please try again in a moment.",
  });
}));
...
router.post("/login", loginRateLimit, requireTurnstile, asyncHandler(async (req, res) => {
...
// GET /api/auth/anniversary-note — soft banner for logged-in owners ~1 year after delivery.
router.get("/anniversary-note", requireAuth, asyncHandler(async (req, res) => {
  const { rows } = await query(
    `SELECT order_number, items, updated_at
     FROM orders
     WHERE customer_id = $1
       AND status = 'delivered'
       AND updated_at BETWEEN now() - interval '380 days' AND now() - interval '350 days'
     ORDER BY updated_at ASC
     LIMIT 1`,
    [req.customer.id]
  );
  if (!rows.length) return res.json({ show: false });
  let items = rows[0].items;
  if (typeof items === "string") {
    try { items = JSON.parse(items); } catch { items = []; }
  }
  const name = Array.isArray(items) && items[0]?.name ? items[0].name : null;
  res.json({
    show: true,
    orderNumber: rows[0].order_number,
    pieceName: name,
    deliveredAround: rows[0].updated_at,
  });
}));
...
router.put("/profile", requireAuth, asyncHandler(async (req, res) => {
  const { name } = req.body || {};
  if (!validPersonName(name)) {
    return res.status(400).json({ error: "Enter a valid name (2–80 letters; spaces, apostrophe, hyphen allowed)." });
  }
  const cleanName = cleanPersonName(name);
  const { rows } = await query(
    "UPDATE customers SET name=$1 WHERE id=$2 RETURNING id, name, email, phone",
    [cleanName, req.customer.id]
  );
...
router.post("/forgot-password", loginRateLimit, requireTurnstile, asyncHandler(async (req, res) => {
...
router.post("/reset-password", otpVerifyRateLimit, asyncHandler(async (req, res) => {
...
router.post("/phone-change/request", requireAuth, publicFormRateLimit, asyncHandler(async (req, res) => {
  ...
  // Prefer WhatsApp to the NEW number; always fall back to the account email
  // (Google/password accounts always have email). Previous logic only emailed
  // when Gupshup template env was missing — so WA "sent" failures never emailed.
  let result = await sendPhoneChangeOTPWhatsApp(cleanPhone, otp);
  let channel = "whatsapp";
  if (result?.skipped || result?.ok === false) {
    if (!req.customer.email) {
      return res.status(502).json({
        error: "Couldn't send a code on WhatsApp, and this account has no email on file. Add an email or try again later.",
      });
    }
    result = await sendPhoneChangeOTPEmail(req.customer.email, otp);
    channel = "email";
  }
  if (result?.skipped || result?.ok === false) {
    return res.status(502).json({
      error: "Couldn't send the verification code. Check your email inbox (and spam), or try again in a moment.",
    });
  }
  res.json({
    ok: true,
    ...
    message: channel === "email"
      ? "Code sent to your account email. Enter it below to confirm this mobile number."
      : "Code sent on WhatsApp to the new number.",
  });
}));
...
router.delete("/account", requireAuth, asyncHandler(async (req, res) => {
  ...
  const { rows } = await query(
    "SELECT id, email, phone, name, password_hash, created_at FROM customers WHERE id=$1",
    [req.customer.id]
  );
  if (rows.length === 0) return res.status(404).json({ error: "Account not found." });
  ...
  if (!cust.password_hash) {
    return res.status(400).json({
      error: "This account signs in with Google only. Contact support@akaraonline.co.in to close it.",
    });
  }
  const ok = await verifyPassword(password, cust.password_hash);
  if (!ok) return res.status(401).json({ error: "Incorrect password." });

  // Snapshot for statutory retention (orders/tax/disputes) before login is disabled.
  const { rows: addrRows } = await query(
    "SELECT name, line, landmark, city, state, pin, phone, email, gstin FROM addresses WHERE customer_id=$1",
    [cust.id]
  ).catch(() => ({ rows: [] }));
  const { rows: orderRows } = await query(
    "SELECT order_number, total, status, payment_status, placed_at FROM orders WHERE customer_id=$1 ORDER BY placed_at DESC LIMIT 200",
    [cust.id]
  ).catch(() => ({ rows: [] }));

  const orderNumbers = orderRows.map((o) => o.order_number).filter(Boolean);
  const snapshot = {
    closedAt: new Date().toISOString(),
    accountCreatedAt: cust.created_at,
    orderSummaries: orderRows.map((o) => ({
  ...
  await query(
    `INSERT INTO deleted_customer_archives
      (former_customer_id, email, phone, name, addresses_json, order_numbers, snapshot, reason)
     VALUES ($1,$2,$3,$4,$5::jsonb,$6,$7::jsonb,'user_request')`,
    [
      cust.id,
      cust.email,
      cust.phone || null,
      cust.name || null,
      JSON.stringify(addrRows || []),
      orderNumbers,
      JSON.stringify(snapshot),
    ]
  ).catch(async (err) => {
    // Table may not exist until db:sync — still record minimal deletion log
    console.error("[auth] deleted_customer_archives insert failed:", err?.message || err);
  });

  await query("INSERT INTO account_deletions (customer_email) VALUES ($1)", [cust.email]).catch(() => {});

  // Removes login; orders keep customer_id NULL via FK; reviews become Verified Buyer
  ...
  res.json({
    ok: true,
    message:
      "Your login has been closed. A sealed copy of account details needed for tax, orders, and legal compliance is retained for up to 8 years and is not used for marketing.",
  });
}));
