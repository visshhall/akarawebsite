...
router.post("/signup/request", publicFormRateLimit, requireTurnstile, asyncHandler(async (req, res) => {
  ...
  // WhatsApp OTP (Gupshup) — primary verification channel for signup.
  // Email is no longer required to receive the code; account still stores email for login/orders.
  const result = await sendSignupOTPWhatsApp(normalizedPhone, otp, name.trim());
  if (result?.skipped || result?.ok === false) {
    // Fallback: email OTP if WhatsApp not configured / failed (keeps signup usable in dev).
    if (result?.skipped && !process.env.GUPSHUP_TEMPLATE_SIGNUP_OTP) {
      const emailResult = await sendSignupOTPEmail(cleanEmail, otp);
      if (emailResult?.skipped || emailResult?.ok === false) {
    ...
    return res.status(502).json({ error: "Couldn't send the WhatsApp verification code. Check the number and try again." });
  }
