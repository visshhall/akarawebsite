...
export async function checkAbandonedCheckouts() {
  ...
  for (const row of rows) {
    const order = {
      // Same real, defensive Array.isArray guard applied everywhere
      // else in this sweep — found and fixed as part of a full audit
      // for the bug class that broke product media uploads. Matters
      // more here than most: an unguarded crash inside this scheduled
      // job's own loop would stop it partway through the whole real,
      // current batch of abandoned-cart reminders, not just the one
      // malformed order.
      orderNumber: row.order_number, email: row.email, items: Array.isArray(row.items) ? row.items : [], total: row.total,
    };
