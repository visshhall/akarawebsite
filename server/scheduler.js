...
import {
  ...
  sendAnniversaryCareEmail,
} from "./email.js";

const MIN_AGE_HOURS = 2;
const MAX_AGE_HOURS = 48;
const CART_MIN_HOURS = 6;
...
export async function checkPostDeliveryCare() {
  ...
  for (const row of rows) {
    ...
    const order = {
      ...
      productId: Array.isArray(items) && items[0]?.id ? items[0].id : "",
    };
    await sendPostDeliveryCareEmail(order);
    await query(
      "UPDATE orders SET care_email_sent_at = now(), review_request_sent_at = now() WHERE order_number = $1",
      [row.order_number]
    );
    sentCount++;
  }
...
/** Delivered ~365 days ago (±3 day window), one email only. */
export async function checkAnniversaryCare() {
  // No delivered_at column yet — use updated_at at the moment status became delivered
  // (admin sets delivered; window ±3 days around the 365-day mark).
  const { rows } = await query(
    `SELECT * FROM orders
     WHERE status = 'delivered'
       AND anniversary_email_sent_at IS NULL
       AND email IS NOT NULL
       AND updated_at BETWEEN now() - interval '368 days' AND now() - interval '362 days'`
  );
  let sentCount = 0;
  for (const row of rows) {
    let addr = row.shipping_address;
    ...
    const result = await sendAnniversaryCareEmail(order);
    await query(
      "UPDATE orders SET anniversary_email_sent_at = now() WHERE order_number = $1",
      [row.order_number]
    );
    if (result?.ok !== false) sentCount++;
  }
...
export function startScheduler() {
  ...
  const run = async () => {
    try {
      ...
      const d = await checkAnniversaryCare();
      if (a || b || c || d) {
        console.log(`[scheduler] abandonedCheckout=${a} abandonedCart=${b} care=${c} anniversary=${d}`);
      }
    } catch (err) {
  ...
  console.log("[scheduler] started (abandoned checkout/cart + post-delivery care + anniversary)");
}
