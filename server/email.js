...
const emailWrapper = (bodyHtml) => `
  <div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;padding:32px 24px;background:#E3DAC9;">
    <p style="font-size:20px;font-style:italic;color:#183630;margin:0 0 24px;">ĀKĀRA</p>
    ${bodyHtml}
...
export async function sendOrderConfirmationEmail(order) {
  const html = emailWrapper(`
    <h1 style="font-size:22px;color:#183630;margin:0 0 8px;">Order Confirmed</h1>
    ...
    <p style="text-align:right;font-size:16px;color:#183630;margin-top:16px;"><strong>Total: ₹${order.total.toLocaleString("en-IN")}</strong></p>
...
export async function sendOrderStatusEmail(order, newStatus) {
  ...
  const html = emailWrapper(`
    <h1 style="font-size:22px;color:#183630;margin:0 0 8px;">Order Update</h1>
...
export async function sendAbandonedCheckoutEmail(order) {
  const html = emailWrapper(`
    <h1 style="font-size:22px;color:#183630;margin:0 0 8px;">You left something behind</h1>
    ...
    <table style="width:100%;border-collapse:collapse;font-size:14px;">${itemRows(order.items)}</table>
    <p style="text-align:right;font-size:16px;color:#183630;margin-top:16px;"><strong>Total: ₹${order.total.toLocaleString("en-IN")}</strong></p>
    <p style="margin-top:24px;"><a href="https://www.akaraonline.co.in/checkout" style="display:inline-block;background:#183630;color:#E3DAC9;padding:12px 24px;text-decoration:none;font-size:13px;">Complete Your Order</a></p>
  `);
...
export async function sendPasswordResetEmail(email, resetLink) {
  const html = emailWrapper(`
    <h1 style="font-size:22px;color:#183630;margin:0 0 8px;">Reset your password</h1>
    ...
    <p style="margin:28px 0;"><a href="${resetLink}" style="display:inline-block;background:#183630;color:#E3DAC9;font-size:13px;text-transform:uppercase;letter-spacing:0.08em;padding:14px 28px;text-decoration:none;">Reset Password</a></p>
...
export async function sendWelcomeEmail(email, name) {
  const html = emailWrapper(`
    <h1 style="font-size:22px;color:#183630;margin:0 0 8px;">Welcome to ĀKĀRA, ${name.split(" ")[0]}.</h1>
    <p style="font-size:14px;color:rgba(36,62,65,0.7);">Your account has been created successfully. You can now track orders, save addresses, and check out faster on your next visit.</p>
  `);
  return sendEmail({ to: email, subject: "Your ĀKĀRA account is ready", html });
}
