// ============================================================================
import { buildCarePlacementCardBase64 } from "./careCardPdf.js";
import { buildOrderInvoiceBase64 } from "./invoicePdf.js";
...
async function sendEmail({ to, subject, html, attachments }) {
  ...
  try {
    ...
    // Resend: attachments[].{ filename, content } where content is base64
    ...
    <tr><td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;background:#E3DAC9;border:1px solid rgba(24,54,48,0.1);">
        <tr><td style="padding:28px 28px 12px;text-align:center;border-bottom:1px solid rgba(24,54,48,0.1);">
          ...
          <p style="margin:8px 0 0;font-size:10px;letter-spacing:0.22em;text-transform:uppercase;color:#183630;font-family:Arial,sans-serif;opacity:0.85;">Artifacts for modern spaces</p>
        </td></tr>
...
export async function sendOrderConfirmationEmail(order) {
  ...
  // REAL, DIRECT GUEST CHECKOUT SUPPORT: for a genuine guest order, this
  // is THE real, actual delivery mechanism for the whole tracking
  // feature — without the real token in this specific link, a guest
  // would have no real way to reach their own order status at all,
  // since they have no real, logged-in session to rely on instead. A
  // logged-in customer's order correctly keeps the plain, existing
  // link — they track through their own real, authenticated account.
  // Always deep-link: token for guests, order= for logged-in (session may still be needed).
  // Bare /order-status shows "No order to track yet" with no order in memory.
  const trackingUrl = order.guestTrackingToken
    ? `https://www.akaraonline.co.in/order-status?token=${encodeURIComponent(order.guestTrackingToken)}`
    : (order.orderNumber
        ? `https://www.akaraonline.co.in/order-status?order=${encodeURIComponent(order.orderNumber)}`
        : "https://www.akaraonline.co.in/order-status");
  const guestNote = order.isGuestOrder
    ? `<p style="font-size:12px;line-height:1.6;color:rgba(24,54,48,0.65);margin:0 0 8px;">You checked out as a guest — keep this email safe, the button below is the real, direct way to check on this order. Want it saved to an account instead? You can still do that from your order confirmation page.</p>`
    : "";
  const html = emailWrapper(`
    <h1 style="font-size:26px;color:#183630;margin:0 0 6px;font-weight:normal;font-family:Georgia,serif;font-style:italic;">Thank you for your order</h1>
    <p style="font-size:11px;letter-spacing:0.18em;text-transform:uppercase;color:#183630;margin:0 0 18px;font-family:Arial,sans-serif;">Order #${order.orderNumber}</p>
    <p style="font-size:14px;line-height:1.75;color:rgba(24,54,48,0.8);margin:0 0 12px;">Your piece is with the Mumbai studio. We print and finish to order — not pulled from a shelf — so the form arrives as it was intended.</p>
    <table role="presentation" width="100%" style="margin:0 0 20px;border-collapse:collapse;">
      <tr>
        <td style="width:33%;padding:12px 8px;text-align:center;border:1px solid rgba(24,54,48,0.1);background:rgba(24,54,48,0.03);">
          <p style="margin:0;font-size:10px;letter-spacing:0.12em;text-transform:uppercase;color:rgba(24,54,48,0.55);font-family:Arial,sans-serif;">1 · Cast</p>
          <p style="margin:6px 0 0;font-size:12px;color:#183630;font-family:Arial,sans-serif;">Printed in studio</p>
        </td>
        <td style="width:33%;padding:12px 8px;text-align:center;border:1px solid rgba(24,54,48,0.1);background:rgba(24,54,48,0.03);">
          <p style="margin:0;font-size:10px;letter-spacing:0.12em;text-transform:uppercase;color:rgba(24,54,48,0.55);font-family:Arial,sans-serif;">2 · Finish</p>
          <p style="margin:6px 0 0;font-size:12px;color:#183630;font-family:Arial,sans-serif;">Hand-checked</p>
        </td>
        <td style="width:33%;padding:12px 8px;text-align:center;border:1px solid rgba(24,54,48,0.1);background:rgba(24,54,48,0.03);">
          <p style="margin:0;font-size:10px;letter-spacing:0.12em;text-transform:uppercase;color:rgba(24,54,48,0.55);font-family:Arial,sans-serif;">3 · Pack</p>
          <p style="margin:6px 0 0;font-size:12px;color:#183630;font-family:Arial,sans-serif;">Shipped with care</p>
        </td>
      </tr>
    </table>
    <p style="font-size:13px;line-height:1.65;color:rgba(24,54,48,0.72);margin:0 0 16px;">Studio work is usually <strong style="color:#183630;">2–3 days</strong>; most pieces reach you in <strong style="color:#183630;">about two weeks</strong>. We email at each clear step — production, QC, and when the courier takes over.</p>
    <p style="font-size:12px;line-height:1.6;color:rgba(24,54,48,0.65);margin:0 0 16px;">A <strong>Care &amp; Placement</strong> card and your <strong>purchase invoice</strong> are attached.</p>
    <p style="font-size:10px;letter-spacing:0.14em;text-transform:uppercase;color:rgba(24,54,48,0.5);margin:0 0 8px;font-family:Arial,sans-serif;">In this order</p>
    <table style="width:100%;border-collapse:collapse;font-size:14px;">${itemRows(order.items)}</table>
    <p style="text-align:right;font-size:18px;color:#183630;margin:18px 0 8px;font-family:Georgia,serif;"><strong>Total · ₹${Number(order.total).toLocaleString("en-IN")}</strong></p>
    <p style="font-size:12px;color:rgba(24,54,48,0.65);margin:0 0 8px;">${payNote}</p>
    ${guestNote}
    <p style="margin:24px 0 8px;text-align:center;"><a href="${trackingUrl}" style="display:inline-block;background:#183630;color:#E3DAC9;font-size:12px;letter-spacing:0.12em;text-transform:uppercase;text-decoration:none;padding:14px 28px;">Track your order</a></p>
  `, { preheader: `Order #${order.orderNumber} confirmed — ĀKĀRA` });
    let attachments = [];
  try {
    const content = await buildCarePlacementCardBase64(order);
    attachments.push({ filename: `AKARA-Care-Placement-${order.orderNumber || "order"}.pdf`, content });
  } catch (err) {
    console.error("[email] Care card PDF failed (confirmation still sends):", err?.message || err);
  }
  try {
    const inv = await buildOrderInvoiceBase64(order);
    attachments.push({ filename: `AKARA-Invoice-${order.orderNumber || "order"}.pdf`, content: inv });
  } catch (err) {
    console.error("[email] Invoice PDF failed (confirmation still sends):", err?.message || err);
  }
  if (!attachments.length) attachments = undefined;
  return sendEmail({
    to: order.email,
    subject: `Order confirmed — #${order.orderNumber} · ĀKĀRA`,
    html,
    attachments,
  });
}
...
export async function sendOrderStatusEmail(order, newStatus) {
  // Only customer-facing milestones — not every atelier step (production/qc).
  const STATUS_COPY = {
    dispatched: { title: "On its way", body: "Your order has been handed to our delivery partner. Track progress anytime from the link below." },
  ...
  const trackHref = order.guestTrackingToken
    ? `https://www.akaraonline.co.in/order-status?token=${encodeURIComponent(order.guestTrackingToken)}`
    : `https://www.akaraonline.co.in/order-status?order=${encodeURIComponent(order.orderNumber || "")}`;
  const html = emailWrapper(`
    ...
    <p style="margin:8px 0;text-align:center;"><a href="${trackHref}" style="display:inline-block;background:#183630;color:#E3DAC9;font-size:12px;letter-spacing:0.12em;text-transform:uppercase;text-decoration:none;padding:14px 28px;">View order status</a></p>
...
/** Notify a collector who joined the waitlist when a piece is available again. */
export async function sendWaitlistRestockEmail({ email, productName, productId }) {
  const url = productId
    ? `https://www.akaraonline.co.in/product/${encodeURIComponent(productId)}`
    : "https://www.akaraonline.co.in/shop";
  const html = emailWrapper(`
    <h1 style="font-size:22px;color:#183630;margin:0 0 10px;font-weight:normal;font-family:Georgia,serif;">Back in the atelier</h1>
    <p style="font-size:14px;line-height:1.7;color:rgba(24,54,48,0.78);margin:0 0 16px;">
      <strong>${escapeHtml(productName || "A piece you asked about")}</strong> is available again — made to order from our Mumbai studio.
    </p>
    <p style="margin:24px 0 8px;text-align:center;">
      <a href="${escapeHtml(url)}" style="display:inline-block;background:#183630;color:#E3DAC9;font-size:12px;letter-spacing:0.12em;text-transform:uppercase;text-decoration:none;padding:14px 28px;">View the piece</a>
    </p>
    <p style="font-size:12px;color:rgba(24,54,48,0.55);text-align:center;margin:12px 0 0;">You asked to be notified when this form returned. No further messages unless you join a waitlist again.</p>
  `, { preheader: `${productName || "Your piece"} is available again · ĀKĀRA` });
  return sendEmail({ to: email, subject: `${productName || "Your piece"} is available again · ĀKĀRA`, html });
}
...
export async function sendBulkOrderNotificationEmail({ company, name, email, phone, quantity, interest, message }) {
  const html = emailWrapper(`
    ...
    <p style="font-size:14px;color:rgba(36,62,65,0.7);white-space:pre-wrap;">${escapeHtml(message)}</p>
  `);
  return sendEmail({ to: "info@akaraonline.co.in", subject: `Trade / project enquiry — ${company || name}`, html });
}
...
// Logged-in cart still has items (server cart_items) — soft recovery.
export async function sendAbandonedCartEmail({ email, name, items }) {
  const first = (name || "").trim().split(/\s+/)[0] || "there";
  const lines = (items || []).slice(0, 5).map((i) =>
    `<li style="margin:4px 0;color:rgba(36,62,65,0.75);">${escapeHtml(i.name || "Piece")}${i.qty > 1 ? ` × ${i.qty}` : ""}</li>`
  ).join("");
  const html = emailWrapper(`
    <h1 style="font-size:24px;color:#183630;margin:0 0 8px;font-weight:normal;font-family:Georgia,serif;font-style:italic;">Your bag is still here, ${escapeHtml(first)}</h1>
    <p style="font-size:14px;color:rgba(36,62,65,0.78);line-height:1.75;margin:0 0 12px;">
      No rush — pieces you set aside stay on your account. When you are ready, the studio is.
    </p>
    <p style="font-size:11px;letter-spacing:0.12em;text-transform:uppercase;color:rgba(24,54,48,0.5);margin:0 0 8px;font-family:Arial,sans-serif;">Waiting quietly</p>
    <ul style="padding-left:18px;margin:0 0 16px;">${lines || "<li style=\"color:rgba(36,62,65,0.75);\">Your saved selection</li>"}</ul>
    <p style="font-size:12px;color:rgba(36,62,65,0.6);line-height:1.6;margin:0 0 8px;">This is the only reminder we send for this bag.</p>
    <p style="margin:20px 0;text-align:center;">
      <a href="https://www.akaraonline.co.in/cart" style="display:inline-block;background:#183630;color:#E3DAC9;font-size:12px;letter-spacing:0.12em;text-transform:uppercase;text-decoration:none;padding:14px 28px;">Return to bag</a>
    </p>
    <p style="font-size:12px;color:rgba(36,62,65,0.55);text-align:center;">Made to order in Mumbai · typically ships in about two weeks</p>
  `, { preheader: "Your selection is still saved" });
  return sendEmail({ to: email, subject: "Your selection is still saved · ĀKĀRA", html });
}

// A few days after delivery — care + quiet review invite (transactional-adjacent).
export async function sendPostDeliveryCareEmail(order) {
  const first = (order.name || "").trim().split(/\s+/)[0] || "there";
  const productId = order.productId || (Array.isArray(order.items) && order.items[0]?.id) || "";
  const reviewUrl = productId
    ? `https://www.akaraonline.co.in/product/${encodeURIComponent(productId)}#reviews`
    : "https://www.akaraonline.co.in/account";
  const html = emailWrapper(`
    <h1 style="font-size:22px;color:#183630;margin:0 0 8px;">Settling in, ${escapeHtml(first)}</h1>
    <p style="font-size:14px;color:rgba(36,62,65,0.75);line-height:1.7;">
      We hope #${escapeHtml(String(order.orderNumber || ""))} has found its place. A short care note:
      dust with a soft dry cloth; avoid harsh cleaners; keep out of prolonged direct heat where finishes may shift.
    </p>
    <p style="margin:16px 0;text-align:center;">
      <a href="https://www.akaraonline.co.in/care-guide" style="color:#183630;">Care guide</a>
      &nbsp;·&nbsp;
      <a href="https://www.akaraonline.co.in/account" style="color:#183630;">Your orders</a>
    </p>
    <p style="font-size:14px;color:rgba(36,62,65,0.78);line-height:1.7;margin-top:20px;">
      When you have a moment, a short review helps other collectors choose with confidence — no pressure.
    </p>
    <p style="margin:20px 0;text-align:center;">
      <a href="${escapeHtml(reviewUrl)}" style="display:inline-block;background:#183630;color:#E3DAC9;text-decoration:none;padding:12px 22px;border-radius:6px;font-size:13px;letter-spacing:0.06em;text-transform:uppercase;">Leave a review</a>
    </p>
  `, { preheader: "Care note + optional review · ĀKĀRA" });
  return sendEmail({ to: order.email, subject: `Care & a quiet note · #${order.orderNumber} · ĀKĀRA`, html });
}


/** One year after delivery — quiet care reminder + repair path (no promo). */
export async function sendAnniversaryCareEmail(order) {
  const first = (order.name || "").split(/\s+/)[0] || "there";
  const itemHint = Array.isArray(order.items) && order.items[0]?.name
    ? `Your ${order.items[0].name}`
    : "Your ĀKĀRA piece";
  const html = emailWrapper(`
    <h1 style="font-size:22px;color:#183630;margin:0 0 8px;">A year with your piece</h1>
    <p style="font-size:14px;color:rgba(36,62,65,0.75);line-height:1.75;">
      Hello ${escapeHtml(first)},
    </p>
    <p style="font-size:14px;color:rgba(36,62,65,0.75);line-height:1.75;">
      ${escapeHtml(itemHint)} from order #${escapeHtml(String(order.orderNumber || ""))} has been with you for about a year.
      A short care reminder: dust with a soft dry cloth; avoid harsh cleaners and prolonged direct heat.
    </p>
    <p style="font-size:14px;color:rgba(36,62,65,0.75);line-height:1.75;">
      If something needs attention — a finish touch-up or a quiet repair conversation — write to us.
      We would rather help early than leave a piece unloved.
    </p>
    <p style="margin:20px 0;text-align:center;">
      <a href="https://www.akaraonline.co.in/care-guide" style="display:inline-block;background:#183630;color:#E3DAC9;text-decoration:none;padding:12px 22px;border-radius:6px;font-size:13px;letter-spacing:0.06em;text-transform:uppercase;">Care guide</a>
      &nbsp;&nbsp;
      <a href="mailto:support@akaraonline.co.in?subject=${encodeURIComponent("Repair / care · #" + (order.orderNumber || ""))}" style="display:inline-block;border:1px solid #183630;color:#183630;text-decoration:none;padding:11px 20px;border-radius:6px;font-size:13px;letter-spacing:0.06em;text-transform:uppercase;">Ask about repair</a>
    </p>
    <p style="font-size:12px;color:rgba(36,62,65,0.55);line-height:1.6;margin-top:24px;">
      This is a one-time note for this order — not a sale, not a campaign.
    </p>
  `, { preheader: "A year with your piece · care note · ĀKĀRA" });
  return sendEmail({
    to: order.email,
    subject: `A year with your piece · #${order.orderNumber} · ĀKĀRA`,
    html,
  });
}
