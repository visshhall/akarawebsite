...
const itemRows = (items) => items.map(i =>
  `<tr><td style="padding:8px 0;color:#243E41;">${escapeHtml(i.name)}${i.size ? " (" + escapeHtml(i.size) + ")" : ""} × ${Number(i.qty)}</td>
...
export async function sendPhoneChangeOTPEmail(email, otp) {
  const html = emailWrapper(`
    <h1 style="font-size:22px;color:#243E41;margin:0 0 8px;">Verify your new phone number</h1>
    ...
    <p style="margin:28px 0;font-size:32px;letter-spacing:0.15em;font-weight:700;color:#243E41;">${otp}</p>
...
export async function sendEmailChangeOTPEmail(newEmail, otp) {
  const html = emailWrapper(`
    <h1 style="font-size:22px;color:#243E41;margin:0 0 8px;">Verify your new email address</h1>
...
export async function sendSignupOTPEmail(email, otp) {
  const html = emailWrapper(`
    <h1 style="font-size:22px;color:#243E41;margin:0 0 8px;">Verify your email to finish creating your account</h1>
...
export async function sendWelcomeEmail(email, name) {
  const html = emailWrapper(`
    <h1 style="font-size:22px;color:#243E41;margin:0 0 8px;">Welcome to ĀKĀRA, ${name.split(" ")[0]}.</h1>
