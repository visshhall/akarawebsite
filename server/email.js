...
export async function sendPostDeliveryCareEmail(order) {
  ...
  const html = emailWrapper(`
    ...
    <p style="font-size:13px;color:rgba(36,62,65,0.65);line-height:1.7;">
      If the piece already feels at home, a quiet rating from your account helps other collectors — no pressure.
    </p>
  `, { preheader: "A short care note from the studio" });
  return sendEmail({ to: order.email, subject: `Care for your piece · #${order.orderNumber} · ĀKĀRA`, html });
}
