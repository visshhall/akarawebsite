...
export async function createShipment(order, pickupLocationName) {
  if (!process.env.SHIPROCKET_EMAIL || !process.env.SHIPROCKET_PASSWORD) {
    ...
    return {
      skipped: true,
      ok: false,
      error: "Shiprocket credentials not set on the server (SHIPROCKET_EMAIL / SHIPROCKET_PASSWORD). Dispatch is blocked until credentials are configured.",
    };
  }
...
export async function fetchTrackingStatus(awb) {
  // WHAT: Live AWB track from Shiprocket (status + scan trail for the map).
  // WHY: Customer/admin map and status need current_status + location scans, not only webhook cache.
  // HOW: GET /courier/track/awb/{awb}; normalise shipment_track / track_activities into events[].
  // CONNECTED TO: orders.courier_*; OrderStatusView ShipmentTrackingMap; webhooks/courier.
  if (!process.env.SHIPROCKET_EMAIL || !process.env.SHIPROCKET_PASSWORD) {
    return { skipped: true };
  }
  ...
  try {
    const token = await getAuthToken();
    const res = await fetch(`${SHIPROCKET_API_BASE}/courier/track/awb/${encodeURIComponent(awb)}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    if (!res.ok) {
    ...
    const shipmentTrack = Array.isArray(track.shipment_track) ? track.shipment_track : [];
    const activities = Array.isArray(track.shipment_track_activities)
      ? track.shipment_track_activities
      : Array.isArray(track.track_activities)
    ...
    const currentStatus =
      ...
      "";
    const isDelivered = /delivered/i.test(String(currentStatus)) && !/rto/i.test(String(currentStatus));
    const events = [];
    for (const a of activities) {
      events.push({
        status: a.status || a.activity || a.sr_status || "",
        location: a.location || a.sr_location || a.origin || "",
        date: a.date || a.sr_status_date || a.updated_at || "",
        activity: a.activity || a.status || "",
      });
    }
    if (events.length === 0 && shipmentTrack[0]) {
      events.push({
        status: shipmentTrack[0].current_status || "",
        location: shipmentTrack[0].origin || shipmentTrack[0].destination || "",
        date: shipmentTrack[0].edd || shipmentTrack[0].updated_time || "",
        activity: shipmentTrack[0].current_status || "",
      });
    }
    return {
      ok: true,
      ...
      etd: shipmentTrack[0]?.etd || track.etd || null,
      raw: data,
    };
  } catch (err) {
