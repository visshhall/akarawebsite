  ...
  try {
    ...
    const res = await fetch(`${SHIPROCKET_API_BASE}/orders/create/adhoc`, {
      ...
      body: JSON.stringify({
        ...
        // Same real, defensive Array.isArray guard applied throughout
        // this audit — a malformed order.items reaching this point
        // would otherwise crash real courier booking outright, blocking
        // an order from ever being dispatched rather than just failing
        // gracefully.
        order_items: (Array.isArray(order.items) ? order.items : []).map(i => ({ name: i.name, sku: i.id, units: i.qty, selling_price: i.price, hsn: i.hsn })),
