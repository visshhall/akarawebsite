  ...
  try {
    ...
    if (!awb && shipmentId) {
      try {
        ...
        awb = extractAwb(awbData) || awbData?.response?.data?.awb_code || null;
      } catch (e) {
        console.warn("[courier] assign awb during findExisting:", e.message);
      }
    }
...
export async function createShipment(order, pickupLocationName) {
  ...
  try {
    ...
    // Many Shiprocket accounts create the order without an AWB until assign is called
    if (!awb && shipmentId) {
      try {
        const awbRes = await fetch(`${SHIPROCKET_API_BASE}/courier/assign/awb`, {
          method: "POST",
        ...
        let awbData = {};
        try {
          awbData = awbText ? JSON.parse(awbText) : {};
        } catch {
          awbData = {};
        }
        awb =
        ...
        if (!awb) {
          console.warn(
            `[courier] Order #${order.orderNumber} created in Shiprocket (shipment ${shipmentId}) but AWB not assigned yet: ${awbText.slice(0, 200)}`
          );
        }
      } catch (e) {
        console.warn(`[courier] AWB assign failed for #${order.orderNumber}:`, e.message);
      }
    }
