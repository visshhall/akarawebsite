...
export async function renderSeoHtml(pathname) {
  ...
  const paragraphs = String(seo.body || "")
    .split(/(?<=\.)\s+/)
    .filter(Boolean)
    .map((para) => `<p style="margin:0.75rem 0;line-height:1.6">${escapeHtml(para)}</p>`)
    .join("");
  const ssrBlock = `<div id="root"><main data-ssr="1" style="max-width:42rem;margin:2rem auto;padding:1.25rem;font-family:system-ui,sans-serif;color:#183630;background:#F5F0E8"><h1 style="font-size:1.5rem;margin-bottom:1rem">${escapeHtml(seo.title)}</h1>${paragraphs}<p style="margin-top:1.5rem"><a href="${escapeHtml(seo.canonical)}">${escapeHtml(seo.canonical)}</a></p><p style="font-size:0.85rem;opacity:0.7">ĀKĀRA — public storefront. Browsing does not require login.</p></main></div>`;
