...
const STATIC_META = {
  ...
  "/shipping": {
    ...
    body:
      "ĀKĀRA pieces are made to order in Mumbai / Thane. Studio work is typically 2–3 days; most pieces arrive in about two weeks. " +
...
export async function resolveSeo(pathname) {
  ...
  // Session / checkout flows — never index (matches robots.txt Disallow intent)
  const privatePrefixes = ["/admin","/cart","/checkout","/account","/login","/signup","/forgot-password","/reset-password","/order-confirmed","/order-status","/payment-failed","/invoice","/email-preferences","/preview","/search"];
  if (privatePrefixes.some((p) => pathOnly === p || pathOnly.startsWith(p + "/"))) {
    return {
      title: DEFAULT_TITLE,
      description: DEFAULT_DESC,
  ...
  if (productMatch) {
    ...
    // Draft / hidden / unknown product URL — do not index
    return {
      title: "Product not available — ĀKĀRA",
      description: DEFAULT_DESC,
      canonical: `${SITE}/product/${id}`,
      image: `${SITE}/icon-512.png`,
      type: "website",
