...
export async function buildCarePlacementCardBase64(order) {
  ...
  const careLines = [
    ...
    "For a deeper clean, a lightly damp cloth is fine — avoid soaking or submerging.",
    "Not dishwasher, microwave, or oven safe.",
    "Avoid prolonged direct sunlight — pigment can fade over months of constant exposure.",
    "Plant-based PLA: treat like a fine ceramic object, not commodity plastic.",
  ];
  ...
  doc.setFontSize(10);
  const placeLines = [
    "Stable, level surface. Leave a little air around lattice and ribbed forms.",
    "Planters: use the drainage tray; empty standing water. Prefer a nursery pot liner when drainage is limited.",
    "Lighting: switch off and cool before dusting; use only the bulb type noted for the piece; indoor use only unless stated otherwise.",
    "Full guide: akaraonline.co.in/care-guide",
  ];
