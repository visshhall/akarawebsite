...
export function ensureAnonId(req, res, next) {
  if (!req.cookies?.[ANON_ID_COOKIE]) {
    ...
    res.cookie(ANON_ID_COOKIE, id, {
      ...
      // Found in an independent security review: this was a year long,
      // longer than it needs to be for what this cookie actually does —
      // identify "this browser" for CSRF-token binding, not remember a
      // logged-in session (that's the separate, actual auth cookie in
      // auth.js). 30 days is plenty for a returning visitor to keep
      // working smoothly without a fresh page load, while meaningfully
      // shrinking how long a stolen/leaked value would stay useful.
